...
$PinnedPnpm = Join-Path $HOME '.xiaoshe-handoff\pnpm-11.7.0\node_modules\.bin\pnpm.cmd'
$Installer = Join-Path $XsRoot '交接工具\接收并安装-Windows.ps1'
...
if (-not (Test-Path -LiteralPath $PinnedPnpm) `
  ...
  Write-Host '[首次启动] 正在接收、重建并验证 Windows 依赖…' -ForegroundColor Cyan
  & $Installer
  if ($LASTEXITCODE -ne 0) { throw 'Windows 接收安装失败。' }
}
if (-not (Test-Path -LiteralPath $PinnedPnpm)) { throw '项目专用 pnpm 11.7.0 不存在，请重新运行 Windows 接收器。' }
