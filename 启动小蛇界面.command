#!/bin/bash
# 小蛇界面 · Mac 双击启动（正式模式：使用 .env 配置的活动提供商）
cd "$(dirname "$0")"
...
echo "[1/1] 启动小蛇界面（正式模式）…"
: > logs/ui-start.log
"$PY" run.py serve --no-browser > logs/ui-start.log 2>&1 &
SRV=$!

URL=""
for _ in $(seq 1 40); do
  sleep 0.5
  URL=$(grep -o 'http://127\.0\.0\.1:[0-9]*/?token=[a-f0-9]*' logs/ui-start.log 2>/dev/null | head -1)
  [ -n "$URL" ] && break
