...
function execCell(end) {
  if (!end) return { kind: "running", dot: "info", text: "执行中…", pulse: true };
...
export function renderToolResult(msg) {
  ...
  return el("div.toolresult", { dataset: { msgId: msg.msg_id ?? "" } },
    resultBody(msg.content, msg.is_error));
}
...
function paint(rec) {
  ...
  const execRow = el("div.tcard-row.exec", {},
    el(`i.dot.${exec.dot}${exec.pulse ? ".pulse" : ""}`),
