#!/usr/bin/env python3
"""Build and verify a deterministic, manifest-backed handoff ZIP."""
...
def _excluded(relative: Path) -> bool:
    ...
    if any(part in {".git", "_handoff", "dist", "work", "__pycache__"} for part in parts):
        return True
...
def build_package(output_dir: Path) -> dict[str, Any]:
    ...
    descriptor, temporary_name = tempfile.mkstemp(prefix=f".{archive_name}.", dir=output_dir)
    os.close(descriptor)
    temporary = Path(temporary_name)
    try:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        loaded_manifest = json.loads(archive.read("_handoff/manifest.json"))
        for entry in loaded_manifest["files"]:
    ...
    relative_archive = archive_path.relative_to(PROJECT_ROOT).as_posix()
    ...
    receipt = {
        ...
        "verification": "manifest-and-sidecar-match",
        "rollback": contract["rollback"],
    }
    atomic_write_json(PROJECT_ROOT / "evidence/handoff.json", receipt)
    return receipt
