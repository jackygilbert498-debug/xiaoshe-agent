...
async function jsonTransferFixture(t, { prepare = true, read = true, goal = JSON_DELIVERY_GOAL, extraField = false, store = true, outputContent, checkpointBrowser = false } = {}) {
  ...
  let ownedContext, ownedSession, ownedEndpoint
  t.after(async () => {
    if (ownedContext && ownedSession && store) await ownedContext.parallel('session/flush', ownedSession)
    if (ownedContext) await ownedContext.fiber.dispose()
  ...
  if (store) new JsonlSessionPersistence(ctx, {
  ...
  const fixture = { ctx, session, agent, controller: new AbortController(), root, cwd, document, content,
    typed: [], dispatchedWrites: [], turn: 1, mode: 'agent', requests: [], statusHook: undefined }
