...
/** One bounded recovery belongs to the tool, not an untracked shell detour.
 * Reuse the same authorized input only after the old process group is gone. */
export async function runVisionWithRecovery(command, args, signal, timeoutMs = 60_000) {
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 120_000) throw failure('VISION_INVALID_BUDGET', '视觉读取预算必须为 1–120000 毫秒。')
  const deadline = Date.now() + Math.min(timeoutMs * 2, 120_000)
  const input = await inputFingerprint(args)
  const attempts = []
  for (let attempt = 0; attempt < 2; attempt++) {
    signal?.throwIfAborted()
    const started = Date.now()
    const remaining = Math.min(timeoutMs, deadline - started)
    if (remaining <= 0) break
    try {
      const result = await runVision(command, args, signal, remaining)
      result.recovery = { attempts: [...attempts, { code: result.code === 0 ? 'RETURNED' : 'PROCESS_FAILED', elapsedMs: Date.now() - started }] }
      return result // Preserve the private provenance object, not a copied receipt.
    } catch (error) {
      attempts.push({ code: error.code ?? 'UNKNOWN', elapsedMs: Date.now() - started })
      if (error.code !== 'VISION_TIMEOUT' || error.cleanup?.status !== 'confirmed' || signal?.aborted) throw error
      const current = input && await inputFingerprint(args)
      if (!input || current?.sha256 !== input.sha256) throw error
      if (attempt === 1 || Date.now() >= deadline) break
      // Operational facts only: no image path, prompt, output or credentials.
      console.error('[xiaoshe-vision-recovery]', JSON.stringify({ event: 'retry-after-timeout', attempt: attempt + 1,
        elapsedMs: Date.now() - started, cleanup: 'confirmed', inputUnchanged: true }))
    }
  }
  throw Object.assign(failure('VISION_TIMEOUT_EXHAUSTED',
    '[VISION_TIMEOUT_EXHAUSTED] 视觉读取超时，工具内的有界恢复预算已用尽，未取得图片内容。不要立即重复同一调用或改用命令行延长；浏览器截图、重新上传仍可能使用同一引擎，不能承诺解决故障。保留任务并明确报告视觉通道超时，需要修复通道或使用已验证且获授权的独立视觉路线，不要求用户重新提供已存在的图片。'), { attempts })
}
...
export async function runVision(command, args, signal, timeoutMs = 60_000) {
  ...
  const deadline = setTimeout(() => stop(failure('VISION_TIMEOUT',
    `[VISION_TIMEOUT] 视觉读取超时：${Math.round(timeoutMs / 1000)} 秒内未返回，正在终止并清理本次读取；没有取得图片内容。`)), timeoutMs)
