...
@xiaoshe/project-knowledge|$PLUGIN_ROOT/packages/project-knowledge
...
sync_current_product_packages() {
  ...
  (
    ...
    "$NODE" "$PNPM_CLI" --dir "$DSH_ROOT" dsh plugin --profile "$PROFILE" add \
      ...
      "$PLUGIN_ROOT/packages/project-knowledge" \
...
# A copied node_modules directory may contain foreign pnpm paths, dangling
# links or non-executable shims. Directory existence is not build readiness.
require_file "$PLUGIN_ROOT/scripts/check-macos-dependencies.mjs"
"$NODE" "$PLUGIN_ROOT/scripts/check-macos-dependencies.mjs" "$PLUGIN_ROOT" "$DSH_ROOT" \
  || fail '本机依赖不可用；已停止，不运行构建或切入旧应用。'
