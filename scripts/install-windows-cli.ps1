...
# The desktop shell may bootstrap an isolated, versioned runtime. That repair
# must never repoint the user's persistent s/ss commands at the disposable
# runtime directory; only an explicit Windows setup owns command installation.
if ($env:XIAOSHE_SETUP_SKIP_CLI_INSTALL -eq '1') {
  [pscustomobject]@{
    schema = 'xiaoshe-windows-cli/v1'
    binPath = $BinPath
    skipped = $true
    reason = 'isolated-service-bootstrap'
