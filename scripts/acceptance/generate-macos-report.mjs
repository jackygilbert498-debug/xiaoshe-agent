...
const report = {
  ...
  checks: [
    ...
    { id: 'macos-app-launch', state: 'pending_external', detail: '在 macOS 真机启动独立窗口并验证单实例、托盘和正常退出。', evidence: {} },
    { id: 'screen-and-accessibility-permissions', state: 'pending_external', detail: '需要真实宿主授予屏幕录制与辅助功能权限。', evidence: {} },
    { id: 'macos-signing-and-notarization', state: 'pending_external', detail: '需要 Apple Developer ID、hardened runtime 签名与公证凭据。', evidence: {} },
    { id: 'install-update-uninstall', state: 'pending_external', detail: '需要在干净 macOS 用户环境验证 DMG/ZIP 安装、无更新 feed 默认态与卸载残留策略。', evidence: {} },
  ],
}
...
if (testState === 'fail') process.exitCode = 1
