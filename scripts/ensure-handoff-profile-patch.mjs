...
function parseArgs(argv) {
  ...
  for (let index = 2; index < argv.length; index += 2) {
    ...
    if (!['--target', '--template'].includes(key) || !value) {
      throw new Error('用法：node scripts/ensure-handoff-profile-patch.mjs --target <cordis.patch.yml> --template <template.yml>')
    }
...
try {
  ...
  if (containsModLensPatch(current)) {
    ...
    const next = body === '' || body === '[]'
      ...
      : `${current.trimEnd()}\n\n# 小蛇交接工具追加：ModLens 使用 DeepSeek 官方 Provider。\n${templateValue.trimEnd()}\n`
