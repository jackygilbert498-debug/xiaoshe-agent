...
LAUNCH_OUTPUT="$(XIAOSHE_DSH_NO_OPEN=1 XIAOSHE_DSH_NO_PAUSE=1 bash "$WEB_STARTER" --ownership-report)"

NODE="$(find_node || true)"
[ -n "$NODE" ] || fail '需要 Node.js 22.23+ 或 24.17+。请先运行 setup/install-macos.sh。'
...
LOGIN_URL="$(printf '%s' "$LAUNCH_OUTPUT" | "$NODE" -e 'let text="";process.stdin.on("data",c=>text+=c);process.stdin.on("end",()=>{try{const rows=text.split(/\r?\n/).filter(r=>r.startsWith("XIAOSHE_LAUNCH_OWNERSHIP="));if(rows.length!==1)throw 0;const value=JSON.parse(rows[0].slice("XIAOSHE_LAUNCH_OWNERSHIP=".length));if(typeof value.loginUrl!=="string")throw 0;process.stdout.write(value.loginUrl)}catch{process.stderr.write("Missing Host login handoff\n");process.exitCode=1}})')" || fail '后台未提供登录链接'
XIAOSHE_AUTH_URL="$LOGIN_URL" exec "$NODE" "$TERMINAL_ENTRY" --url "http://127.0.0.1:$PORT" "$@"
