...
find_pnpm_cli() {
  ...
  for candidate in \
    "${XIAOSHE_PNPM_CLI:-}" \
    "${HOME}/.local/share/xiaoshe-handoff/pnpm-11.7.0/node_modules/pnpm/bin/pnpm.cjs" \
...
NODE="$(find_node || true)"
[ -n "$NODE" ] || fail '需要 Node.js 24。请先运行交接工具/接收并安装-macOS.command。'
...
if [ "$NEEDS_BUILD" = 1 ]; then
  ...
  [ -n "$PNPM_CLI" ] || fail '缺少项目锁定的 pnpm 11.7.0；请运行交接安装器。'
