...
import { configureVision, visionTimeout, runVision, runVisionWithRecovery, releaseVisionProcessGroup, createVisionRead, waitVision, latestVisionQuestion, createVisionEvidenceScope, visionEvidenceScopeKey, createVisionEvidenceBlock, visionEvidenceRequest } from './modlens-vision-runtime.mjs'
...
async function recoveryFixture(t, mode) {
  const dir = await mkdtemp(join(tmpdir(), 'xs-vision-recovery-'))
  t.after(() => rm(dir, { recursive: true, force: true }))
  const input = join(dir, 'input.png'), cli = join(dir, 'cli.mjs'), count = join(dir, 'count')
  await writeFile(input, imageBytes)
  await writeFile(cli, `import {readFileSync,writeFileSync} from 'node:fs';
    let n=0;try{n=Number(readFileSync(${JSON.stringify(count)},'utf8'))}catch{}
    writeFileSync(${JSON.stringify(count)},String(++n));
    const mode=${JSON.stringify(mode)};
    if(mode==='exit'){process.exitCode=2;}
    else if(mode==='change'){writeFileSync(process.argv[3],'changed');setInterval(()=>{},1000);}
    else if(mode==='hang'||n===1){setInterval(()=>{},1000);}
    else console.log(JSON.stringify({image:process.argv[3],provider:'fixture',meta:{model:null},result:{summary:'read',uncertainty:[]}}));`)
  return { args: [cli, '-i', input], count: async () => Number(await readFile(count, 'utf8')) }
}

test('owned timeout recovers once after cleanup with unchanged local input', async t => {
  const f = await recoveryFixture(t, 'recover')
  const result = await runVisionWithRecovery(process.execPath, f.args, undefined, 1000)
  assert.equal(await f.count(), 2)
  assert.equal(result.code, 0)
  assert.deepEqual(result.recovery.attempts.map(row => row.code), ['VISION_TIMEOUT', 'RETURNED'])
  assert.equal(result.cleanup.status, 'confirmed')
  const proof = provenanceRequest()
  const block = createVisionEvidenceBlock({ original: proof.block, stored: proof.stored, run: result, scope: proof.scope, render: value => value.summary })
  assert.match(block.text, /read/)
})

test('second timeout ends recovery; nonzero exit and changed input never retry', async t => {
  const hanging = await recoveryFixture(t, 'hang')
  await assert.rejects(runVisionWithRecovery(process.execPath, hanging.args, undefined, 700), error => {
    assert.equal(error.code, 'VISION_TIMEOUT_EXHAUSTED'); assert.equal(error.attempts.length, 2); return true
  })
  assert.equal(await hanging.count(), 2)
  const exit = await recoveryFixture(t, 'exit')
  assert.equal((await runVisionWithRecovery(process.execPath, exit.args, undefined, 700)).code, 2)
  assert.equal(await exit.count(), 1)
  const changed = await recoveryFixture(t, 'change')
  await assert.rejects(runVisionWithRecovery(process.execPath, changed.args, undefined, 700), { code: 'VISION_TIMEOUT' })
  assert.equal(await changed.count(), 1)
})

test('cancelled recovery cleans up without launching a second process', async t => {
  const f = await recoveryFixture(t, 'hang'), controller = new AbortController()
  const timer = setTimeout(() => controller.abort(), 300)
  try { await assert.rejects(runVisionWithRecovery(process.execPath, f.args, controller.signal, 1000), { code: 'VISION_CANCELLED' }) }
  finally { clearTimeout(timer) }
  assert.equal(await f.count(), 1)
})

test('recovery rejects invalid budgets before launching', async () => {
  for (const timeout of [0, -1, NaN, Infinity, 120001, 1.5]) {
    await assert.rejects(runVisionWithRecovery(process.execPath, [], undefined, timeout), { code: 'VISION_INVALID_BUDGET' })
  }
})
