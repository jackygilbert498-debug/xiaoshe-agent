...
test('bounds repeated failure while allowing changed arguments and ordinary coding', () => {
  const c = new RecoveryController(); const a = agent(); const e = execution(a)
  c.result(e, failure()); assert.equal(c.denial(e), undefined)
  c.result(e, failure()); assert.match(c.denial(e), /同一调用已失败/)
...
test('blocks shell vision bypass but not legitimate environment queries or unrelated work', () => {
  const c = new RecoveryController(); const a = agent()
  c.result(execution(a, 'modlens_read_image'), failure('The operation was aborted'))
  for (const [tool, command] of [
    ['bash', 'modlens -i /image.png --timeout 999999'],
    ['pwsh', 'node /pkg/modlens/dist/main.js --input=/image.png'],
    ['bash', 'npx --yes @liustack/modlens doctor'],
    ['pwsh', 'pnpm dlx @liustack/modlens doctor'],
    ['bash', 'codex exec --image /image.png "read"'],
    ['pwsh', '"/Applications/ChatGPT.app/Contents/Resources/codex" exec -i /image.png "read"'],
  ]) {
    assert.match(c.denial(execution(a, tool, { command })), /不能改用命令行/)
  }
...
test('str_replace_editor view selects a filesystem_read recovery route', () => {
  const c = new RecoveryController(); const a = agent()
  ...
  assert.equal(c.state(a).routeRecovery.alternativeFamily, 'filesystem_read')
})
...
test('prevents synonymous capability-plan loops until a non-advisory tool produces new evidence', () => {
  const c = new RecoveryController(); const a = agent()
  ...
  assert.match(c.denial(plan), /已经给出|新证据/)
  assert.match(c.denial(execution(a, 'xiaoshe_capability_plan', { goal: '帮我查一下最近的消息' })), /已经给出|新证据/)
  c.result(execution(a, 'xiaoshe_runtime_info'), success)
  assert.match(c.denial(execution(a, 'xiaoshe_capability_plan', { goal: '换个说法，查近期消息' })), /已经给出|新证据/)
...
test('turn changes preserve one task recovery state until a genuinely new goal resets it', () => {
  ...
  c.begin(a, 2); assert.match(c.denial(execution(a)), /同一调用已失败/)
...
test('permanent capability/permission errors do not burn another identical call', () => {
  for (const reason of ['model does not declare image input', 'permission denied']) {
    const c = new RecoveryController(); const a = agent(); const e = execution(a)
    c.result(e, failure(reason)); assert.ok(c.denial(e))
  }
})
...
test('an explicitly requested standalone verifier remains available under write-path constraints', async () => {
  ...
  const unrequested = await events.get('tools/pre-execute')(execution(a, 'pwsh', {
    command: 'npm run lint', workdir: 'C:\\work',
  }), allow)
  assert.equal(unrequested.kind, 'deny')
  assert.match(unrequested.reason, /任务路径约束.*sandbox_permissions/)
...
test('foreground shell exit metadata overrides a transport-level success flag', () => {
  ...
  c.result(failedCommand, nonzero)
  assert.match(c.denial(failedCommand), /失败|不再重复/)
