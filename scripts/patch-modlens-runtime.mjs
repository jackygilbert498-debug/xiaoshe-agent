...
export function patchSource(source) {
  if (source.includes('// xiaoshe-vision-deadline-v2')) return patchBoundedRecovery(patchPreparedCalls(patchProviderDirectory(patchEvidenceOrigin(patchTaskFocus(patchToolGuidance(patchSharedReads(source)))))))
  ...
  return patchBoundedRecovery(patchPreparedCalls(patchProviderDirectory(patchEvidenceOrigin(patchTaskFocus(patchToolGuidance(patchSharedReads(source)))))))
}

function patchBoundedRecovery(source) {
  if (source.includes('// xiaoshe-vision-recovery-v1')) return source
  source = once(source, 'configureVision, visionTimeout, runVision,', 'configureVision, visionTimeout, runVisionWithRecovery,')
  source = once(source, 'timeoutMs: cliTimeoutMs + 5_000,', 'timeoutMs: Math.min(cliTimeoutMs * 2, 120_000) + 10_000,')
  source = once(source, 'return runVision(command, args, signal, timeoutMs)', '// xiaoshe-vision-recovery-v1\n  return runVisionWithRecovery(command, args, signal, timeoutMs)')
  return source
}
