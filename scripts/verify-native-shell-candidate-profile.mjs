...
const candidateTar = join(artifacts, 'xiaoshe-native-shell-candidate-0.1.0.tgz')
const overlayTar = join(artifacts, 'xiaoshe-native-shell-candidate-bundle-0.1.0.tgz')
const productTar = join(artifacts, 'xiaoshe-product-bundle-0.1.0.tgz')
const candidateClientPath = '/plugins/@xiaoshe%2Fnative-shell-candidate/client.js'
...
await runNodeBuild('native-shell-candidate', 'scripts/build-client.mjs')
await runTypeScript('native-shell-candidate', 'tsconfig.build.json')
await runTypeScript('native-shell-candidate-bundle', 'tsconfig.json')
progress('pack exact candidate artifacts')
await packPackage('native-shell-candidate')
await packPackage('native-shell-candidate-bundle')
...
const candidatePresent = dump.includes('xiaoshe-native-shell-candidate')
...
async function probeServer(url) {
  const [rootResponse, candidateClientResponse, candidateBrandResponse, heartbeatResponse] = await Promise.all([
    ...
    fetch(`${url}/api/xiaoshe/candidate-brand-icon`),
...
async function packPackage(packageName) {
  ...
  try {
    ...
    if (packageName === 'native-shell-candidate-bundle') {
      manifest.dependencies['@xiaoshe/native-shell-candidate'] = fileSpec(candidateTar)
    }
...
function progress(message) {
  process.stderr.write(`[native-shell-candidate-profile] ${message}\n`)
}
