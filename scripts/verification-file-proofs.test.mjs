...
import { attachSessionLog, loadSessionLog } from './helpers/session-persistence.mjs'
...
async function runColdChild(root, phase, scenario) {
  ...
  if (phase === 'seed') session = ctx.sessions.create(sessionId, { meta: { cwd: workspace } })
    ...
    loaded = await loadSessionLog(persistence, sessionId)
    assert.ok(loaded)
  ...
  const log = await attachSessionLog(ctx, persistence, session, { existing: phase !== 'seed' && scenario !== 'other-session' })
  ...
  } finally { try { await log.close() } finally { await ctx.fiber.dispose() } }
}
