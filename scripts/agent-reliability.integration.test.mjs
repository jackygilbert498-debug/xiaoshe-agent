...
test('real DSH registry accepts schemas and enforces retry limit without fabricating success', async t => {
...
test('real DSH admits an evidence-complete edit and its explicitly requested verifiers without todo churn', async t => {
  ...
  assert.equal((await call(ctx, agent, 'pwsh', { command: 'npm run lint', workdir: 'C:\\work' })).isError, true)
  assert.equal(verifierRuns, 1)
})
...
test('capability planning stays revision-bounded without withdrawing earlier tools', async t => {
  ...
  assert.equal((await call(ctx, agent, 'xiaoshe_capability_plan', { goal: '换个说法，调用 mcp__service1__action' })).isError, true)
  assert.equal((await call(ctx, agent, 'xiaoshe_runtime_info')).isError, false)
  assert.equal((await call(ctx, agent, 'xiaoshe_capability_plan', { goal: '再换个说法，调用 mcp__service2__action' })).isError, true)
...
test('route recovery resumes from the Session Log and closes only after a durable alternative result', async t => {
  const ctx = harness(t)
  ctx.tools.register({
  ...
  assert.deepEqual(routes.slice(-3).map(event => event.data.status), ['needs-alternative', 'needs-proof', 'satisfied'], JSON.stringify(routes, null, 2))
  assert.equal(routes.at(-1).data.proofResultSeq, events.find(event => event.type === 'tool/result' && event.data.message.source.callId === callId).seq)
  assert.equal((await call(ctx, agent, 'xiaoshe_runtime_info')).value.execution.route_changes, 1)
})
...
test('real capability planning consumes optional experience only for matching recovery candidates', async t => {
  const ctx = new Context()
  new SystemPrompt(ctx, { includeHarnessIdentity: false })
  new ToolRuntime(ctx)
  const queries = []
  ...
  assert.deepEqual(plan.value.candidates.filter(candidate => candidate.name.startsWith('browser_')).map(candidate => candidate.name), ['browser_beta', 'browser_alpha'])
  assert.ok(queries.some(query => query.failedFamily === 'web_search' && query.presetId === 'standard'))
  assert.ok(queries.every(query => query.candidates.every(candidate => ['web_search', 'browser_alpha', 'browser_beta'].includes(candidate.tool))))
})
