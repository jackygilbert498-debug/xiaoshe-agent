...
export class TerminalApp {
  ...
  private async printBanner(): Promise<void> {
    ...
    write(this.streams.output, `${c.dim}:help 帮助 · :tasks 任务图 · :queue 队列 · :effort 思考强度 · :stop 停止 · :details 工具明细\n运行中输入默认排队；:steer <文本> 立即调整。:exit 退出，/ 查看会话命令。${c.reset}\n\n`)
  }
  ...
  private async loop(): Promise<void> {
    while (true) {
      ...
      if (input === ':tasks') { await this.printTasks(); continue }
  ...
  private async printTasks(): Promise<void> {
    if (this.active === undefined) return
    const history = await this.api.history(this.active.sessionId, 20)
    const lines = taskGraphStatusLines(history.projections, this.active.sessionId)
    if (lines.length === 0) {
      write(this.streams.output, '当前会话没有可用任务图。\n')
      return
    }
    for (const line of lines) write(this.streams.output, `${line}\n`)
  }
