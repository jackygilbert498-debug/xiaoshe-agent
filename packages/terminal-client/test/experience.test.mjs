...
async function fixture(t, options = {}) {
  ...
  const snapshot = () => ({ type: 'snapshot', cursor: Math.max(seq, hostCursor), records: [], hasMore: false, projections: { asOfSeq: Math.max(seq, hostCursor), values: { modelSelection: { next: catalog.default, lastUsed: null }, ...(options.taskGraph === undefined ? {} : { taskGraph: options.taskGraph }) } } })
...
test(':tasks reads the existing history projection without creating execution state', async t => {
  const taskGraph = {
    version: 1, id: 'terminal-graph', revision: 1, sessionId: 'session-a', taskGeneration: 1,
    goalId: null, objective: '终端图', runtimeInstance: 'runtime-a', durability: 'durable', status: 'ready', stale: false, recoveryRequired: false,
    nodes: [{ id: 'next', title: '下一步', dependencies: [], acceptance: [{ id: 'a1', text: '完成' }], status: 'pending', attempt: 0, startSeq: null, evidence: [], feedback: [] }], feedback: [],
  }
  const f = await fixture(t, { taskGraph })
  f.send(':tasks')
  await until(() => f.text().includes('下一步'))
  assert.match(f.text(), /任务图：0 \/ 1/u)
  assert.equal(f.calls.filter(row => row.endpoint === 'session/prompt').length, 0)
