...
test('MigrationExporter emits hashed portable facts without secrets', async () => {
  ...
  const exporter = new MigrationExporter({
    ...
    settings: { describe() { return [{ ns: 'models', user: { apiKey: 'must-not-export', theme: 'light' }, status: 'ready', secrets: [['apiKey']] }] } },
  ...
  assert.deepEqual(JSON.parse(await readFile(join(target, 'settings.json'), 'utf8')), [
    { ns: 'models', status: 'ready', user: { theme: 'light' } },
  ])
})
