...
test('model catalog loads independently of history without inventing a selection, then follows projection changes', async () => {
  const { ProjectionValueStore } = await loadUpstream('packages/api/session-controller/src/client/sessions/projection-store.ts')
  const store = new ProjectionValueStore()
  const sessions = { list: { getSnapshot: () => ({ current: 'one' }), subscribe: () => () => {} }, binding: () => ({ session: { projections: store } }) }
  let requests = 0
  const connection = provider.createRemoteConnection({ session: { modelCatalog: async () => {
    requests++
    return { ok: true, value: { default: { provider: 'p', model: 'default' }, routableProviders: ['p'], groups: [{ id: 'p', name: 'Provider', models: [{ id: 'default', name: 'Flash' }] }], failures: [] } }
  } } }, sessions)
  const catalog = new provider.DshModelCatalog(sessions, connection)
  const settle = () => new Promise(resolve => setImmediate(resolve))
  await settle()
  assert.equal(catalog.getSnapshot().status, 'ready')
  assert.equal(catalog.getSnapshot().groups[0].models[0].name, 'Flash')
  assert.equal(catalog.getSnapshot().current, undefined)
  assert.equal(catalog.getSnapshot().routable, undefined)
...
test('a catalog failure during history loading surfaces an error and can be retried', async () => {
  const sessions = { list: { getSnapshot: () => ({ current: 'one' }), subscribe: () => () => {} }, binding: () => ({ session: { projections: { faceOf: () => ({ getSnapshot: () => undefined }) } } }) }
  let failed = true
  const connection = provider.createRemoteConnection({ session: { modelCatalog: async () => {
    if (failed) throw new Error('model service unavailable')
    return { ok: true, value: { default: { provider: 'p', model: 'flash' }, routableProviders: ['p'], groups: [{ id: 'p', name: 'Provider', models: [{ id: 'flash', name: 'Flash' }] }], failures: [] } }
  } } }, sessions)
  const catalog = new provider.DshModelCatalog(sessions, connection)
  await new Promise(resolve => setImmediate(resolve))
  assert.equal(catalog.getSnapshot().status, 'error')
  assert.match(catalog.getSnapshot().error, /model service unavailable/)
  failed = false
  assert.equal((await catalog.refresh()).ok, true)
  assert.equal(catalog.getSnapshot().status, 'ready')
  assert.equal(catalog.getSnapshot().current, undefined)
  assert.equal(catalog.getSnapshot().groups[0].models[0].id, 'flash')
  catalog.dispose()
})
