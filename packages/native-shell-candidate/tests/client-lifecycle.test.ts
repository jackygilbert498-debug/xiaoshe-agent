...
describe('candidate client lifecycle', () => {
  it('owns only the candidate root seat and releases it', () => {
    ...
    expect([...seats.keys()]).toEqual(['root:xiaoshe-native-shell-candidate'])
    dispose()
  ...
  it('aborts stale searches and refuses work after disposal', async () => {
    ...
    const coordinator = createSearchCoordinator(async (_query, signal) => { signals.push(signal); return { ok: true, value: { items: [] } } })
  ...
  it('projects Xiaoshe browser title and canonical favicon, survives host rewrites, then restores both', () => {
    ...
    expect(brandBrowserTitle('整理交接 — 小蛇')).toBe('整理交接 — 小蛇')
    ...
    let replayHostMutation: (() => void) | undefined
    let observerDisconnected = false
    const dispose = mountBrowserBrand(
      fakeDocument as unknown as Document,
      callback => {
        replayHostMutation = callback
    ...
    expect(links.some(link => link.id === 'xiaoshe-candidate-browser-icon' && !link.removed)).toBe(true)

    const lateHostIcon = makeLink({ rel: 'shortcut icon', href: '/late-host.svg', type: 'image/svg+xml' })
    links.push(lateHostIcon)
    fakeDocument.title = '整理交接 — DSH Local Build'
    replayHostMutation?.()
    expect(fakeDocument.title).toBe('整理交接 — 小蛇')
    expect(lateHostIcon.getAttribute('href')).toBe(BROWSER_BRAND_ICON_HREF)

    dispose()
    expect(observerDisconnected).toBe(true)
    expect(fakeDocument.title).toBe('DSH Local Build')
    expect(hostIcon.getAttribute('href')).toBe('/favicon.svg')
    expect(lateHostIcon.getAttribute('href')).toBe('/late-host.svg')
    expect(links.some(link => link.id === 'xiaoshe-candidate-browser-icon' && !link.removed)).toBe(false)
  })
})
