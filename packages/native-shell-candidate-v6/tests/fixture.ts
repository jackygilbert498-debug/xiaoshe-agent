import type { NativeShellV6ClientContext } from '../src/client/index.js'

export function contextFixture(
  slots: NativeShellV6ClientContext['slots'],
  runtime: {
    currentSessionId?: string
    sessions: Record<string, { state: string; completionReceipt?: { outcome?: string; sourceSeq?: number } }>
  } = { sessions: {} },
): NativeShellV6ClientContext {
  ...
  return {
    ...
    taskTimeline: {
      getSnapshot: () => ({
        items: [
          { key: 'u1', kind: 'user', text: '完成交接' },
          { key: 'v1', kind: 'status', text: '验证通过' },
        ],
      }),
      subscribe: () => () => {},
    },
    contextGovernance: {
      getSnapshot: () => ({ sessions: { s1: { pressure: 'normal', budget: { remaining: 42 } } } }),
      subscribe: () => () => {},
    },
    userApproval: {
    ...
    memoryLifecycle: {
      getSnapshot: () => ({
        ...
        memory: {
          ...
          entries: [{ scope: 'global', state: 'active' }, { scope: 'project', state: 'active' }],
        },
      }),
      subscribe: () => () => {},
