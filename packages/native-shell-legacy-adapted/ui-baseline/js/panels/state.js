    ...
    visible.map((j) => {
      ...
      return el("li", { class: `job${open ? " open" : ""}` },
        el("button.job-row", {
          ...
          el("i", { class: j.status === "running"
            ? "job-dot d-running task-state-indicator spin job-running-indicator"
            : `job-dot d-${j.status}`, "aria-hidden": "true" }),
    ...
    list.map((s) => {
      ...
      return el("li", { class: `sub st-${s.status}` },
        el("button.sub-row", {
          ...
          el("i", { class: s.status === "running"
            ? "sub-ic d-running task-state-indicator spin"
            : `sub-ic d-${s.status}`, text: s.status === "running" ? "" : meta.icon, "aria-hidden": "true" }),
