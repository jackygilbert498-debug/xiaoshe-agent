...
function buildMenu(s) {
  const items = [
    el("button.menu-item", { type: "button", role: "menuitem",
      dataset: { action: "rename-session", sid: s.id }, text: "重命名会话" }),
    el("button.menu-item.danger", { type: "button", role: "menuitem",
      dataset: { action: "delete-session", sid: s.id }, text: "删除会话" }),
  ];
...
const actions = {
  ...
  renameSession: async (sid, trigger) => {
    const s = sessions.find((item) => item.id === sid);
    const name = await promptModal({
      title: "重命名会话", label: "会话名称", initialValue: s?.title || s?.preview || "",
      confirmText: "保存", trigger,
    });
    if (!name?.trim()) return;
    return act(() => net.post("/api/sessions/rename", { sid, title: name.trim() }), "会话已重命名");
  },
  deleteSession: async (sid, trigger) => {
    const s = sessions.find((item) => item.id === sid);
    const ok = await confirmModal({
      title: "删除会话",
      body: `删除会话「${s?.title || s?.preview || sid}」？会移到本机回收区，可人工恢复。`,
      confirmText: "删除会话", cancelText: "取消", danger: true, trigger,
    });
    if (!ok) return;
    try {
      if (sid === store.get().sid) {
        const switched = await net.post("/api/sessions/new", {});
        if (!switched.switched) throw new Error("当前仍在运行，暂不能删除");
      }
      await net.post("/api/sessions/delete", { sid });
      notify("会话已移到本机回收区");
...
export function mount({ toast } = {}) {
  ...
  on(listEl, "click", ".menu-item", (_ev, btn) => {
    ...
    else if (action === "rename-session") actions.renameSession(sid, btn);
    else if (action === "delete-session") actions.deleteSession(sid, btn);
  });
