...
const MAX_INJECTION_CHARS = 16_000
...
export interface MemoryService {
  ...
  injection(project?: string, options?: MemoryInjectionOptions): MemoryInjection
...
export function createMemoryService(
  ...
  return {
    ...
    injection(projectKey, injectionOptions) {
      const observed = observeState(settings)
...
export interface MemoryInjection {
  ...
  /** Relevant active entries excluded by the item or rendered-character budget. */
  readonly omitted: number
  readonly text: string
}

export interface MemoryInjectionOptions {
  /** Optional current-turn query used only for deterministic relevance ordering. */
  readonly query?: string
  /** UTF-16 code-unit budget for the complete rendered context, including notice. */
  readonly maxChars?: number
  /** Hard-capped number of complete memory entries. */
 ...
 */
export function selectMemoryInjection(
  snapshot: MemorySnapshot,
  projectKey?: string,
  options?: MemoryInjectionOptions,
): MemoryInjection {
  return selectMemoryInjectionWith(snapshot, projectKey, canonicalProjectKey, options)
}
...
function selectMemoryInjectionWith(
  ...
  const limits = normalizeInjectionOptions(options)
  const eligible = snapshot.entries.flatMap((entry, index): Array<{
    ...
    return [{
      ...
      index,
      relevance: limits.query === undefined ? 0 : memoryRelevance(entry.text, limits.query),
    }]
  })

  eligible.sort((left, right) => {
    const scopeOrder = memoryScopeOrder(left.entry.scope) - memoryScopeOrder(right.entry.scope)
    if (scopeOrder !== 0) return scopeOrder
    if (limits.query !== undefined && left.relevance !== right.relevance) {
      return right.relevance - left.relevance
    }
    return left.index - right.index
  })

  const selected: Array<{ readonly item: MemoryInjectionItem; readonly frame: string }> = []
  for (const candidate of eligible) {
    if (selected.length >= limits.maxItems) break
    const frame = renderMemoryFrame(candidate.entry, candidate.item)
    const tentativeCount = selected.length + 1
    const omitted = eligible.length - tentativeCount
    const text = renderInjectionText([...selected.map(row => row.frame), frame], omitted)
    if (text.length <= limits.maxChars) selected.push({ item: candidate.item, frame })
  }

  const omitted = eligible.length - selected.length
  const text = renderInjectionText(selected.map(row => row.frame), omitted)
  // A tiny budget may be unable to carry the mandatory omission warning. In
  // that case suppress the context and audit set together; metadata remains
  // available to the caller without implying that omitted constraints vanish.
  const fits = text.length <= limits.maxChars

  return {
    ...(project === undefined ? {} : { project }),
    items: fits ? selected.map(({ item }) => item) : [],
    omitted: fits ? omitted : eligible.length,
    text: fits ? text : '',
  }
}

function normalizeInjectionOptions(options: MemoryInjectionOptions): {
  readonly query?: string
  readonly maxChars: number
  readonly maxItems: number
} {
  if (!isRecord(options)) throw new TypeError('memory injection options must be an object')
  if (options.query !== undefined && typeof options.query !== 'string') {
    throw new TypeError('query must be a string')
  }
  return {
    ...(normalizeRelevanceText(options.query ?? '') === ''
      ? {}
      : { query: normalizeRelevanceText(options.query ?? '').slice(0, MAX_TEXT_LENGTH) }),
    maxChars: normalizeBoundedOption(options.maxChars, MAX_INJECTION_CHARS, 'maxChars'),
    maxItems: normalizeBoundedOption(options.maxItems, MAX_INJECTION_ITEMS, 'maxItems'),
  }
}

function normalizeBoundedOption(value: unknown, maximum: number, name: string): number {
...
function memoryScopeOrder(scope: MemoryScope): number {
...
function memoryRelevance(text: string, query: string): number {
  ...
  let score = normalizedText.includes(query) ? 10_000 + query.length : 0
  const terms = query.match(/[\p{L}\p{N}]+/gu) ?? []
  for (const term of new Set(terms)) {
    if (normalizedText.includes(term)) score += 100 + term.length
    // CJK has no mandatory whitespace word boundary. Per-character matching
    // keeps compact Chinese queries useful without interpreting note content
    // as priority or instructions.
    if (/\p{Script=Han}/u.test(term)) {
      for (const character of new Set([...term])) {
        if (normalizedText.includes(character)) score += 1
      }
    }
  }
  return score
...
function renderMemoryFrame(entry: MemoryEntry, item: MemoryInjectionItem): string {
  return [
...
function renderInjectionText(frames: readonly string[], omitted: number): string {
  const parts = [...frames]
  if (omitted > 0) {
    parts.push(`<!-- xiaoshe-memory omitted="${omitted}"; relevant memory may be absent; consult xiaoshe_memory_list; omitted constraints may still apply -->`)
  }
  return parts.join('\n')
}
