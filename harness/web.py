...
def fetch(url, runner=None, max_bytes: int = 2_000_000, timeout: int = 25, post_fields=None, _hops: int = 4, resolver=None):
    ...
    **重定向**：不用 curl -L 自动跟，改从尾部标记拿目标、对每一跳重过 is_safe_url + 预检再手动跟（防重定向 SSRF），最多 _hops 跳。
    ...
    if redirect and _hops > 0:                # 有重定向目标：复校验后手动跟一跳（GET）
        ...
        ok2, out2 = fetch(redirect, runner=runner, max_bytes=max_bytes, timeout=timeout, _hops=_hops - 1, resolver=resolver)
...
def search(query, runner=None, fetcher=None, limit: int = 8):
    """搜索 query → [{title,url,snippet}]（抓 Mojeek 结果页解析）。可注入 fetcher(url)->(ok,html) 离线 TDD。"""
    if fetcher is None:
        def fetcher(url):
            return fetch(url, runner=runner)
