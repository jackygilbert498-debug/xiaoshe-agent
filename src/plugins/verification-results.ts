...
import { isPlainDocumentWrite, verificationResultSatisfied, standaloneArtifactKind } from '@xiaoshe/verification-policy'
import { artifactContext, artifactRead, prepareArtifact, type ArtifactMutation, type ArtifactRead } from './verification-applicability.js'
...
import {
  observeOutputFile, observeDataFile, outputTargetKey, sameOutputIdentity, VerificationFileProofStore,
...
type VerificationStatus = 'passed' | 'failed' | 'not-applicable'
...
interface VerificationPolicy {
  ...
  planTool(input: { readonly toolName: string; readonly arguments?: unknown; readonly kind: ChangeKind; readonly risk?: Risk }): {
    readonly gates: readonly VerificationGate[]
  }
...
interface CapturedResult {
  readonly artifactContext?: ArtifactMutation
  readonly artifactRead?: ArtifactRead
  readonly callId: string
  readonly name: string
  readonly arguments: unknown
  readonly isError: boolean
...
interface VerificationProgress {
  ...
  readonly notApplicableGates?: readonly VerificationGate[]
...
export function apply(ctx: Host): void {
  ...
  const pendingArtifacts = new WeakMap<SessionLike, Map<string, ArtifactMutation>>()
    ...
    reconcile(agent) {
      try {
        ...
        for (const call of calls) {
          ...
          const gates = ctx.xiaosheVerificationPolicy.planTool({ toolName: call.name, arguments: call.arguments, ...classification.change }).gates
        ...
        const actuallyPassed = new Set<VerificationGate>()
        for (const call of calls.filter(belongs)) {
          for (const [gate, result] of trustedProducedResults(readSessionEvents(agent.session), call, calls, agent.session, ctx.xiaosheVerificationPolicy)) {
            if (result.status === 'passed') actuallyPassed.add(gate)
          }
        }
        return {
          ...
          passedGates: [...required].filter(gate => !missing.has(gate) && actuallyPassed.has(gate)).sort(),
          notApplicableGates: [...required].filter(gate => !missing.has(gate) && !actuallyPassed.has(gate)).sort(),
  ...
  ctx.on('tools/pre-execute', async (execution, next) => {
    ...
    if (decision.kind === 'allow' && execution.agent && !execution.signal?.aborted) {
      ...
      if (['write', 'edit'].includes(execution.name)) {
        const context = prepareArtifact(execution.name, execution.arguments, session.header.cwd)
        if (context) {
          let entries = pendingArtifacts.get(session)
          if (!entries) { entries = new Map(); pendingArtifacts.set(session, entries) }
          entries.set(execution.callId, context)
          while (entries.size > 128) entries.delete(entries.keys().next().value as string)
        }
      }
  ...
  ctx.on('tools/result', (execution, result) => {
    ...
    const preparedArtifact = pendingArtifacts.get(session)?.get(execution.callId)
    pendingArtifacts.get(session)?.delete(execution.callId)
    const currentArtifact = !result.isError && preparedArtifact
      ? artifactContext(execution.name, execution.arguments, session.header.cwd) : undefined
    const readArtifact = !result.isError && execution.name === 'read'
      ? artifactRead(execution.arguments, result.value, session.header.cwd) : undefined
    ...
    const record: CapturedResult = {
      ...
      ...(currentArtifact?.context === preparedArtifact?.context && currentArtifact && preparedArtifact ? { artifactContext: preparedArtifact } : {}),
      ...(readArtifact ? { artifactRead: readArtifact } : {}),
  ...
  ctx.on('session/event', (session, event) => {
    ...
    pendingArtifacts.delete(session)
...
function produceTurnResults(
  ...
  const mutations = calls.flatMap<PendingMutation>(call => {
    ...
    if (change === undefined) return SHELL_TOOLS.has(call.name)
      ...
      : []
    const gates = new Set(policy.planTool({ toolName: call.name, arguments: call.arguments, ...change }).gates)
  ...
  if (mutations.some(mutation => mutation.unknownEffect)) {
    for (const call of calls) {
      ...
      const gates = new Set(policy.planTool({ toolName: call.name, arguments: call.arguments, ...classification.change }).gates)
  ...
  return mutations.flatMap(mutation => {
    ...
    const missing = new Set([...mutation.gates].filter(gate => !verificationResultSatisfied(gate, results.get(gate))))
...
async function redirectUnverifiedCompletion(
  ...
  const instruction = browserUnavailable
    ...
    ? [
        ...
        '门禁应按项目实际情况适用：独立 Bash/plist/Markdown 可先用 read 完整回读修改文件，宿主检查项目入口后记录编译门禁不适用。不要伪造 build/typecheck，也不要把 bash -n 等语法检查称为类型检查；实际测试、加载或触发的要求仍保留。已核实不适用的项目不是缺失，不必反复向用户解释。',
...
function pruneCapturedResults(
  ...
  for (const call of calls) {
    ...
    if ([...gates].every(gate => verificationResultSatisfied(gate, results.get(gate)))) continue
    ...
    for (const result of results.values()) {
      if (result.status === 'passed' || result.status === 'not-applicable') retain.add(result.verifierCallId)
    }
  }
...
): Map<VerificationGate, { readonly gate: VerificationGate; readonly evidence: string; readonly status: VerificationStatus; readonly verifierCallId: string }> {
  const byId = new Map(calls.map(call => [call.callId, call]))
  const latest = new Map<VerificationGate, { readonly gate: VerificationGate; readonly evidence: string; readonly status: VerificationStatus; readonly verifierCallId: string }>()
  for (const event of events) {
    ...
    if (verifier === undefined || verifier.callId === mutation.callId
      ...
      || (status !== 'passed' && status !== 'failed' && status !== 'not-applicable') || evidence === undefined) continue
    ...
    latest.set(candidate.gate, { gate: candidate.gate, evidence: candidate.evidence, status: candidate.status, verifierCallId: verifier.callId })
  }
  return latest
}
...
function verificationCandidates(
  ...
  if (FILE_READ_TOOLS.has(verifier.name)) return [...fileReadbackCandidates(verifier, session, calls), ...artifactCandidates(verifier, session)]
  return []
}

function artifactCandidates(verifier: DurableCall, session: SessionLike): Candidate[] {
  const proof = verifier.capture.artifactRead
  if (!proof || verifier.failed || verifier.capture.isError) return []
  const current = artifactRead(verifier.arguments, verifier.capture.value, session.header.cwd)
  if (!current || current.context !== proof.context
    || !sameOutputIdentity(current.observation.identity, proof.observation.identity)) return []
  const matches = (mutation: DurableCall): boolean => {
    const prepared = mutation.capture.artifactContext
    return prepared !== undefined && prepared.context === proof.context && prepared.path === proof.path
      && prepared.expected === current.observation.content
  }
  const applicabilityMatches = (mutation: DurableCall): boolean => ['write', 'edit'].includes(mutation.name)
    && standaloneArtifactKind(mutation.name, mutation.arguments) === proof.kind
    && resolvePath(nonEmptyString(record(mutation.arguments)?.file_path), session.header.cwd) === proof.path
  const reason = evidence(['applicability/v1', `target=${proof.path}`, `kind=${proof.kind}`,
    `workspace_without_build_entry=${proof.context}`, `read=${verifier.callId}`, `content_sha256=${proof.observation.identity.contentSha256}`])
  const excluded: VerificationGate[] = proof.kind === 'document' ? ['typecheck', 'build', 'test'] : ['typecheck', 'build']
  return [
    ...excluded.map(gate => ({ gate, status: 'not-applicable' as const, evidence: reason, matches: applicabilityMatches })),
    { gate: 'functional-probe', status: 'passed', evidence: evidence([
      `verifier=${verifier.callId}`, `target=${proof.path}`, 'scope=exact-file-readback-only',
      `content_sha256=${proof.observation.identity.contentSha256}`,
    ]), matches },
  ]
}
...
function classifyCommandSegment(
  ...
  if (isShellTestInvocation(tokens)) {
    if (!workdir) return undefined
    const path = resolve(workdir, tokens[1] as string)
    const script = observeDataFile(path)
    if (!inside(workdir, path) || !script || script.identity.size > 1_000_000) return undefined
    return { gates: ['test'], proofs: [], packageGraphs: [{ path, sha256: script.identity.contentSha256 }], needsPositiveTestSummary: true }
  }
...
function commandInvokesPackageScript(command: string): boolean {
  ...
  return trimmed.split(/\s*&&\s*/u).some(segment => {
    ...
    return packageScriptInvocation(executable, tokens) !== undefined || isShellTestInvocation(tokens)
  })
}

function isShellTestInvocation(tokens: readonly string[]): boolean {
  return tokens.length === 2 && ['bash', 'sh', 'zsh'].includes(basename(tokens[0] ?? ''))
    && /(?:^|[/\\])[^/\\]+\.test\.(?:sh|bash|zsh)$/iu.test(tokens[1] ?? '')
}
...
function hasPositiveTestSummary(output: string | undefined): boolean {
  ...
  // Existing standalone Bash suites use this explicit non-empty final tally.
  // A zero exit, help text, or a bare "19/19" is not a test result contract.
  if (/(?:^|\n)结果[:：]\s*通过\s+[1-9]\d*[，,]\s*失败\s+0\s*$/u.test(summary.trim())) return true
