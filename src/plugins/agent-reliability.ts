...
import { shellOutputWrites } from './shell-output-effects.js'
...
import { isPlainDocumentWrite } from '@xiaoshe/verification-policy'
import { applyGraphEvent, canonicalGraphPlan, initialGraphState, replayGraphPlan, type GraphEvent } from './task-graph-domain.js'
...
失败按错误处理：修正输入、临错退避、缺信息询问，用授权独立路线；不按次数换族或无证据热循环，不因普通工具故障擅自安装、改账号、密钥或全局配置。shell 的 sandbox_permissions 与 justification 只能在工具明确返回“escalation available”后，用严格更宽的权限对原命令重试一次；任务路径或能力策略拒绝不能靠升权绕过，也不要为只读验证预先添加这两个参数。写入、发送、部署、安装和界面动作成功后，以回读、状态、测试或可观察结果闭环；最终只报告实际结果、验证证据和仍未完成的边界。`

interface Agent {
  readonly id: string
  readonly session?: object & {
    readonly header?: { readonly agentPreset?: string; readonly id?: string }
...
interface UncertainExternalEffect {
  /** Payload-free retry match; the map key identifies one invocation, not this fingerprint. */
  ...
  readonly at: number
  readonly generation: number
  /** Observation admission must be strictly later than this unknown result. */
  readonly resultCausalOrder: number
  /** Digests of caller-supplied idempotency/correlation identifiers, never payload text. */
...
interface State {
  ...
  /** Causal order captured at admission; removed when the call settles. */
  pendingCallAdmissionOrders: Map<string, number>
  ...
  /** Unresolved invocation facts are not evictable history; release only by safe reconciliation/authority. */
  uncertainEffects: Map<string, UncertainExternalEffect>
...
function canonicalVerificationContext(value: unknown, generation: number): string | undefined {
  ...
  const notApplicableGates = progress.notApplicableGates ?? []
  if (!gateList(notApplicableGates)) return undefined
  if (passedGates.some(gate => !requiredGates.includes(gate) || missingGates.includes(gate))
    ...
    || notApplicableGates.some(gate => !requiredGates.includes(gate) || passedGates.includes(gate) || missingGates.includes(gate))
    || passedGates.length + missingGates.length + notApplicableGates.length !== requiredGates.length) return undefined
  ...
  if (notApplicableGates.length > 0) lines.push(`已核实不适用：${[...notApplicableGates].sort().join('、')}。不记为通过，也不记为缺失；不得因此输出部分完成或要求用户确认。`)
...
export function toolFamily(name: string): CapabilityFamily {
  // First-party Session bookkeeping is planning, not a business side effect.
  if (name === 'xiaoshe_task_graph') return 'todo'
  ...
  // These first-party tools only read this worktree or update the optional
  // private knowledge projection. They are not unknown remote integrations;
  // explicit offline mode must not disable a local memory lookup.
  if (name === 'xiaoshe_knowledge_inspect') return 'filesystem_read'
  if (/^xiaoshe_knowledge_(?:query|save|forget)$/u.test(name)) return 'memory'
...
function pathCandidates(value: string): string[] {
  ...
  for (const part of withoutQuoted.split(/\s*(?:、|和|或|,|\band\b|\bor\b)\s*/iu)) {
    const cleaned = part
      .replace(/^(?:the\s+)?(?:file|path|directory|(?:任何)?源文件\s+|文件|路径|目录)\s*/iu, '')
      .replace(/\s*(?:文件|路径)$/u, '')
      .trim()
...
function constrainedPaths(goal: string): PathConstraints {
  ...
  for (const clause of goal.split(/[\n，。；;]/u).map(value => value.trim()).filter(Boolean)) {
    ...
    if (deny?.[1]) {
      // Delegation often lists protected sources in parentheses. Treat the
      // surrounding instruction as syntax, not as part of one fictitious path.
      const sourceList = /^(?:任何)?源文件[（(]([^()（）]+)[）)]$/u.exec(deny[1].trim())
      const listed = sourceList ? sourceList[1]!.replace(/\s*都不要改动\s*$/u, '').split(/\s+\/\s+/u).join('、') : deny[1]
      for (const candidate of pathCandidates(listed)) forbidden.add(candidate)
    }
...
function readTargetPaths(args: unknown, toolName?: string): string[] {
  if (typeof args !== 'object' || args === null || Array.isArray(args)) return []
  // Only this first-party tool certifies a complete bounded batch read. Do not
  // interpret arbitrary connector `paths` or partial malformed batches as evidence.
  if (toolName === 'xiaoshe_knowledge_inspect') {
    const paths = (args as Record<string, unknown>).paths
    if (!Array.isArray(paths) || paths.length === 0 || paths.length > 8
      || paths.some(value => typeof value !== 'string')) return []
    const normalized = paths.map(value => normalizeConstraintPath(value as string))
    return normalized.every((value): value is string => Boolean(value)) ? [...new Set(normalized)] : []
  }
  const output: string[] = []
...
function recordInputStopResult(state: State, execution: Execution, result: Result): void {
  ...
  if (!kind) return
  const targets = readTargetPaths(execution.arguments, execution.name), workspace = executionWorkspaceRoot(execution)
...
function recordOrderedReadResult(state: State, execution: Execution, succeeded: boolean): void {
  ...
  const targets = new Set(readTargetPaths(execution.arguments, execution.name))
...
const POWERSHELL_WRITE_COMMAND = /(?:^|[\s;&|])(?:Microsoft\.PowerShell\.(?:Management|Utility)\\)?(?:Set-Content|Add-Content|Clear-Content|Out-File|New-Item|Remove-Item|Move-Item|Rename-Item|Copy-Item)\b|\[System\.IO\.File\]::(?:WriteAllText|WriteAllBytes|AppendAllText|Create)\s*\(/iu
...
function demonstrablyLocalReadOnlyShell(command: string): boolean {
  ...
  if (/^(?:python3?|node|pytest)(?:\.exe)?\s+--version$/iu.test(value)) return true
...
function explicitlyRequestedVerifier(execution: Execution, goal: string): boolean {
  let command = commandText(execution).trim().replace(/\s+/gu, ' ')
  // One literal directory change is an execution location, not an arbitrary
  // command chain. Every other separator remains disallowed for this route.
  const changeDirectory = /^cd\s+(?:"([^"$`]+)"|'([^']+)'|([^\s;&|$`]+))\s*&&\s*(.+)$/u.exec(command)
  if (changeDirectory) command = changeDirectory[4]!
  const shellTest = /^(?:bash|sh|zsh)\s+(?:"[^"\r\n]+\.test\.(?:sh|bash|zsh)"|'[^'\r\n]+\.test\.(?:sh|bash|zsh)'|[^\s;&|<>]+\.test\.(?:sh|bash|zsh))$/iu.test(command)
  if (!command || command.length > 512 || SHELL_CONTROL_OR_REDIRECTION.test(command)
    || MUTATION_COMMAND.test(command) || SHELL_NETWORK_COMMAND.test(command) || !VERIFY_COMMAND.test(command) && !shellTest) return false
  if (!shellTest && !/^(?:python(?:3(?:\.\d+)?)?(?:\.exe)?\s+-m\s+pytest|pytest|(?:npm|pnpm|yarn|bun)(?:\.cmd)?\s+(?:run\s+)?(?:test|lint|typecheck|check|verify|build)(?::[\w:-]+)?|node(?:\.exe)?\s+--test|cargo\s+test|go\s+test|dotnet\s+test)(?:\s|$)/iu.test(command)) return false
  const quoted = [...goal.matchAll(/`([^`\r\n]{1,512})`/gu)]
    .map(match => (match[1] ?? '').trim().replace(/\s+/gu, ' '))
  // Natural-language authorization to run project checks is sufficient. This
  // authorizes execution, not a proof that tests are pure; the real host policy
  // and explicit no-write/no-network restrictions remain in force below.
  const naturalVerification = /(?:运行|执行|完成后|修改后|修复后).{0,24}(?:测试|验证|验收|检查)|\b(?:run|execute)\s+(?:the\s+)?(?:project\s+)?(?:tests?|checks?|verification)\b/iu.test(goal)
  if (!naturalVerification && !quoted.some(value => value.toLocaleLowerCase('en-US') === command.toLocaleLowerCase('en-US'))) return false
  if (/[$`<>]|(?:^|\s)(?:--(?:rootdir|confcutdir|override-ini|basetemp|import-mode|prefix|directory|cwd|output|junitxml|import|require|loader|eval)|-c|-o|-p|-C|-e)(?:=|\s)|(?:^|[\s="'])(?:\/|[a-z]:[\\/])|(?:^|[\s=\\/])\.\.(?:[\\/]|$)/iu.test(command)) return false
  ...
  const resolveWorkdir = (value: string, base: string): string | undefined => normalizeConstraintPath(/^(?:[a-z]:[\\/]|\/)/iu.test(value) ? value : `${base}/${value}`)
  let workdir = typeof rawWorkdir === 'string' ? resolveWorkdir(rawWorkdir, workspaceRoot) : workspaceRoot
  if (changeDirectory && workdir) workdir = resolveWorkdir(changeDirectory[1] ?? changeDirectory[2] ?? changeDirectory[3]!, workdir)
  if (workdir) {
    try {
      const actualRoot = normalizeConstraintPath(realpathSync(workspaceRoot))!
      const actualCwd = normalizeConstraintPath(realpathSync(workdir))!
      if (actualCwd !== actualRoot && !actualCwd.startsWith(`${actualRoot}/`)) return false
    } catch { /* A nonexistent location cannot execute; the host reports that error. */ }
  }
...
function shellConstraintDenial(
  ...
  return '当前任务路径约束下无法证明该终端命令的副作用边界；已保守拒绝执行。这不是 shell 沙箱拒绝，提升或添加 sandbox_permissions/justification 无效。已授权的项目测试可使用工具工作目录，或单个 cd 到项目内目录后运行；不要混入其他操作。其他命令请拆分为目标明确的类型化操作；确需超出约束时说明具体冲突，请用户确认范围。'
}
...
function shellWriteEffect(command: string): WriteEffect {
  const output = shellOutputWrites(command)
  // A redirection to /dev/null is not a source edit. Conversely, never inspect
  // quoted Python/CSS text as if its comparison operators were shell redirects.
  const namedWrite = POWERSHELL_WRITE_COMMAND.test(command)
  if (!namedWrite && output.targets.length === 0 && !output.unresolved) return { detected: false, targets: [], unresolved: false }
  const targets: string[] = [...output.targets]
  ...
  return { detected: true, targets: normalized, unresolved: output.unresolved || (namedWrite && normalized.length === 0) }
}
...
interface FailureEvidence {
  readonly structuredCodes: readonly string[]
  readonly detail: string
}

/** Normalize the complete bounded failure envelope once so classification and
 * unknown-effect protection cannot disagree about content-only evidence. */
function failureEvidence(result: Result): FailureEvidence {
  const structuredCodes = [result.error?.info?.code, result.error?.code]
    .filter((code): code is string => typeof code === 'string' && code.trim() !== '')
    .map(code => code.trim().slice(0, 128))
  const content = result.content
    .map(block => typeof block.text === 'string' ? block.text.slice(0, 2_000) : '')
    .join(' ')
  return {
    structuredCodes,
    detail: `${structuredCodes.join(' ')} ${result.error?.message ?? ''} ${content}`.slice(0, 8_000),
  }
}

/** Authoritative typed causes outrank incidental wording such as an approval
 * message that says it "timed out" after the policy already rejected it. */
function structuredFailureCategory(codes: readonly string[]): string | undefined {
  if (codes.some(code => /^VISION_TIMEOUT_EXHAUSTED$/iu.test(code))) return 'vision_timeout_exhausted'
  if (codes.some(code => /^(?:IMAGE_NOT_SUPPORTED|UNSUPPORTED_(?:IMAGE|MODALITY))$/iu.test(code))) return 'image_not_supported'
  if (codes.some(code => /^(?:PERMISSION_DENIED|UNAUTHORIZED|FORBIDDEN|EACCES|EPERM|TOOL_NOT_ALLOWED|POLICY_DENIED)$/iu.test(code))) return 'permission_denied'
  if (codes.some(code => /^(?:INVALID_(?:ARGUMENT|INPUT)|VALIDATION_(?:ERROR|FAILED)|SCHEMA_(?:ERROR|INVALID)|BAD_REQUEST|HTTPS_REQUIRED)$/iu.test(code))) return 'invalid_input'
  if (codes.some(code => /^(?:ENOENT|NOT_FOUND|FS_NOT_FOUND|RESOURCE_NOT_FOUND|MESSAGE_NOT_FOUND)$/iu.test(code))) return 'not_found'
  if (codes.some(code => /^(?:UNKNOWN_TOOL|SERVICE_UNAVAILABLE|NOT_CONFIGURED|NO_ADAPTER|ECONNREFUSED|CAPABILITY_UNAVAILABLE)$/iu.test(code))) return 'capability_unavailable'
  if (codes.some(code => /^(?:ABORTED|ABORTED_BEFORE_DISPATCH|BROWSER_CANCELLED)$/iu.test(code))) return 'aborted'
  if (codes.some(code => /^(?:TIMEOUT|TIMED_OUT|VISION_TIMEOUT|ETIMEDOUT)$/iu.test(code))) return 'timeout'
  return undefined
}

function category(result: Result): string {
  const evidence = failureEvidence(result)
  const structured = structuredFailureCategory(evidence.structuredCodes)
  if (structured !== undefined) return structured
  const text = evidence.detail
  if (/VISION_TIMEOUT_EXHAUSTED/u.test(text)) return 'vision_timeout_exhausted'
  ...
  if (/timeout|timed out|超时|视觉读取在\s*\d+\s*秒内未完成|VISION_TIMEOUT/iu.test(text)) return 'timeout'
...
function toolWasCancelled(execution: Execution, result: Result): boolean {
  const codes = failureEvidence(result).structuredCodes
  const externalMutation = executionToolFamily(execution).startsWith('integration:')
    && executionActionFamily(execution) !== undefined
  // Before-dispatch is a definitive no-effect cancellation. Any other abort of
  // an external mutation may have raced dispatch, so it must reach the
  // unknown-effect guard even when the caller's signal is now aborted.
  if (externalMutation) return codes.some(code => /^ABORTED_BEFORE_DISPATCH$/iu.test(code))
...
function resultOutcome(execution: Execution, result: Result): ResultOutcome {
  ...
  // The optional knowledge plugin reports recoverable failures as structured
  // data so a transient indexing problem cannot abort the whole user turn.
  if (/^xiaoshe_knowledge_(?:inspect|query|save|forget)$/u.test(execution.name)
    && argumentRecord(result.value)?.ok === false) return { succeeded: false, category: 'invalid_input' }
...
/** A transport failure can leave an external mutation's outcome unknown even
 * on its first attempt. Definite input, path, permission and availability
 * failures prove that the effect did not happen and remain correctable. */
function leavesExternalEffectUnknown(execution: Execution, result: Result, kind: string): boolean {
  const family = executionToolFamily(execution)
  if (!family.startsWith('integration:') || executionActionFamily(execution) === undefined) return false
  const evidence = failureEvidence(result)
  if (evidence.structuredCodes.some(code => /^ABORTED_BEFORE_DISPATCH$/iu.test(code))) return false
  if (['invalid_input', 'not_found', 'permission_denied', 'capability_unavailable', 'image_not_supported'].includes(kind)) return false
  if (execution.signal.aborted) return true
  if (kind === 'timeout' || kind === 'aborted') return true
  return /unknown|uncertain|outcome unavailable|result unavailable|结果未知|状态未知/iu.test(evidence.detail)
}

const EFFECT_CORRELATION_KEY = /^(?:client_?request_?id|request_?id|idempotency_?key|operation_?id|submission_?id|transaction_?id|message_?id|event_?id|job_?id|task_?id|external_?id)$/iu

/** Keep only strong, caller-controlled correlation identifiers. Recipient or
 * channel alone is not unique enough to prove that a particular send did not happen. */
function uncertainEffectCorrelation(execution: Execution): Readonly<Record<string, string>> {
  const args = argumentRecord(execution.arguments)
  if (!args) return {}
  const entries = Object.entries(args).flatMap(([key, value]) => {
    if (!EFFECT_CORRELATION_KEY.test(key) || !['string', 'number'].includes(typeof value)) return []
    const normalized = key.toLocaleLowerCase('en-US').replace(/_/gu, '')
    const digest = createHash('sha256').update(stable(value)).digest('hex')
    return [[normalized, digest] as const]
  }).slice(0, 8)
  return Object.fromEntries(entries)
}

function definitiveNegativeReconciliation(result: Result): boolean {
  const evidence = failureEvidence(result)
  if (result.isError) return structuredFailureCategory(evidence.structuredCodes) === 'not_found'
  const value = argumentRecord(result.value)
  if (!value) return false
  const status = typeof value.status === 'string' ? value.status.trim().toLocaleLowerCase('en-US').replace(/[\s-]+/gu, '_') : ''
  if (['not_sent', 'not_submitted', 'not_found', 'absent', 'aborted_before_dispatch', 'cancelled_before_dispatch', 'rejected_before_dispatch'].includes(status)) return true
  return value.found === false || value.exists === false || value.sent === false || value.submitted === false
}

function explicitUnknownEffectRetryAuthorization(text: string): boolean {
  const direct = boundedGoalText(text).trim()
  const authorization = /^(?:我\s*)?(?:明确\s*)?(?:授权|允许|同意|确认)(?:你)?|^(?:i\s+)?(?:explicitly\s+)?(?:authorize|approve|confirm)\b/iu.test(direct)
  const retry = /重试|再次发送|重新发送|重复发送|再次提交|重新提交|retry|resend|resubmit|repeat/iu.test(direct)
  const risk = /结果未知|状态未知|可能重复|重复(?:发送|提交)?.{0,12}风险|unknown\s+(?:result|outcome|status)|duplicate\s+risk/iu.test(direct)
  return authorization && retry && risk
}
...
function recoveryAdvice(kind: string): string {
  switch (kind) {
    ...
    case 'timeout': return '只读操作可退避后重试；外部副作用若结果未知，先用已授权的只读状态路线核对，不能盲目重复。'
    // Tool-owned recovery reports evidence, not a failure-count permission ban.
    case 'vision_timeout_exhausted': return '视觉工具本次内部有界恢复预算已用尽，未取得图片内容。临时故障可退避后只读重测，或使用已验证且获授权的独立视觉路线；浏览器截图和重新上传不等于独立引擎，不要求用户补交已有图片。'
    case 'permission_denied': return '不要改参数碰运气；改用已授权路径，或准确报告缺失权限。'
    case 'capability_unavailable': return '核对当前注册与配置；可修正输入、询问必要信息，或选择已授权的独立路线。'
    case 'image_not_supported': return '修正图片输入，或使用已安装且获授权的独立 OCR/视觉路线；若无则明确阻塞。'
    case 'aborted': return '确认是否为用户取消；否则可退避后有界重试、询问必要信息或使用已授权的独立路线。'
    default: return '依据错误事实修正输入；临时故障可退避重试，缺少信息时询问，必要时使用已授权的独立路线。'
  }
}
...
function exactReadbackMatches(execution: Execution, pending: PendingVerification): boolean {
  ...
  const observed = readTargetPaths(execution.arguments, execution.name)
...
function localTargetEvidence(state: State, execution: Execution): boolean {
  ...
  // A new preview/fixture has no previous implementation to read. Successful
  // task input reads provide preparation, but never bypass the earlier path
  // authorization guard or the existing-target read-before-write check.
  if (!effect.unresolved && executionWorkspaceRoot(execution) !== undefined
    && state.taskAssessment?.research_required !== true && state.readEvidencePaths.size > 0
    && !state.taskAssessment?.signals.some(signal => signal.startsWith('local_data_'))
    && [...state.readEvidencePaths].some(observed => observed.startsWith(`${executionWorkspaceRoot(execution)}/`)
      && /\.(?:css|html?|svg|[cm]?[jt]sx?|py|rs|go|c|h|cpp|java|swift)$/iu.test(observed))
    && targets.every(target => !existingMutationTarget(execution, target))) return true
...
export class RecoveryController {
  ...
  state(agent: Agent): State {
    ...
    if (!state) {
      state = {
        ...
        pendingCallAdmissionOrders: new Map(),
        causalSequence: 0,
  ...
  recordAdmission(execution: Execution): void {
    ...
    if (!state.pendingCallGenerations.has(execution.callId) && !state.callGenerations.has(execution.callId)) {
      ...
      state.pendingCallAdmissionOrders.set(execution.callId, ++state.causalSequence)
    }
  }
  ...
  goalChanged(
    ...
    // A later accepted direct user message may consciously authorize one exact
    // duplicate-risk retry. Ambiguous bulk release and quoted/tool text do not.
    if (options.directGoal && options.triggerMessageId && state.uncertainEffects.size === 1
      && explicitUnknownEffectRetryAuthorization(options.directGoal)) {
      state.uncertainEffects.clear()
    }
  ...
  denial(execution: Execution, schemas: readonly ToolSchema[] = []): string | undefined {
    ...
    let uncertainEffect: UncertainExternalEffect | undefined
    for (const effect of state.uncertainEffects.values()) {
      if (effect.fingerprint === callFingerprint) { uncertainEffect = effect; break }
    }
    if (uncertainEffect) {
      state.blockedRepeats += 1
      addBounded(state.policyRedirects, redirectKey, 8)
      return `外部副作用调用 ${uncertainEffect.tool} 的结果未知（${uncertainEffect.category}），为避免重复发送或重复提交，本次相同调用未执行。可使用现有且已授权的只读状态/查询路线确认结果；不能据此自动获得新的查询权限。`
    }
  ...
  result(execution: Execution, result: Result, options: { readonly durableReplay?: boolean } = {}): void {
    ...
    const callAdmissionOrder = callId === undefined ? undefined : state.pendingCallAdmissionOrders.get(callId)
    ...
    // A stale result cannot reconcile a newer task or an effect that became
    // unknown only after this observation was already admitted.
    const belongsToCurrentTask = callGeneration === state.taskGeneration
    if (belongsToCurrentTask && callAdmissionOrder !== undefined
      && observationTool(execution.name, execution.arguments) && definitiveNegativeReconciliation(result)) {
      const observed = uncertainEffectCorrelation(execution)
      for (const [effectKey, effect] of state.uncertainEffects) {
        const expected = Object.entries(effect.correlation)
        if (effect.family === family && effect.generation <= callGeneration
          && callAdmissionOrder > effect.resultCausalOrder && expected.length > 0
          && expected.every(([correlationKey, digest]) => observed[correlationKey] === digest)) {
          state.uncertainEffects.delete(effectKey)
        }
      }
    }
    ...
    if (succeeded && family === 'todo') {
      const args = typeof execution.arguments === 'object' && execution.arguments !== null && !Array.isArray(execution.arguments)
        ...
        : undefined
      if (execution.name === 'xiaoshe_task_graph') {
        // Credit only the canonical durable Session snapshot. A tool value
        // claiming to contain a graph cannot manufacture preparation evidence.
        if (args?.action === 'create' || args?.action === 'replan') {
          this.recordPlan(execution.agent, replayGraphPlan(readSessionEvents(execution.agent.session), execution.agent.session?.header?.id, state.taskGeneration))
        }
      } else this.recordPlan(execution.agent, args?.todos)
    }
    if (succeeded && observationTool(execution.name, execution.arguments)
    ...
    if (belongsToCurrentTask && succeeded && ['filesystem_read', 'filesystem_search'].includes(family)) {
      ...
      for (const target of readTargetPaths(execution.arguments, execution.name)) {
    ...
    if (succeeded) {
      // This invocation's success cannot reconcile an identical concurrent
      // invocation. Terminal deliveries are idempotent; only the correlated
      // observation or explicit single-effect authority paths release unknowns.
      if (recoveryAttempt && typeof execution.callId === 'string' && execution.callId.trim() !== '') {
        state.routeRecovery!.proofCallId = execution.callId
      }
      // Legacy recovery events remain historical telemetry. A real success in
      // the originally failed family proves there is no present route pressure.
      if (state.routeRecovery?.failedFamily === family && state.routeRecovery.status !== 'satisfied') {
        state.routeRecovery = undefined
      }
    ...
    if (leavesExternalEffectUnknown(execution, result, kind)) {
      const resultCausalOrder = ++state.causalSequence
      // Keep each actual unresolved invocation, including parallel equal-input
      // calls. Hash identity and retain compact digests, never argument payloads.
      const invocationKey = createHash('sha256').update(stable({ callId,
        generation: callGeneration, admissionOrder: callAdmissionOrder ?? resultCausalOrder })).digest('hex')
      state.uncertainEffects.set(invocationKey, {
        fingerprint: key,
        ...(callAdmissionOrder === undefined ? {} : { admissionOrder: callAdmissionOrder }),
        tool: execution.name,
        family,
        category: kind,
        at: this.now(),
        generation: callGeneration,
        resultCausalOrder,
        correlation: uncertainEffectCorrelation(execution),
      })
      // Unlike bounded diagnostic history, dropping an unresolved safety fact
      // would silently authorize duplicate effects. Resolved facts are removed
      // above; remaining compact records live only for this agent/session.
    }
    ...
    // Failure counters and historical route events are telemetry/advice only.
    // They must not create a fresh planning obligation or exclude this family.
    if (recoveryAttempt) state.routeRecovery = undefined
  ...
  private settleCallGeneration(state: State, callId: string): number | undefined {
    ...
    if (pendingGeneration !== undefined) {
      ...
      state.pendingCallAdmissionOrders.delete(callId)
  ...
  restoreRouteRecovery(agent: Agent, value: unknown, sourceSeq = Number.POSITIVE_INFINITY): void {
    ...
    if (status === 'needs-alternative') {
      ...
      // Retain compatibility with the V1 event shape without reactivating a
      // counter-derived planning restriction during cold replay.
      return
    }
  ...
  deliberationContext(agent: Agent, assessment: TaskAssessment, schemas: readonly ToolSchema[]): string {
    ...
    const replan = this.inputStopContext(agent) || (state.lastFailure
      ? '最近路线失败；请依据具体错误修正输入，临时故障可退避后有界重试；缺少必要信息时询问，已有独立且获授权路线时也可选用。不要仅因历史失败次数排除当前工具。'
...
function renderTaskGuidance(
  ...
  const pathRestrictions = hasPathConstraints(paths)
    ...
    : ''
  const recovery = failed ? '最近路线已失败；依据具体错误修正输入，临时故障可退避后有界重试；缺少必要信息时询问，已有独立且获授权路线时也可选用。' : ''
...
export function apply(ctx: Host): void {
  ...
  const restoreAgent = (agent: Agent, force = false): void => {
    ...
    let graphPlanState = initialGraphState(agent.session?.header?.id ?? null)
    for (const event of events) {
      if (!event || typeof event.type !== 'string') continue
      if (Number.isSafeInteger(event.seq)) graphPlanState = applyGraphEvent(graphPlanState, event as GraphEvent)
      const data = replayRecord(event.data)
      ...
      if (event.type === 'xiaoshe/task-graph') {
        // Restore the historical planning fact at its original generation,
        // including when later nodes were completed before this cold resume.
        if (data?.version === 1 && data.action === 'durable' && taskGenerations.protocol !== 'invalid') {
          recovery.recordPlan(agent, canonicalGraphPlan(graphPlanState, agent.session?.header?.id, recovery.state(agent).taskGeneration))
        }
        continue
      }
      if (event.type === 'tool/call') {
  ...
  ctx.effect(() => ctx.systemPrompt.context({
    ...
    text: ({ agent }) => {
      ...
      if (VISUAL_CONTEXT_INTENT.test(goal) || imageInput.recorded_image_count > 0) {
        ...
        if (state.visionStatus === 'failed_this_turn') lines.push('视觉失败不是用户缺图：已存在的本地图片不要求重新上传。浏览器截图仍需视觉引擎，不能承诺换浏览器就能读取；本地路径不是 HTTPS 地址。图片是任务必需证据时保留任务、明确故障，不以通用文字冒充逐图验收。')
