...
export class VerificationFileProofStore {
  ...
  static open(logPath: string, cwd: string, create: boolean): VerificationFileProofStore | undefined {
    ...
    try {
      // The current persistence locator uses session.v3.jsonl; retaining only
      // the legacy basename silently discarded valid readback on every new run.
      if (!isAbsolute(logPath) || !/^session(?:\.v[123])?\.jsonl(?:\.zstd)?$/u.test(logPath.split(/[\\/]/u).at(-1) ?? '')) return undefined
