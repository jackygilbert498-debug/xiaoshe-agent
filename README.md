...
每轮直接用户任务会生成一份可替换的运行时路线快照，优先专用能力而不是通用 shell；低置信度时不猜工具，下一轮快照会取代旧路线而不是在历史中堆叠。复杂项目改动会在首次写入前要求真实读取/搜索证据，并在 `todo_write` 可见时先记录可验证步骤；“比较公开项目、最佳实践或最新资料后实现”会先走研究路线。用户已经提供本地参考时，本地读取可以满足证据要求，不会为了形式重复联网。预检最多纠偏两次，证据能力不存在或路线穷尽时不会形成死锁。对于联网搜索、网页读取、视觉、记忆、插件状态等已有专用入口的任务，第一次无必要的 shell 捷径会被引导回具体工具；用户明确要求终端，或正在编码、构建、测试时不会拦截。会话内真实成功会对同等匹配工具作温和加分，失败作温和降权，但不会覆盖当前任务意图。

同一调用连续失败两次会被拦截；明确不可用的工具在一次失败后不再靠换参数重试，只有另一个已注册工具也确认不可用时才封住整个能力族。路径不存在、图片尺寸过大和参数无效仍允许一次针对性修正，真正换到不同能力族也可以继续。写入、发送、部署、安装和界面动作成功后会进入待验证状态；只有对应的回读、状态、测试或可观察结果成功，运行时闭环提示才会消失。读图失败后最多尝试一次输入修正和一次已注册备用路线，不会转入 PowerShell OCR、临时安装或命令行延长读取。执行摘要只保存工具名、能力族、类别和计数，不保存原始参数、命令或错误正文。该保护不替代沙箱或审批。
