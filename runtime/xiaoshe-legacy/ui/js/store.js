import { createRuntimeProjectionAdapter, renderRuntimeProjectionSlots } from "./runtime-projection.js";

export { renderRuntimeProjectionSlots } from "./runtime-projection.js";
 ...
 * 会话列表 / 工具元数据缓存 / 只读运行时投影视图；1000 条环形 WS 事件缓冲（导出日志用）。
...
const runtimeProjectionAdapter = createRuntimeProjectionAdapter();
...
const state = {
  ...
  toolsCount: null,
  registryRev: null,
  runtimeProjection: runtimeProjectionAdapter.view(), // 仅四个公开槽位，非原始 RuntimeEvent
};
...
export function ingest(env) {
  ...
  state.runtimeProjection = runtimeProjectionAdapter.ingest(env);
  route(env);
  // A direct runtime.projection frame is deliberately never exposed as its
  // raw payload: UI consumers receive only the finite public view model.
  if (env.type !== "runtime.projection") emit(env.type, env.payload);
  if (env.type !== "session.snapshot") emit("runtime.projection", state.runtimeProjection);
}
...
function route(env) {
  ...
  switch (env.type) {
    case "session.snapshot":
      hydrate({ ...p, sid: p.sid ?? env.sid }, true);   // sid 在信封，不在 payload（§3.1）
      break;
    case "message.append":
      appendMessage(p);
      break;
...
export function hydrate(snapshot, runtimeAlreadyHandled = false) {
  if (!snapshot || typeof snapshot !== "object") return;
  if (!runtimeAlreadyHandled) {
    state.runtimeProjection = runtimeProjectionAdapter.ingest({ type: "session.snapshot", payload: snapshot });
  }
  ...
  emit("runtime.projection", state.runtimeProjection);
}
...
export function patchState(patch, silent = false) {
  ...
  const KEYS = ["todos", "notes", "jobs", "subagents", "vision_pending",
    ...
    "autonomy", "model", "run_active", "tool_round"];
  for (const k of KEYS) {
    if (k in patch) {
