...
import { backgroundSummary, hasVerifiedCompletion } from "./background-view.js";
...
import { createOfflineInboxBridge } from "./offline-inbox.js";
...
async function workerRequest(message) {
  if (!("serviceWorker" in navigator)) throw new Error("offline inbox unavailable");
  const registration = await navigator.serviceWorker.ready;
  const worker = navigator.serviceWorker.controller || registration.active;
  if (!worker) throw new Error("offline inbox unavailable");
  return new Promise((resolve, reject) => {
    const channel = new MessageChannel();
    const timer = setTimeout(() => reject(new Error("offline inbox timeout")), 5000);
    channel.port1.onmessage = ({ data }) => { clearTimeout(timer); resolve(data); };
    worker.postMessage(message, [channel.port2]);
  });
}

async function queueOfflineIntent(intent) {
  const result = await workerRequest({ type: "QUEUE_INTENT", intent });
  if (!result?.ok) throw new Error(result?.code || "offline inbox failed");
  return result;
}

const offlineInbox = createOfflineInboxBridge({
  connected: () => appStore.get().connected,
  post: (path, body) => net.post(path, body),
  workerRequest,
  notify: (text) => notify(text),
});
export function flushOfflineInbox() { return offlineInbox.flush(); }
...
export function renderTaskDetail(task, availability = null) {
  ...
  const runtimeProjection = appStore.get().runtimeProjection;
  const canProposeTool = hasVerifiedCompletion(task, runtimeProjection) && Boolean(availability?.changesetId)
  ...
  return el("section.task-detail", { "aria-live": "polite" },
    ...
    backgroundSummary(task, queueItem, runtimeProjection),
...
function openCreate(trigger) {
  ...
  const form = el("form.confirm-box", { onsubmit: async (event) => {
    ...
    try {
      ...
      if (!appStore.get().connected && project) {
        await queueOfflineIntent({ client_id: `client-${crypto.randomUUID()}`, project_id: project.id,
          title: text.slice(0, 80), goal: text, acceptance: items });
        handle.close("queued"); notify("任务意图已保存在本机，联网并验证身份后进入收件箱"); return;
      }
...
export async function revalidateAndOpenToolProposal(task, trigger, {
  ...
  runtimeProjectionFn = () => appStore.get().runtimeProjection,
  ...
  const stillCurrent = freshTask?.id === task.id
    ...
    && hasVerifiedCompletion(freshTask, runtimeProjectionFn())
...
export function mount({ toast } = {}) {
  ...
  navigator.serviceWorker?.addEventListener("message", (event) => {
    if (event.data?.type === "XIAOSHE_FLUSH_INBOX") void flushOfflineInbox();
  });
  window.addEventListener("online", () => void flushOfflineInbox());
  appStore.on("auth_error", () => offlineInbox.authError());
  appStore.on("auth_revoked", (event) => void offlineInbox.revoke(event));
  appStore.on("*", (type, payload) => {
    if (type === "runtime.projection") { render(); return; }
