...
function uniqueInternalLines(thought) {
  const seen = new Set();
  return String(thought ?? "").split(/\r?\n/).filter((line) => {
    const key = line.trim();
...
function thoughtDetails(thought) {
  ...
  d.append(el("summary", { text: "内部说明" }), el("pre.thought-body", { text: uniqueInternalLines(thought) }));
  return d;
}
...
export function renderMessage(msg, hooks = {}) {
  ...
  if (role === "assistant" && typeof msg.content === "string" && msg.content.trim()) {
    wrap.append(el("i.message-return-marker", { "aria-hidden": "true" }));
  }
