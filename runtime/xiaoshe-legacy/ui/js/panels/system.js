...
function secMetrics() {
  if (!metrics) {
    return metricsError
      ? el("div.p-err", { text: `运行压力读取失败：${metricsError}` })
      : skel();
  }
  const pressure = { low: "低", medium: "中", high: "高" }[metrics.pressure] || "未知";
  const trend = { stable: "平稳", rising: "上升", falling: "回落" }[metrics.trend] || "未知";
  const details = metrics.details || {};
  const sessionMetrics = metrics.session_observations || {};
  const sessionDetails = sessionMetrics.details || {};
  const sessionPressure = { low: "低", medium: "中", high: "高" }[sessionMetrics.pressure] || "未知";
  const sessionTrend = { stable: "平稳", rising: "上升", falling: "回落" }[sessionMetrics.trend] || "未知";
  const detailRows = [
    ["上下文", typeof details.context_usage_ratio === "number" ? `${Math.round(details.context_usage_ratio * 100)}%` : "—"],
    ["稳定前缀", details.stable_prefix_changed === true ? "有变化" : details.stable_prefix_changed === false ? "稳定" : "—"],
    ["压缩 / 召回", `${details.compaction_count ?? 0} / ${details.recall_count ?? 0}`],
    ["首 token / 总耗时", `${details.first_token_latency_ms ?? "—"} / ${details.total_duration_ms ?? "—"} ms`],
    ["会话压力", `${sessionPressure} · 近期${sessionTrend} · ${sessionMetrics.runner_active ? "运行中" : "空闲"}`],
    ["会话事件积压 / 投影落后",
      `${sessionDetails.event_backlog ?? 0} / ${sessionDetails.projection_lag ?? 0}`],
  ];
  return el("div.runtime-pressure", { dataset: { pressure: metrics.pressure || "unknown" } },
    el("div.runtime-pressure-summary", {},
      el("b", { text: `总体压力 ${pressure}` }), el("span", { text: `近期${trend}` })),
    metrics.warnings?.length
      ? el("p.runtime-pressure-note", { text: "部分资源指标不可用或达到警戒，当前结论保持谨慎。" })
      : el("p.runtime-pressure-note", { text: "指标仅用于观察，不会改变任务、上下文或模型决策。" }),
    el("details.runtime-pressure-details", {}, el("summary", { text: "查看详细数字" }),
      el("div", {}, detailRows.map(([key, value]) => kv(key, String(value))))));
}
...
async function loadRuntimeMetrics() {
  ...
  metricsError = "";
  try {
    const result = await api.get("/api/runtime-metrics");
    if (lifecycle !== lifecycleGeneration) return;
    metrics = result;
  } catch (error) {
    if (lifecycle !== lifecycleGeneration) return;
