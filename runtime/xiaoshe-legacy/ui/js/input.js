...
export function initInput(h = {}) {
  ...
  sendBtn?.addEventListener("click", () => {
    if (store.panels().run_active) doCancel();
    else doSend();
  });
  ...
  store.on("state.patched", (p) => { if (p && "run_active" in p) syncSendBtn(); });
  store.on("hydrated", syncSendBtn);
  ...
  function doCancel() {
    if (!store.get().connected || !store.panels().run_active) return;
    if (net.cancel()) window.XS.toast?.("正在停止本轮…");
  }

  function syncSendBtn() {
    if (!sendBtn) return;
    const running = store.panels().run_active === true;
    sendBtn.disabled = !store.get().connected || (!running && !ta.value.trim());
    sendBtn.classList.toggle("is-cancel", running);
    sendBtn.title = running ? "停止当前运行" : "发送";
    sendBtn.setAttribute("aria-label", running ? "停止当前运行" : "发送");
    const use = sendBtn.querySelector("use");
    if (use) use.setAttribute("href", running ? "#stop" : "#send");
  }
}
