...
def runtime_session_mode() -> str:
    """读取统一 RuntimeSession 开关；V1 默认 shadow，on 仅供受控测试。"""
    value = get("XIAOSHE_RUNTIME_SESSION", "shadow").strip().lower()
    if value not in {"off", "shadow", "on"}:
        raise ValueError("XIAOSHE_RUNTIME_SESSION 只能是 off、shadow 或 on")
    return value


def runtime_closure_mode() -> str:
    """Read the runtime closure activation flag for a new frozen revision."""
    value = get("XIAOSHE_RUNTIME_CLOSURE", "off").strip().lower() or "off"
    if value not in {"off", "shadow", "on"}:
        raise ValueError("XIAOSHE_RUNTIME_CLOSURE must be off, shadow or on")
    return value


def runtime_events_mode() -> str:
    """Read the RuntimeEvent flag; off preserves every legacy execution path."""
    value = get("XIAOSHE_RUNTIME_EVENTS", "off").strip().lower() or "off"
    if value not in {"off", "shadow", "on"}:
        raise ValueError("XIAOSHE_RUNTIME_EVENTS 只能是 off、shadow 或 on")
    return value
