...
TRASH_DIR = config.ROOT / ".state" / "trash" / "sessions"
...
def save_session(session_id: str, history: list, todos: list, notes: list | None = None,
                 ...
                 tasking_project_id: str | None = None, title: str | None = None) -> None:
    ...
    if title:
        rec["title"] = " ".join(str(title).split())[:80]
...
def rename_session(session_id: str, title: str) -> dict | None:
    """只改用户标题，不改稳定 sid/日志引用；原子写，旧档案兼容。"""
    clean = " ".join(str(title).split())[:80]
    if not clean:
    ...
    if data is None:
        return None
    data["title"] = clean
    _io.atomic_write_json(SESSIONS_DIR / f"{session_id}.json", data)
    _fts_upsert(session_id)
    return data


def trash_session(session_id: str) -> bool:
    """把会话档案移入可恢复回收区，而非永久删除。"""
    path = SESSIONS_DIR / f"{session_id}.json"
    if not path.exists():
        return False
    TRASH_DIR.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
    path.replace(TRASH_DIR / f"{stamp}-{session_id}.json")
    vision.purge_session(session_id)
    return True
