...
def _schedule_nominal_time(task: dict, now: datetime) -> datetime:
    """Return the stable scheduler slot that owns this firing's idempotency key."""
    if now.tzinfo is not UTC:
        raise ValueError("SCHEDULE_NOW_UTC_REQUIRED")
    if task.get("every_minutes"):
        created = datetime.fromisoformat(str(task["created_at"]).replace("Z", "+00:00")).astimezone(UTC)
        period = int(task["every_minutes"]) * 60
        if period <= 0:
            raise ValueError("SCHEDULE_INTERVAL_INVALID")
        elapsed = max(0, int((now - created).total_seconds()))
        return created + timedelta(seconds=(elapsed // period) * period)
    hour, minute = (int(part) for part in str(task["daily"]).split(":"))
    local_now = now.astimezone()
    nominal = local_now.replace(hour=hour, minute=minute, second=0, microsecond=0)
    if nominal > local_now:
        nominal -= timedelta(days=1)
    return nominal.astimezone(UTC)
...
def add_task(name: str, prompt: str, every: str | None = None, daily: str | None = None,
    ...
    if task_id is not None and config.tasking_mode() != "on":
        raise ValueError("绑定 Task 的定时任务需要 XIAOSHE_TASKING_V2=on")
...
def activate_schedule_trigger_closure(
    name: str,
    task: dict,
    activation,
    runtime_factory=None,
    *,
    legacy_route: str = "schedule.task_trigger",
):
    """Gate one Task-only trigger using its caller's frozen activation snapshot."""
    from .runtime_closure import RuntimeClosureError
    from .runtime_factory import RuntimeSessionFactory, record_trigger_closure_receipt
    from .runtime_session import RuntimeActivationSnapshot, RuntimeIdentity

    if not isinstance(activation, RuntimeActivationSnapshot):
        raise RuntimeClosureError("runtime_closure_assembly_failed")
    mode = activation.closure_mode
    if mode == "off":
        return None
    try:
        factory = runtime_factory or RuntimeSessionFactory()
        task_id = task.get("task_id")
        session = factory.create(RuntimeIdentity(
            f"schedule-{name}-{uuid.uuid4().hex}", "schedule",
            task_id=task_id if isinstance(task_id, str) and task_id else None,
        ), task={"id": task_id, "policy_id": task.get("policy_id")}
        if task_id else None, activation=activation)
        record_trigger_closure_receipt(session, legacy_route=legacy_route)
        return session
...
def run_task(name: str, popen=subprocess.Popen, runtime_factory=None) -> int:
    ...
    try:
        from .runtime_closure import RuntimeClosureError
        from .runtime_session import RuntimeActivationSnapshot

        activation = None
        try:
            activation = RuntimeActivationSnapshot.capture(include_runtime=False)
            closure_session = activate_schedule_trigger_closure(
                name, task, activation, runtime_factory,
            )
        except RuntimeClosureError as exc:
            shadow_error = activation is not None and activation.closure_mode == "shadow"
            record = {
                "start": start,
                "end": _now(),
                "outcome": "closure_shadow_error" if shadow_error else "closure_blocked",
                "exit_code": 0 if shadow_error else 1,
                "duration_s": round(time.monotonic() - t0, 1),
                "denied_calls": None,
                "session_id": None,
                "output_tail": exc.code,
            }
            if exc.report is not None:
                record["closure"] = exc.report.public_dict()
            append_history(name, record)
            if not shadow_error:
                return 1
            closure_session = None
        if closure_session is not None:
            report = closure_session.closure_report
            if report is not None:
                append_history(name, {
                    ...
                    "outcome": (
                        "closure_shadow"
                        if activation.closure_mode == "shadow"
                        else "closure_ready"
                    ),
                    "exit_code": 0,
                    "duration_s": round(time.monotonic() - t0, 1),
                    "denied_calls": None,
                    "session_id": closure_session.identity.session_id,
                    "output_tail": "",
                    "closure": report.public_dict(),
                })
            closure_session.close()
        if task.get("task_id"):
            ...
            try:
                _queue_bound_task(task, _schedule_nominal_time(task, _utc_now()))
