...
class ModelClient:
    def __init__(self, registry: "ModelRegistry"):
        ...
        self._last_probe_diagnostic: dict | None = None

    @property
    def last_probe_diagnostic(self) -> dict | None:
        return dict(self._last_probe_diagnostic) if self._last_probe_diagnostic else None
    ...
    def _chat_resolved(self, resolved: "ResolvedModel", messages: list[dict], tools: list | None,
        ...
        try:
            validator = getattr(self.registry, "validate_for_transport", None)
            ...
            raise ModelError(exc.code, resolved.provider.display_name) from None
    ...
    def chat_resolved(self, resolved: "ResolvedModel", messages: list[dict],
                      tools: list | None = None, timeout: int = 90, retry: int = 5,
                      on_delta=None, cache_key: str | None = None) -> dict:
        """Send through one immutable session-selected provider binding."""
        return self._chat_resolved(
            resolved, messages, tools, timeout, retry, on_delta, cache_key,
        )
    ...
    def probe(self, model_id: str) -> dict:
        ...
        try:
            ...
            self._record_probe(model_id, exc.code, started)
            raise ModelError(exc.code, "模型配置") from None
        try:
            ...
            self._record_probe(model_id, exc.code, started)
            raise
        self._record_probe(model_id, "available", started)
        return {"ok": True, "provider": resolved.provider.display_name, "model_id": resolved.model.id}

    def _record_probe(self, model_id: str, result_class: str, started_ns: int) -> None:
        self._last_probe_diagnostic = None
        recorder = getattr(self.registry, "record_verification", None)
        if not callable(recorder):
            self._last_probe_diagnostic = {"code": "verification_state_not_recorded"}
            return
        latency_ms = max(0, (time.perf_counter_ns() - started_ns + 999_999) // 1_000_000)
        try:
            recorder(model_id, result_class=result_class, latency_ms=latency_ms)
        except ModelRegistryError:
            # Probe classification must never replace the authoritative request result.
            self._last_probe_diagnostic = {"code": "verification_state_not_recorded"}
            return
