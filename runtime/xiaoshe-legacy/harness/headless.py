...
from .runtime_adapters import route_headless_runtime
from .runtime_session import RuntimeIdentity


def run_doctor(argv: list[str] | None = None) -> int:
    """Route the terminal diagnostic without starting a model session."""
...
def run_headless(prompt: str, allow: tuple[str, ...] = (), workdir: str | None = None,
    ...
    identity = RuntimeIdentity(f"runtime-{uuid.uuid4().hex}", "headless")
    return route_headless_runtime(
        identity,
        prompt,
        lambda value: _run_headless_legacy(
            value, allow=allow, workdir=workdir, model_fn=model_fn,
            no_mcp=no_mcp, session_prefix=session_prefix,
        ),
    )


def _run_headless_legacy(prompt: str, allow: tuple[str, ...] = (), workdir: str | None = None,
                         model_fn=kimi_chat, no_mcp: bool = False,
                         session_prefix: str = "headless-") -> int:
