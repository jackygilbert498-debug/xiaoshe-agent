...
from .verification_model import mirror_verification_result
...
class VerificationService:
    def __init__(self, store: TaskStore, artifacts: ArtifactStore | None = None, runner: CheckRunner | None = None,
        ...
        self.runtime_session_provider = None
        self.runtime_event_mirror = None

    def bind_runtime_events(self, runtime_session_provider, runtime_event_mirror) -> None:
        """Attach one live RuntimeSession lookup without changing verification ownership."""
        self.runtime_session_provider = runtime_session_provider
        self.runtime_event_mirror = runtime_event_mirror

    def _mirror_verification(self, verification: dict, task_id: str, run_id: str) -> None:
        provider = self.runtime_session_provider
        mirror = self.runtime_event_mirror
        if not callable(provider) or mirror is None:
            return
        try:
            runtime_session = provider(task_id, run_id)
            mirror_verification_result({**verification, "run_id": run_id}, runtime_session, mirror)
        except Exception:
            # Verification completion remains authoritative even if its
            # observational event cannot be mirrored.
            pass
    ...
    def run(self, task_id: str, profile: VerificationProfile, expected_version: int, actor: str="user", stop_token: StopToken | None=None) -> dict:
        ...
        self._mirror_verification(verification, task_id, changeset["run_id"])
        mapping={}
