...
def fsync_parent_directory(path) -> None:
    """Sync ``path``'s parent directory after creating or renaming an entry.

    POSIX directory fsync makes a successful create/rename survive a crash,
    whereas Windows does not offer the same directory-handle semantics.  The
    Windows no-op is deliberate; callers still use one portable primitive.
    """
    if os.name == "nt":
        return
    p = Path(path)
...
def ensure_durable_directory(path) -> None:
    """Create ``path`` and establish durable directory-entry barriers.

    An event log's first file may live below several newly-created state
    directories.  Syncing only the leaf directory does not make each link in
    that chain crash-durable, so walk from the filesystem root down to the
    requested directory.  Repeating the barriers for an existing directory is
    deliberate: it repairs an earlier failed sync before a later append can
    report success.  Windows uses the portable no-op in
    :func:`fsync_parent_directory`.
    """
    directory = Path(path)
    ...
    for ancestor in reversed(chain):
        fsync_parent_directory(ancestor)


def fsync_text_file_and_parent(path) -> None:
    """Re-establish data and directory durability for an already-written file."""
    p = Path(path)
    # Windows requires a write-capable descriptor for FlushFileBuffers even
    # though reconciliation never changes the file contents.
    with open(p, "r+b") as f:
        os.fsync(f.fileno())
    fsync_parent_directory(p)
...
def atomic_write_text(path, text: str) -> None:
    ...
    try:
        with open(tmp, "w", encoding="utf-8") as f:
            ...
            os.fsync(f.fileno())
        os.replace(tmp, p)
        if os.name != "nt":  # 保持旧调用方语义：目录 fsync 失败不推翻原子写成功本身
            try:
                fsync_parent_directory(p)
            except OSError:
                pass
...
def atomic_append_text(path, text: str) -> None:
    """Append one fully encoded text record and fsync it before reporting success.

    Callers that need cross-process ordering must hold :func:`file_lock` for
    ``path`` themselves.  Keeping the append primitive lock-free avoids a
    second, incompatible locking scheme while still ensuring that a returned
    success means the record reached the operating system's durable-write
    boundary.
    """
    p = Path(path)
    ensure_durable_directory(p.parent)
    ...
    with open(p, "ab") as f:
        ...
        f.flush()
        os.fsync(f.fileno())
    # Run this even for an existing file: a prior append can have written and
    # fsynced its bytes but failed the directory barrier before returning.
    fsync_parent_directory(p)


def truncate_text_tail(path, size: int) -> None:
    """Durably remove an incomplete tail while the caller holds ``file_lock``."""
    if size < 0:
