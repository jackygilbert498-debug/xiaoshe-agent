...
from .effect_recovery import EffectRecoveryCoordinator, EffectRecoveryError
...
from .inbox_adapters import AuthenticatedInboxRequest, AuthenticatedLocalInboxAdapter, InboxSecurityError
...
class TaskAPI:
    def __init__(self, store: TaskStore, engine: TaskEngine | None = None,
                 ...
                 effect_recovery: EffectRecoveryCoordinator | None = None,
                 recovery_session_provider: Callable[[str, str], object | None] | None = None):
        self.store = store
        ...
        self.recovery = RecoveryService(self.store, self.checkpoints, effect_recovery=effect_recovery)
        self.recovery_session_provider = recovery_session_provider
    ...
    def _recovery_session(self, task_id: str, effect_id: str):
        """Build only the exact ledger-bound runtime; never trust request input."""
        provider = self.recovery_session_provider
        if not callable(provider):
            raise EffectRecoveryError("RECOVERY_SESSION_UNAUTHORIZED")
        try:
            run_id = self.recovery.unknown_effect_run_id(effect_id, task_id=task_id)
            session = provider(task_id, run_id)
        except Exception as error:
            raise EffectRecoveryError("RECOVERY_SESSION_UNAUTHORIZED") from error
        return session

    def dispatch(self, method: str, path: str, body: Any = None,
                 headers: Mapping[str, str] | None = None, query: Mapping[str, list[str]] | None = None,
                 *, trusted_actor: str | None = None) -> HTTPResult:
        headers, query = headers or {}, query or {}
        try:
            return self._dispatch(method.upper(), path, body, headers, query, trusted_actor=trusted_actor)
            ...
            status = 409 if exc.code == "INBOX_IDEMPOTENCY_CONFLICT" else (403 if exc.code.endswith(("DENIED", "CROSS_SITE")) else 400)
            return self._error(status, exc.code, "收件箱请求被拒绝", "检查身份与请求来源")
            ...
            return self._error(409, exc.code, "外部效果恢复未执行", "刷新恢复状态后重试")
    ...
    def _dispatch(self, method: str, path: str, raw_body: Any,
        ...
        if path == "/api/v2/inbox/intents" and method == "POST":
            if not trusted_actor:
                raise InboxSecurityError("INBOX_AUTH_DENIED")
            adapter = AuthenticatedLocalInboxAdapter(
                inbox=TaskInbox(self.store),
                identity_id="ui_" + hashlib.sha256(trusted_actor.encode("utf-8")).hexdigest()[:24],
                project_ids=tuple(item["id"] for item in self.store.list_projects()),
            )
            request = AuthenticatedInboxRequest(
                origin=headers.get("Origin", ""), host=headers.get("Host", ""),
                fetch_site=headers.get("Sec-Fetch-Site", ""),
                idempotency_key=body.get("client_id", ""),
                body={key: value for key, value in body.items() if key != "client_id"},
            )
            identity = adapter.authenticate(request)
            intent = adapter.normalize(request, identity)
            receipt = adapter.submit(intent, adapter.deduplicate(intent))
            return HTTPResult(200 if receipt.duplicate else 201,
                              {"receipt": receipt.__dict__, "client_id": request.idempotency_key})
        ...
        effect_recovery_match = re.fullmatch(r"/api/v2/tasks/(tsk_[A-Za-z0-9_-]+)/effects/(eff_[A-Za-z0-9_-]+)/recovery", path)
        ...
        if effect_recovery_match and method == "GET":
            task_id, effect_id = effect_recovery_match.groups()
            self.store.get_task(task_id)
            case = self.recovery.inspect_unknown_effect(effect_id, session=self._recovery_session(task_id, effect_id))
            return HTTPResult(200, {"recovery": case.public_summary()})
        if effect_recovery_match and method == "POST":
            task_id, effect_id = effect_recovery_match.groups()
            self.store.get_task(task_id)
            # `actor` is intentionally not read from the JSON body.  The UI
            # handler binds it to its authenticated local session before this
            # call, so a browser form cannot impersonate another operator.
            if trusted_actor is None:
                raise EffectRecoveryError("RECOVERY_UI_IDENTITY_REQUIRED")
            decision = self._required_text(body, "decision")
            result = self.recovery.resolve_unknown_effect(
                effect_id, decision,
                evidence_ref=self._required_text(body, "evidence_ref"), actor=trusted_actor,
                session=self._recovery_session(task_id, effect_id),
                # Only an authenticated recovery POST can supply this opaque
                # capability.  The coordinator verifies its HMAC fingerprint
                # and never persists or returns the raw remote key.
                idempotency_proof=body.get("idempotency_proof") if decision == "retry" else None,
            )
            return HTTPResult(200, {"recovery": result.public_summary()})
