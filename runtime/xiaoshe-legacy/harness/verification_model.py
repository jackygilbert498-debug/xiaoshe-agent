...
def mirror_verification_result(result: Mapping[str, Any], runtime_session, runtime_event_mirror):
    """Mirror a completed legacy verification result without changing its owner.

    The caller retains the legacy verification result and any persistence/UI
    responsibility.  This bridge only forwards a completed fact to the
    best-effort Plan 10 adapter when a real RuntimeSession was supplied.
    """
    if runtime_session is None or runtime_event_mirror is None:
        return None
    try:
        enqueue = getattr(runtime_event_mirror, "enqueue_verification", None)
        if callable(enqueue):
            enqueue(result, runtime_session)
    except Exception:
        pass
    return None
