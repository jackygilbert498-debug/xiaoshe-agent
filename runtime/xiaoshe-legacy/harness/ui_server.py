...
from .runtime_adapters import route_gui_runtime
from .runtime_closure import RuntimeClosureError
from .runtime_factory import RuntimeSessionFactory
from .runtime_metrics import RuntimeMetrics, RuntimeMetricError
from .runtime_session import (RuntimeActivationSnapshot, RuntimeIdentity, RuntimeOutcome,
                              RuntimePolicySnapshot, RuntimeSession)
from .effect_recovery import EffectRecoveryCoordinator, RecoveryExecution
...
class _ProjectionEventSink:
    """Notify the UI only after the durable RuntimeEvent write has succeeded."""

    def __init__(self, sink, on_committed) -> None:
        self._sink = sink
    ...
    def reconcile(self, event) -> None:
        self._sink.reconcile(event)

    def read(self, **kwargs):
        return self._sink.read(**kwargs)
...
_INSTALLATION_ID_RE = re.compile(r"^installation_[0-9a-f]{32}$")
_INSTALLATION_ID_LOCK = threading.Lock()


def installation_principal(state_dir) -> str:
    """Return a non-secret, installation-stable authenticated principal."""
    path = Path(state_dir) / "installation_id"
    with _INSTALLATION_ID_LOCK:
        try:
            value = path.read_text(encoding="utf-8").strip()
        except FileNotFoundError:
            value = ""
        if _INSTALLATION_ID_RE.fullmatch(value):
            return value
        value = "installation_" + secrets.token_hex(16)
...
class UISession:
    ...
    def __init__(self, ctx: dict, sid: str, history: list, log_file: Path,
        ...
        self.inbox_principal = installation_principal(self.state_dir)
        ...
        self.task_api = None
        self.feishu_inbox = None
        self.feishu_inbox_diagnostic = {"enabled": False, "state": "disabled"}
        self.task_worker = None
        self.runtime_event_sink = None
        self.runtime_event_mirror = None
        self.projection_registry = None
        self._projection_transition_observer_token = None
        self._projection_rebuild_lock = threading.RLock()
        self._projection_generation = 0
        self._projection_unavailable = False
        self._projection_server_epoch = "epoch-" + secrets.token_hex(16)
        self._projection_runtime_sessions: dict[tuple[str, str], RuntimeSession] = {}
        self._projection_runtime_engines = {}
        self.runtime_metrics = RuntimeMetrics()
        self.session_runtime_metrics = RuntimeMetrics(
            max_streams=1, samples_per_stream=8,
            resource_sampler=lambda: {"status": "ok", "memory_ratio": 0.0, "cpu_ratio": 0.0})
        self._metric_sources = OrderedDict()
        self._metric_sources_lock = threading.RLock()
        self._session_metrics_lock = threading.RLock()
        self._metrics_last_overflow_total = 0
        self._metrics_active_stream = None
        self._runtime_event_commits = 0
        self._projection_applied_events = 0
        ...
        if self.tasking_mode == "on":
            ...
            from .runtime_events import JsonlRuntimeEventSink
            from .runtime_event_adapters import RuntimeEventMirror
            from .runtime_projections import (ProjectionRegistry, RuntimeSummaryProjection,
            ...
            try:
                ...
                projection_registry = ProjectionRegistry(task_store)
                projection_registry.register(TaskTimelineProjection())
                projection_registry.register(RuntimeSummaryProjection())
                self.projection_registry = projection_registry
                self.runtime_event_sink = _ProjectionEventSink(
                    JsonlRuntimeEventSink(self.state_dir / "runtime" / "events.jsonl"),
                    self._on_runtime_event_committed,
                )
                self.runtime_event_mirror = RuntimeEventMirror(
                    sink=self.runtime_event_sink,
                    diagnostics_path=self.state_dir / "runtime" / "adapter-diagnostics.jsonl",
                )
                task_engine = TaskEngine(task_store)
                # Recovery is shipped with the same live UI policy sources as
                # normal runtime work.  Its executors deliberately report a
                # durable manual handoff when the redacted effect ledger lacks
                # original action parameters.
                effect_recovery = EffectRecoveryCoordinator(
                    retry_executor=self._retry_unknown_effect,
                    compensation_executor=self._compensate_unknown_effect,
                    approval_bridge=self._approve_recovery_action,
                )
                self.task_api = TaskAPI(task_store, task_engine, workspace_root=config.ROOT,
                                        ...
                                        recovery_session_provider=self._recovery_runtime_session)
                self.task_api.verification.bind_runtime_events(
                    self._registered_runtime_projection_session, self.runtime_event_mirror,
                )
                self._rebuild_runtime_projections()
                self.tasking_diagnostic["store"] = "ready"
                # Optional Feishu ingress is registered only when every public
                # binding and both protected SecretStore references resolve.
                # Partial configuration is indistinguishable from disabled to
                # the HTTP surface and never opens a second execution path.
                from .feishu_inbox import FeishuInboxAdapter, load_feishu_config
                from .model_secrets import SecretStore
                from .task_inbox import TaskInbox
                feishu_config = load_feishu_config(
                    getter=config.get,
                    secret_store=SecretStore(self.state_dir / "feishu_secrets.bin"),
                )
                if feishu_config is not None:
                    try:
                        task_store.get_project(feishu_config.project_id)
                    except KeyError:
                        self.feishu_inbox_diagnostic = {"enabled": False, "state": "project_unavailable"}
                    else:
                        self.feishu_inbox = FeishuInboxAdapter(
                            inbox=TaskInbox(task_store), config=feishu_config)
                        self.feishu_inbox_diagnostic = {"enabled": True, "state": "ready"}
        ...
        if model_fn is None:
            ...
            def model_fn(messages, tools=None):
                binding = self.current_model_binding()
                if binding is None:
                    return self.model_client.chat(
                        messages, tools=tools, model_id=self.current_model_id())
                return self.model_client.chat_resolved(binding, messages, tools=tools)
    ...
    def _recovery_runtime_session(self, task_id: str, run_id: str) -> RuntimeSession | None:
        """Build an exact Task/Run-bound session from the live UI sources.

        Request JSON never supplies a RuntimeSession.  Recovery gets the same
        model registry, control store and policy assembly path as GUI runtime,
        while the task API supplies the ledger-bound Task and Run identifiers.
        """
        try:
            if self.task_api is None:
                return None
            task = self.task_api.store.get_task(task_id)
            run = self.task_api.store.get_run(run_id)
            if run.get("task_id") != task_id or task.get("project_id") is None:
                return None
            frozen = json.loads(run.get("policy_json") or "{}")
            mode = frozen.get("mode", "collaborate")
            unattended = frozen.get("unattended", False)
            if mode not in {"observe", "plan", "collaborate"} or type(unattended) is not bool:
                return None
            ...
            facts = {"policy_snapshot": {
                ...
                "tool_capability_ids": sorted(tools_mod.REGISTRY),
            }}
            return factory.create(
                RuntimeIdentity(
                    self.sid, "gui", project_id=task["project_id"], task_id=task_id, run_id=run_id,
                ),
                task=task, run=run, ctx=facts,
            )
        except Exception:
            return None

    def _runtime_projection_session(self, task_id: str, run_id: str) -> RuntimeSession | None:
        """Recover an observer-only session from the durable reviewed Task/Run pair."""
        try:
            if self.task_api is None:
                return None
            task = self.task_api.store.get_task(task_id)
            ...
            if run.get("task_id") != task_id or task.get("project_id") is None:
                return None
            changeset = self.task_api.store.current_changeset(task_id)
            if not isinstance(changeset, dict) or changeset.get("run_id") != run_id:
                return None
            frozen = json.loads(run.get("policy_json") or "{}")
            mode = frozen.get("mode", "collaborate")
            unattended = frozen.get("unattended", False)
            controls = {
                "sandbox_enabled": frozen.get("sandbox_enabled"),
                "network_mode": frozen.get("network_mode"),
                "heartbeat_enabled": frozen.get("heartbeat_enabled"),
            }
            budget = frozen.get("budget")
            if (mode not in {"observe", "plan", "collaborate"} or type(unattended) is not bool
                    or type(controls["sandbox_enabled"]) is not bool
                    or controls["network_mode"] not in {"off", "proxy", "open"}
                    or type(controls["heartbeat_enabled"]) is not bool
                    or not isinstance(budget, dict)):
                return None
            recovered_digest = "sha256:" + hashlib.sha256(
                f"projection-observer:{task_id}:{run_id}".encode("utf-8")
            ).hexdigest()
            return RuntimeSession(
                identity=RuntimeIdentity(
                    run_id, "worker", project_id=task["project_id"], task_id=task_id, run_id=run_id,
                ),
                policy=RuntimePolicySnapshot(
                    model_id="runtime:recovered-projection-observer",
                    plan_revision_id=run.get("plan_revision_id"), workspace_id=run.get("workspace_id"),
                    permission_mode=mode, sandbox_enabled=controls["sandbox_enabled"],
                    network_mode=controls["network_mode"], heartbeat_enabled=controls["heartbeat_enabled"],
                    unattended=unattended, budget=budget, capability_digest=recovered_digest,
                ),
                runner=lambda _value: RuntimeOutcome("failed", error_code="projection_observer_only"),
                activation=RuntimeActivationSnapshot("off", "off", "off"),
            )
        except Exception:
            return None

    def _registered_runtime_projection_session(self, task_id: str, run_id: str) -> RuntimeSession | None:
        """Return a live or durably recovered observer for this exact reviewed Run."""
        if not isinstance(task_id, str) or not isinstance(run_id, str):
            return None
        with self._projection_rebuild_lock:
            current = self._projection_runtime_sessions.get((task_id, run_id))
            if current is not None:
                return current
            recovered = self._runtime_projection_session(task_id, run_id)
            if recovered is None:
                return None
            self._register_runtime_projection_session(recovered)
            return self._projection_runtime_sessions.get((task_id, run_id))

    def _register_runtime_projection_session(self, runtime: RuntimeSession) -> None:
        """Attach post-commit Task transitions to the concrete Worker session."""
        identity = getattr(runtime, "identity", None)
        task_id = getattr(identity, "task_id", None)
        run_id = getattr(identity, "run_id", None)
        if (self.task_api is None or self.runtime_event_mirror is None
                or not isinstance(task_id, str) or not isinstance(run_id, str)):
            return
        from .task_engine import TaskEngine
        key = (task_id, run_id)
        with self._projection_rebuild_lock:
            current = self._projection_runtime_sessions.get(key)
            if current is runtime:
                return
            replaced = self._projection_runtime_engines.pop(key, None)
            if replaced is not None:
                try:
                    replaced.close()
                except Exception:
                    pass
            for old_key in [item for item in self._projection_runtime_engines if item[0] == task_id and item != key]:
                try:
                    self._projection_runtime_engines.pop(old_key).close()
                except Exception:
                    pass
                self._projection_runtime_sessions.pop(old_key, None)
            self._projection_runtime_sessions[key] = runtime
            self._projection_runtime_engines[key] = TaskEngine(
                self.task_api.store, runtime_session=runtime,
                runtime_event_mirror=self.runtime_event_mirror,
            )

    def _mirror_committed_task_transition(self, before: dict, after: dict) -> None:
        """Observe a durable Run-bound transition without taking ownership of it."""
        mirror = self.runtime_event_mirror
        task_id = after.get("id") if isinstance(after, dict) else None
        if not isinstance(task_id, str) or not task_id:
            return
        run_id = after.get("active_run_id") or before.get("active_run_id")
        if not isinstance(run_id, str) or not run_id:
            return
        runtime = self._runtime_projection_session(task_id, run_id)
        if runtime is None or mirror is None:
            return
        try:
            mirror.enqueue_task_transition(before, after, runtime)
        except Exception:
            pass

    def _retry_unknown_effect(self, _case, _runtime: RuntimeSession, _proof) -> RecoveryExecution:
        """Do not fabricate a replay when the durable ledger has no request body."""
        return RecoveryExecution("manual_required")

    def _compensate_unknown_effect(self, _case, _runtime: RuntimeSession) -> RecoveryExecution:
        """Do not fabricate a compensation command when its registered args are absent."""
        return RecoveryExecution("manual_required")

    def _approve_recovery_action(self, case, _runtime: RuntimeSession, operation: str) -> bool:
        """Bridge a current ``ask`` verdict through the normal UI approval channel."""
        verdict = self.ui_approver(
            case.tool,
            {"recovery_operation": operation},
            "The current recovery policy requires another approval.",
            force_ask=True,
            ctx=self.ctx,
        )
        return verdict in {True, "always", "persist"}
    ...
    def _run_background_task(self, run_context) -> None:
        ...
        runtime_binding = run_context.policy_snapshot.get("_runtime_event_binding")
        runtime_session = runtime_binding.get("runtime_session") if isinstance(runtime_binding, dict) else None
        runtime_event_mirror = runtime_binding.get("runtime_event_mirror") if isinstance(runtime_binding, dict) else None
        runtime_ctx = {"todos": [], "memory_file": memory.MEMORY_FILE,
                       ...
                       "_runtime_event_mirror": runtime_event_mirror}
        history = []
        # Project Memory is an optional, separately delivered extension.  A
        # clean committed runner must still execute the ordinary task path
        # when that extension is absent; it must not claim an injection.
        try:
            from .project_memory_retrieval import ProjectMemoryRetriever, RetrievalQuery
        except ModuleNotFoundError as error:
            if error.name not in {"harness.project_memory_retrieval", "project_memory_retrieval"}:
                raise
        else:
            ...
            if retrieved.records:
                ...
                history.append(agent._bind_context_message(
                    {"role": "system", "content": retriever.render_for_context(retrieved)},
                    runtime_ctx, kind="evidence",
                ))
    ...
    def start_task_worker(self) -> None:
        ...
        self.task_worker = TaskWorker(self.task_api.store, RunLeaseService(self.task_api.store), self.task_api.engine,
                                      ...
                                      runtime_event_mirror=self.runtime_event_mirror,
                                      runtime_session_observer=self._register_runtime_projection_session)
    ...
    def stop_task_worker(self) -> None:
        ...
        if self.task_api is not None and self._projection_transition_observer_token is not None:
            self.task_api.store.remove_transition_observer(self._projection_transition_observer_token)
            self._projection_transition_observer_token = None
        with self._projection_rebuild_lock:
            observers = tuple(self._projection_runtime_engines.values())
            self._projection_runtime_engines.clear()
            self._projection_runtime_sessions.clear()
        for observer in observers:
            try:
                observer.close()
            except Exception:
                pass
        if self.runtime_event_mirror is not None:
            self.runtime_event_mirror.close(timeout=2)
    ...
    def _rebuild_runtime_projections(self) -> bool:
        """Replay the committed immutable stream into the disposable read models."""
        if self.projection_registry is None or self.runtime_event_sink is None:
            return False
        try:
            with self._projection_rebuild_lock:
                report = self.projection_registry.rebuild(self.runtime_event_sink.read_strict())
                self._projection_generation += 1
                self._projection_unavailable = not bool(report.ok)
                if report.ok:
                    self._runtime_event_commits = report.applied_events
                    self._projection_applied_events = report.applied_events
            return bool(report.ok)
        except Exception:
            with self._projection_rebuild_lock:
                self._projection_generation += 1
                self._projection_unavailable = True
            return False

    def _on_runtime_event_committed(self, event) -> None:
        """Publish one newly rebuilt public projection without reconnecting clients."""
        payload = None
        failed = False
        generation = self._projection_generation
        self._runtime_event_commits += 1
        try:
            with self._projection_rebuild_lock:
                if self.projection_registry is None or self.runtime_event_sink is None:
                    failed = True
                    ...
                    report = self.projection_registry.rebuild(self.runtime_event_sink.read_strict())
                    failed = not bool(report.ok)
                    if report.ok:
                        self._projection_applied_events = report.applied_events
                self._projection_generation += 1
                self._projection_unavailable = failed
                generation = self._projection_generation
                if not failed:
                    payload = self._runtime_projection_view_unlocked()
        except Exception:
            with self._projection_rebuild_lock:
                self._projection_generation += 1
                self._projection_unavailable = True
            ...
            failed = True
        if failed:
            error = {"code": "projection_unavailable", "generation": generation,
                     "server_epoch": self._projection_server_epoch}
            for key in ("task_id", "runtime_id", "run_id"):
                value = getattr(event, key, None)
                if isinstance(value, str) and value:
                    error[key] = value
            try:
                ui_bus.emit("runtime.projection.error", error)
            except Exception:
                pass
            return
        try:
            ui_bus.emit("runtime.projection", payload)
        except Exception:
            pass
    ...
    def sink(self, type_: str, payload) -> None:
        try:
            if type_ == "message.append" and isinstance(payload, dict):
                ...
                source = self._metric_source(self._metrics_active_stream)
                if (payload.get("role") == "assistant" and source["run_started"] is not None
                        and source["first_token_ms"] is None):
                    source["first_token_ms"] = max(
                        0, (time.monotonic() - source["run_started"]) * 1000)
            ...
            elif type_ == "compaction.event":
                source = self._metric_source(self._metrics_active_stream)
                source["compaction_count"] += 1
    ...
    def snapshot_payload(self) -> dict:
        with ui_bus.STATE_LOCK:
            ...
            return {"contract_v": 1,
                    ...
                    "runtime_projections": self.runtime_projection_view(),
                    "negotiated": {"v": 1}}

    def runtime_projection_view(self, task_id: str | None = None) -> dict:
        """Return the whitelisted, disposable projection read models for UI.

        This is intentionally not a RuntimeEvent endpoint: raw payload JSON,
        adapter diagnostics, and database failures do not cross the UI trust
        boundary.  Missing/rebuilding projections are an empty, ignorable
        public diagnostic, never a reconstructed task truth.
        """
        with self._projection_rebuild_lock:
            return self._runtime_projection_view_unlocked(task_id)

    def runtime_metrics_view(self) -> dict:
        """Observe bounded session pressure without exporting labels or content."""
        task_id, run_id = self._metric_stream()
        source = self._metric_source((task_id, run_id))
        values = {
            "compaction_count": source["compaction_count"],
            "recall_count": source["recall_count"],
        }
        used = source["context_input_tokens"]
        window = source["context_window_tokens"]
        if type(used) is int and type(window) is int and window > 0:
            values["context_usage_ratio"] = min(1.0, used / window)
        prefix_hash = source["stable_prefix_hash"]
        if isinstance(prefix_hash, str):
            values["stable_prefix_hash"] = prefix_hash
        if type(source["last_duration_ms"]) in (int, float):
            values["total_duration_ms"] = source["last_duration_ms"]
        if type(source["first_token_ms"]) in (int, float):
            values["first_token_latency_ms"] = source["first_token_ms"]
        try:
            self.runtime_metrics.record(str(task_id), str(run_id), values)
            task_public = self.runtime_metrics.snapshot(str(task_id), str(run_id))
        except RuntimeMetricError:
            task_public = {"v": 1, "status": "hold", "pressure": "high",
                           "trend": "stable", "sample_count": 0,
                           "warnings": ["sample_failed"], "details": {}}
        session_public = self._session_runtime_metrics_view()
        order = {"low": 0, "medium": 1, "high": 2}
        task_pressure = str(task_public.get("pressure", "high"))
        session_pressure = str(session_public.get("pressure", "high"))
        overall_pressure = max((task_pressure, session_pressure), key=order.__getitem__)
        dominant = session_public if order[session_pressure] > order[task_pressure] else task_public
        public = dict(task_public)
        public["pressure"] = overall_pressure
        public["status"] = (
            "hold" if "hold" in {task_public.get("status"), session_public.get("status")}
            else "pass")
        public["trend"] = dominant.get("trend", "stable")
        public["warnings"] = sorted(set(task_public.get("warnings", ())).union(
            session_public.get("warnings", ())))
        public["task_run"] = task_public
        public["session_observations"] = session_public
        return public

    def _session_runtime_metrics_view(self) -> dict:
        with self._session_metrics_lock:
            dispatcher = None
            getter = getattr(self.runtime_event_mirror, "dispatcher_metrics", None)
            if callable(getter):
                try:
                    candidate = getter()
                    ...
                    dispatcher = None
            overflow_total = (
                dispatcher.get("overflow_total", 0) if dispatcher is not None else 0)
            if type(overflow_total) is not int or overflow_total < 0:
                overflow_total = 0
            overflow_delta = (
                overflow_total - self._metrics_last_overflow_total
                if overflow_total >= self._metrics_last_overflow_total else overflow_total)
            self._metrics_last_overflow_total = overflow_total
            pending = dispatcher.get("pending", 0) if dispatcher is not None else 0
            if type(pending) is not int or pending < 0:
                pending = 0
            values = {
                "event_backlog": pending,
                "event_overflow": overflow_delta,
                "projection_lag": max(
                    0, self._runtime_event_commits - self._projection_applied_events),
            }
            try:
                self.session_runtime_metrics.record("session", "runtime", values)
                public = self.session_runtime_metrics.snapshot("session", "runtime")
            except RuntimeMetricError:
                public = {"v": 1, "status": "hold", "pressure": "high",
                          "trend": "stable", "sample_count": 0,
                          "warnings": ["sample_failed"], "details": {}}
            public["scope"] = "session"
            public["runner_active"] = self._runner_lock.locked()
            public["dispatcher_available"] = dispatcher is not None
            if dispatcher is not None:
                public["dispatcher"] = {
                    key: dispatcher[key] for key in (
                        "capacity", "pending", "backpressured_total", "overflow_total", "closed")
                    if key in dispatcher
                }
            return public

    def _metric_stream(self) -> tuple[str, str]:
        run_context = self.ctx.get("_run_context")
        candidate = (str(getattr(run_context, "task_id", None) or "session"),
                     str(getattr(run_context, "run_id", None) or self.sid))
        try:
            return RuntimeMetrics._stream(*candidate)
        except RuntimeMetricError:
            return "session", self.sid

    def _metric_source(self, stream=None) -> dict:
        stream = stream or self._metric_stream()
        with self._metric_sources_lock:
            source = self._metric_sources.pop(stream, None)
            if source is None:
                source = {"compaction_count": 0, "recall_count": 0,
                          "last_duration_ms": None, "first_token_ms": None,
                          "run_started": None, "context_input_tokens": None,
                          "context_window_tokens": None, "stable_prefix_hash": None}
            self._metric_sources[stream] = source
            while len(self._metric_sources) > 64:
                self._metric_sources.popitem(last=False)
            return source

    def _capture_metric_context(self, stream=None) -> None:
        """Bind a completed Run's public context facts to that Run only."""
        source = self._metric_source(stream)
        usage = ui_state.usage_safe(self.ctx)
        budget = self.ctx.get("_context_budget")
        used = usage.get("input_tokens")
        window = usage.get("window")
        if type(window) is not int or window <= 0:
            candidate = getattr(budget, "window_tokens", None)
            window = candidate if type(candidate) is int and candidate > 0 else None
        prefix = self.ctx.get("_stable_prefix_hash")
        source["context_input_tokens"] = (
            used if type(used) is int and 0 <= used <= 1_000_000_000 else None)
        source["context_window_tokens"] = (
            window if type(window) is int and 0 < window <= 1_000_000_000 else None)
        source["stable_prefix_hash"] = (
            prefix if isinstance(prefix, str) and re.fullmatch(r"[0-9a-f]{64}", prefix) else None)

    def _runtime_projection_view_unlocked(self, task_id: str | None = None) -> dict:
        """Read one coherent derived snapshot while the replay generation is held."""
        empty = {"v": 1, "generation": self._projection_generation,
                 "server_epoch": self._projection_server_epoch, "task_id": task_id,
                 "RuntimeSummaryProjection": [], "TaskTimelineProjection": []}
        if self.task_api is None:
            return empty
        if self._projection_unavailable:
            return {**empty, "error": {"code": "projection_unavailable"}}
        try:
            with self.task_api.store.derived_state_connection() as conn:
                if task_id is None:
                    selected_run = conn.execute("""
                        SELECT runtime_id,task_id,run_id FROM runtime_task_timeline
                        WHERE task_id IS NOT NULL
                        ORDER BY projection_order DESC LIMIT 1
                    """).fetchone()
                else:
                    selected_run = conn.execute("""
                        SELECT runtime_id,task_id,run_id FROM runtime_task_timeline
                ...
                if selected_run is None:
                    return empty
                selected = selected_run["task_id"]
                runtime_id = selected_run["runtime_id"]
                ...
                summaries = [dict(row) for row in conn.execute("""
                    ...
                    WHERE task_id=? AND runtime_id=? AND run_id IS ?
                """, (selected, runtime_id, run_id))]
                timeline = []
                for row in conn.execute("""
                    SELECT event_id,runtime_id,task_id,run_id,event_type,occurred_at,seq,payload_json
                    FROM runtime_task_timeline
                    WHERE task_id=? AND runtime_id=? AND run_id IS ?
                    ORDER BY projection_order ASC
                """, (selected, runtime_id, run_id)):
                    item = {key: row[key] for key in (
                        "event_id", "runtime_id", "task_id", "run_id", "event_type", "occurred_at", "seq",
                    )}
                    if item["event_type"] == "verification.finished":
                        try:
                ...
                return {"v": 1, "generation": self._projection_generation,
                        "server_epoch": self._projection_server_epoch, "task_id": selected,
                        "RuntimeSummaryProjection": summaries,
    ...
    def _runner_body(self, text: str) -> None:
        ...
        metric_stream = self._metric_stream()
        metric_source = self._metric_source(metric_stream)
        metric_source["run_started"] = metrics_started
        metric_source["first_token_ms"] = None
        self._metrics_active_stream = metric_stream
        try:
            if self._fallback_events:
                with ui_bus.STATE_LOCK:
                    pre_ids = set(self.msg_ids.sync(self.history))
            def _legacy(value: str):
                return agent.run_once(value, self.history, model_fn=self.model_fn,
            ...
            reply = route_gui_runtime(
                RuntimeIdentity(
                    self.sid, "gui", project_id=self.ctx.get("_tasking_project_id")),
                text, _legacy, factory=factory, ctx=facts, request_ctx=self.ctx,
            )
            self._capture_metric_context(metric_stream)
        except RuntimeClosureError as e:
            self._emit_system("（运行能力尚未闭合，本次未启动）")
            payload = {
                "level": "error",
                "code": e.code,
                "text": "运行能力闭包未通过；请查看安全诊断后创建新会话。",
            }
            if e.report is not None:
                payload["closure"] = e.report.public_dict()
            ui_bus.emit("system.alert", payload)
            ...
            metric_source["last_duration_ms"] = max(
                0, (time.monotonic() - metrics_started) * 1000)
            metric_source["run_started"] = None
            if self._metrics_active_stream == metric_stream:
                self._metrics_active_stream = None
            try:
                if self._fallback_events:
                ...
                if agent._ends_clean(self.history):   # 只存干净断点（repl :1557-1561）
                    try:
                        session.save_session(self.sid, self.history,
                                             ...
                                             title=self.ctx.get("_session_title"))
                    except OSError as e:
                        self.log(f"会话存档失败（对话不受影响）：{e}")
            except Exception:
                pass
            self.ctx["_run_active"] = False
            # usage 只在模型响应后才完整；无论走直接事件还是 fallback，
            # 收尾都应广播一次，否则固定状态栏要刷新页才看到真 token。
            self._emit_dirty("run_active", "tool_round", "usage")
            self._runner_lock.release()
    ...
    def _reset_model_selection(self) -> None:
        ...
        try:
            ...
            binding = self.model_registry.resolve(model_id)
    ...
    def current_model_binding(self):
        """Return the session's one immutable model/provider/endpoint/credential binding."""
        model_id = self.current_model_id()
        binding = self._model_binding
        if binding is not None and binding.model.id == model_id:
            return binding
        try:
            binding = self.model_registry.resolve(model_id)
        except ModelRegistryError:
    ...
    def handle_set_model_id(self, model_id: str) -> tuple[bool, str]:
        ...
        # This single immutable object is the authoritative next-request selection.
        # Public ctx fields are compatibility projections and never contain the key.
        self._model_binding = resolved
    ...
    def _dispatch_command(self, name: str, args: dict) -> None:
        ...
        if name == "todos":
            ...
            self._metric_source(self._metrics_active_stream)["recall_count"] += 1
    ...
    def handle_rename_session(self, sid: str, title: str) -> tuple[bool, str]:
        if not _SID_RE.match(sid):
            return False, "bad_sid"
        clean = " ".join(title.split())[:80]
        if sid == self.sid and not session.load_session(sid):
            if not agent._ends_clean(self.history):
                return False, "busy"
            session.save_session(sid, self.history, self.ctx.get("todos", []), notes.current(self.ctx),
                                 tasking_project_id=self.ctx.get("_tasking_project_id"), title=clean)
            self.ctx["_session_title"] = clean
            return True, "ok"
        rec = session.rename_session(sid, clean)
        if rec is None:
    ...
    def handle_delete_session(self, sid: str) -> tuple[bool, str]:
        if not _SID_RE.match(sid):
            return False, "bad_sid"
        if sid == self.sid:
            return False, "current"
        if not session.trash_session(sid):
            return False, "not_found"
        projects.remove_session(sid, self.state_dir / "projects.json")
        return True, "ok"
    ...
    def handle_new_session(self) -> str | None:
        ...
        try:
            ...
            with ui_bus.STATE_LOCK:
                ...
                self.ctx["todos"] = []
                self.ctx.pop("_session_title", None)
...
def _resolved_path_for(name, args):
    ...
    def _path_error(raw):
        return {"error": "path_error", "raw": str(raw)[:80]}

    vals = []
    for c in cands:
        if any(ord(ch) < 32 for ch in c):
            vals.append(_path_error(c))
            continue
        try:
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _route(self, method: str) -> None:
        ...
        if path == "/integrations/feishu/events":
            if method != "POST" or self.server.session.feishu_inbox is None:
                self._err(404, "not_found", "integration unavailable", "enable a complete local Feishu configuration")
                return
            from .feishu_inbox import FeishuWebhookRequest
            from .inbox_adapters import InboxSecurityError
            try:
                result = self.server.session.feishu_inbox.handle(
                    FeishuWebhookRequest(dict(self.headers.items()), self._read_feishu_body(limit=1 << 20)))
            except InboxSecurityError as exc:
                self.server.session.log(f"feishu callback rejected code={exc.code}")
                status = 409 if exc.code == "FEISHU_REPLAYED" else 401 if exc.code in {
                    "FEISHU_SIGNATURE_INVALID", "FEISHU_TIMESTAMP_INVALID", "FEISHU_TOKEN_INVALID",
                    "FEISHU_TENANT_DENIED", "FEISHU_APP_DENIED", "FEISHU_ACTOR_DENIED",
                } else 400
                self._json({"error": {"code": exc.code}}, status=status)
                return
            if set(result.public_body) == {"challenge"}:
                self._json_exact(result.public_body, status=result.status_code)
            else:
                self._json(result.public_body, status=result.status_code)
            return
    ...
    def _api_inner(self, method: str, path: str, query: dict) -> None:
        ...
        if path.startswith("/api/v2/"):
            ...
            # The pairing-token gate has already authenticated this local UI
            # session.  Bind recovery actors here rather than accepting the
            # caller-controlled JSON `actor` field.
            result = sess.task_api.dispatch(
                method, path, payload, dict(self.headers), query,
                trusted_actor=sess.inbox_principal,
            )
        ...
        if method == "GET":
            ...
            if path == "/api/runtime-projections":
                requested_task = (query.get("task_id") or [None])[0]
                if requested_task is not None and not _REF_RE.fullmatch(requested_task):
                    return self._err(400, "bad_request", "task_id 格式无效", "使用任务工作台返回的 task_id")
                return self._json(sess.runtime_projection_view(requested_task))
            if path == "/api/runtime-metrics":
                return self._json(sess.runtime_metrics_view())
            if path == "/api/tools":
                return self._tools()
            if path == "/api/state":
                ...
                return self._json({"sid": sess.sid, **ui_state.snapshot_full(sess.ctx),
                                   "pending_approvals": ui_bus.pending_approvals(),
                                   "runtime_projections": sess.runtime_projection_view()})
        ...
        if method == "POST":
            ...
            if path == "/api/sessions/rename":
                p = self._json_body(body)
                ui_schema.check(p, ui_schema.SCHEMA_SESSION_RENAME, "sessions.rename")
                ok, reason = sess.handle_rename_session(p["sid"], p["title"])
                if not ok:
                    status = 404 if reason == "not_found" else 409 if reason == "busy" else 400
                    return self._err(status, reason, "会话改名失败", "刷新会话列表后重试")
                return self._json({"ok": True, "sid": p["sid"], "title": " ".join(p["title"].split())[:80]})
            if path == "/api/sessions/delete":
                p = self._json_body(body)
                ui_schema.check(p, ui_schema.SCHEMA_SESSION_DELETE, "sessions.delete")
                ok, reason = sess.handle_delete_session(p["sid"])
                if not ok:
                    status = 404 if reason == "not_found" else 409 if reason == "current" else 400
                    return self._err(status, reason, "会话删除失败",
                                     "当前会话请先新建会话；历史会话可直接删除")
                return self._json({"ok": True, "deleted": p["sid"], "recoverable": True})
            if path == "/api/projects":
                p = self._json_body(body)
    ...
    def _model_probe_error(self, exc: Exception) -> None:
        ...
        if isinstance(exc, KimiError):
            ...
            status = {"invalid_url": 400, "missing_credential": 400, "disabled_model": 400,
                      "authentication_failed": 401, "quota_limited": 429,
    ...
    def _read_feishu_body(self, limit: int) -> bytes:
        """Read an authenticated webhook body only after strict HTTP framing checks."""
        from .inbox_adapters import InboxSecurityError
    ...
    def _json_exact(self, obj: dict, status: int = 200) -> None:
        """Protocol-owned JSON without the UI API envelope (Feishu URL challenge)."""
