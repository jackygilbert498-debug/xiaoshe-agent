...
def wire_references(history: list, ctx: dict) -> list:
    """Append bounded image identities for budgeting, never bytes, URLs or paths."""
    pend = ctx.pop("_vision_pending", None) or []
    sid = ctx.get("session_id")
    overflow = pend[:-VISION_LIVE_MAX] if len(pend) > VISION_LIVE_MAX else []
    parts, tok = [], 0
    for ref in pend[-VISION_LIVE_MAX:]:
        metadata = meta(sid, ref)
        if not metadata or metadata.get("evicted"):
            continue
        parts.append({
            "type": "image_reference",
            "ref": ref,
            "media_id": ref,
            "purpose": _label_text(sid, ref),
            "mime_type": "image/png",
        })
        tok += metadata.get("tokens_est") or 0
    ctx["_vision_last_tokens"] = tok
    if not parts:
        return history
    hint = _WIRE_HINT
    if overflow:
        hint += f"（本批只附了最近 {VISION_LIVE_MAX} 张；{'、'.join(overflow)} 未附上。）"
    return list(history) + [{"role": "user", "content": [
        {"type": "text", "text": hint}, *parts,
    ]}]


def materialize_references(history: list, ctx: dict) -> list:
    """Resolve selected safe refs into provider payloads at the final send boundary."""
    sid = ctx.get("session_id")
    current_turn = ctx.get("_turn")
    output = []
    for message in history:
        ...
        changed = False
        for part in content:
            if not isinstance(part, dict) or part.get("type") != "image_reference":
                parts.append(part)
                continue
            changed = True
            ref = part.get("ref") or part.get("media_id")
            if not isinstance(ref, str):
                continue
            uri = _wire_image_uri(sid, ref, current_turn, runner=ctx.get("_sips_runner"))
            if not uri:
                continue
            parts.append({"type": "text", "text": str(part.get("purpose") or ref)})
            parts.append({"type": "image_url", "image_url": {"url": uri}})
        output.append({**message, "content": parts} if changed else message)
    return output
