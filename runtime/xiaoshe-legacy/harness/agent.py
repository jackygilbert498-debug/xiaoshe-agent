...
MAX_TOOL_ROUNDS = 60  # 默认给长任务足够空间；仍保留绝对兜底，且可由 XIAOSHE_MAX_TOOL_ROUNDS 调整
...
def _tool_round_limit() -> int:
    """本轮工具硬上限：默认 60，可配置 20~200；坏值安全回落默认值。"""
    raw = os.getenv("XIAOSHE_MAX_TOOL_ROUNDS")
    if raw is None:
...
def _publish_tool_round(ctx: dict, used: int, limit: int, status: str) -> None:
    """把纯计数状态发布给 UI；观测失败绝不阻断 agent 主循环。"""
    if not isinstance(ctx, dict) or not _top_level(ctx):
        return
    ctx["_tool_round"] = {"used": max(0, used), "limit": limit,
                          "remaining": max(0, limit - used), "status": status}
    try:
        ui_bus.mark_dirty(ctx, "tool_round")
        ui_bus.flush(ctx)
    except Exception:
        pass
...
def _append_tool_result(history: list, tc_id: str, name: str, content: str,
    ...
    history.append(_bind_context_message(
        {"role": "tool", "tool_call_id": tc_id, "content": _wrap_tool_data(content, ctx)}, ctx,
        kind="recent_error" if is_error else "tool_result",
    ))
    log_turn(
...
def _drain_steering(ctx: dict, history: list, log_file: Path) -> None:
    ...
    for msg in hub.drain_steering():
        ...
        if not msg:
            continue
        text = f"[用户插话] {msg}"
        history.append(_bind_context_message({"role": "user", "content": text}, ctx,
...
def _run_tool(name: str, args: dict, ctx: dict, approver, log_file: Path):
    ...
    if run_context is not None:
        try:
            ...
            decision = apply_mode(decision, run_context.policy_snapshot.get("mode", "observe"), name)
        except Exception:
    ...
    caller_action_id = ctx.get("_ui_call_id")
    try:
        from .effects import safe_action_id
        action_id = safe_action_id(caller_action_id)
    except Exception:
        action_id = None
    ...
    if fenced_effect:
        try:
            ...
            # This durable boundary must precede dispatch.  If the process is
            # killed after it, recovery cannot pretend a non-idempotent peer
            # request never left the process.
            effects_module.mark_task_effect_started(pending_effect_id, fence_held=True)
        except Exception:
    ...
    try:
        ...
        # A raised tool call may be a timeout after the remote peer acted.  Do
        # not turn that uncertainty into a verified failure.
        if fenced_effect:
            try:
                effects_module.complete_task_effect(
                    pending_effect_id, None, fence_held=True,
                    runtime_session=ctx.get("_runtime_session"),
                    runtime_event_mirror=ctx.get("_runtime_event_mirror"),
                )
            except Exception:
                # The durable start marker remains an unknown outcome even if
                # its completion write fails.  Preserve the original tool
                # exception and still publish the non-final runtime fact.
                pass
            finally:
                effect_fence.__exit__(None, None, None)
        if run_context is not None:
            try:
                run_context.emit_event("action.outcome_unknown", {
                    "task_id": run_context.task_id, "run_id": run_context.run_id,
                    "action_id": action_id, "tool": name,
                    "reason_code": "tool_response_unknown", "reconciliation_required": True,
                })
            except Exception:
                pass
        raise
    try:
        ...
        pass
    effect_completion_unknown = False
    try:
        if fenced_effect:
            effects_module.complete_task_effect(
        ...
        if fenced_effect:
            effect_completion_unknown = True
    ...
    if run_context is not None:
        try:
            if effect_completion_unknown:
                run_context.emit_event("action.outcome_unknown", {
                    "task_id": run_context.task_id, "run_id": run_context.run_id,
                    "action_id": action_id, "tool": name,
                    "reason_code": "effect_ledger_completion_unknown",
...
def _context_budget_requested(ctx: dict) -> bool:
    """Resolve the opt-in flag; explicit session state wins and bad env fails closed."""
    explicit = ctx.get("_context_budget_enabled")
    if type(explicit) is bool:
        return explicit
    return os.environ.get("XIAOSHE_CONTEXT_BUDGET", "off").strip().lower() == "on"


def _active_runtime_session(ctx: dict):
    """Resolve only the dynamically bound session; never reuse another request."""
    from .context_budget import ContextBudgetError
    from .runtime_factory import current_runtime_session
    from .runtime_session import RuntimeSession

    runtime_session = current_runtime_session()
    if not isinstance(runtime_session, RuntimeSession) or runtime_session.closed:
        raise ContextBudgetError("runtime_session_required")
    capabilities = getattr(runtime_session, "capability_snapshot", None)
    if capabilities is None:
        raise ContextBudgetError("capability_snapshot_required")
    return runtime_session


def _bind_context_message(message: dict, ctx: dict, *, kind: str = "active_task",
                          completed: bool = False) -> dict:
    """Bind one newly produced history item to the current request domain."""
    if not _context_budget_requested(ctx):
        return message
    if message.get("_runtime_session_id") is not None:
        return message
    runtime_session = _active_runtime_session(ctx)
    identity = runtime_session.identity
    return {
        **message,
        "_context_kind": kind,
        "_completed": completed,
        "_runtime_session_id": identity.session_id,
        "_task_id": identity.task_id,
        "_run_id": identity.run_id,
    }


def _close_previous_completed_round(history: list, ctx: dict) -> None:
    """Mark only a prior final-assistant boundary as closed; never infer by position."""
    if not _context_budget_requested(ctx):
        return
    final_index = next((index for index in range(len(history) - 1, -1, -1)
                        if isinstance(history[index], dict)
                        and history[index].get("role") == "assistant"
                        and not history[index].get("tool_calls")), None)
    if final_index is None:
        return
    identity = _active_runtime_session(ctx).identity
    start = next((index + 1 for index in range(final_index - 1, -1, -1)
                  if history[index].get("role") == "user"), 0)
    for message in history[start:final_index + 1]:
        if (message.get("role") in {"assistant", "user"}
                and message.get("_context_kind") == "active_task"
                and message.get("_runtime_session_id") == identity.session_id
                and message.get("_task_id") == identity.task_id
                and message.get("_run_id") == identity.run_id):
            message["_context_kind"] = "history"
            message["_completed"] = True


def _prepare_budgeted_send(history: list, ctx: dict) -> list[dict]:
    """Build one bounded send view from the current calibrated runtime facts."""
    from .context_budget import ContextBudgetError, allocate_context_budget
    from .prompt_prefix import build_stable_prefix

    runtime_session = _active_runtime_session(ctx)
    capabilities = runtime_session.capability_snapshot
    window = calibrate.effective_window(ctx)
    if type(window) is not int or window < 64:
        raise ContextBudgetError("invalid_context_window")
    image_tokens = ctx.get("_vision_last_tokens", 0)
    if type(image_tokens) is not int or image_tokens < 0:
        raise ContextBudgetError("invalid_image_token_estimate")
    reserved = max(16, window // 5)
    margin = max(8, window // 10)
    available_window = window - image_tokens
    if available_window < 64:
        raise ContextBudgetError("image_context_overflow")
    prefix_capacity = available_window - reserved - margin - 3
    if prefix_capacity <= 0:
        raise ContextBudgetError("image_context_overflow" if image_tokens else
                                 "stable_prefix_exceeds_context_window")
    try:
        prefix = build_stable_prefix(
            runtime_session,
            capabilities,
            max_tokens=prefix_capacity,
            max_bytes=min(131_072, max(1024, prefix_capacity * 8)),
        )
        budget = allocate_context_budget(
            available_window,
            reserved_output_tokens=reserved,
            stable_prefix_tokens=prefix.token_count,
            estimation_error_tokens=margin,
        )
    except (TypeError, ValueError) as error:
        raise ContextBudgetError("stable_prefix_exceeds_context_window") from error
    if (prefix.token_count > budget.stable_prefix_tokens
            or prefix.token_count + budget.reserved_output_tokens + image_tokens > window):
        raise ContextBudgetError("stable_prefix_exceeds_context_window")
    send_history, selection = compaction.prepare_budgeted_context(
        history, budget, session=runtime_session,
    )
    if selection.overflow_tokens:
        raise ContextBudgetError("protected_context_overflow", selection.source_ids)
    result = [{"role": "system", "content": prefix.text}, *send_history]
    if (tokens.estimate_messages(result) + image_tokens
            + budget.reserved_output_tokens > window):
        raise ContextBudgetError("estimated_context_window_exceeded", selection.source_ids)
    ctx["_context_budget"] = budget
    ctx["_stable_prefix_hash"] = prefix.digest
    ctx["_stable_prefix_tokens"] = prefix.token_count
    ctx["_context_selection"] = selection
    ctx["_vision_selected_tokens"] = image_tokens
    return result
...
def _send(model_fn, history: list, ctx: dict, summarizer, tools):
    ...
    """
    attempts = 0
    budget_mode = _context_budget_requested(ctx)
    wired_history = None
    if budget_mode:
        noted_history = notes.wire(history, ctx)
        if len(noted_history) > len(history):
            noted_history = list(noted_history[:len(history)]) + [
                _bind_context_message(message, ctx, kind="evidence")
                for message in noted_history[len(history):]
            ]
        wired_history = vision.wire_references(noted_history, ctx)
        if len(wired_history) > len(noted_history):
            wired_history = list(wired_history[:len(noted_history)]) + [
                _bind_context_message(message, ctx, kind="image_reference")
                for message in wired_history[len(noted_history):]
            ]
    while True:
        try:
            # 发送时注入（都只改临时副本、不动真 history）：先拼工作笔记（跨压缩存活），再拼待发图（最靠尾=最新）
            if budget_mode:
                send_history = _prepare_budgeted_send(wired_history, ctx)
                return model_fn(vision.materialize_references(send_history, ctx), tools=tools)
            return model_fn(vision.wire(notes.wire(history, ctx), ctx), tools=tools)
            ...
            if budget_mode:
                attempts += 1
                _io.warn(f"[!] provider 上下文超限；已按真窗口 {window} 重新预算安全上下文"
                         f"（第 {attempts} 次），持久 history 与附件引用未改写。")
                continue
...
def run_once(user_text: str, history: list, model_fn=kimi_chat, approver=_default_approver,
    ...
    try:
        ...
        if (not _context_budget_requested(ctx)
                and compaction.should_clear(history, anchor, calibrate.trigger_budget(ctx))):
        ...
        if not _context_budget_requested(ctx):
            _observe_compaction(history,
                                ...
                                log_file, ctx)
        _close_previous_completed_round(history, ctx)
        history.append(_bind_context_message({"role": "user", "content": user_text}, ctx,
                                             kind="user_constraint"))
        ...
        rounds = 0
        round_limit = _tool_round_limit()
        _publish_tool_round(ctx, rounds, round_limit, "running")
        ...
        while True:
            ...
            assistant_msg = _bind_context_message(
                assistant_msg, ctx, kind="tool_result" if tool_calls else "active_task",
            )
            ...
            if rounds > round_limit:
                ...
                _publish_tool_round(ctx, round_limit, round_limit, "limit")
            ...
            for index, tc in enumerate(tool_calls):
                ...
                if _ce is not None and _ce.is_set():
                    # 用户可在一批工具执行期间停止：已完成的保留，未执行的补齐
                    # tool result，避免会话回放留下未配对 tool_call。
                    _finalize_dangling(history, tool_calls[index:], "（用户已取消，未执行）", log_file, ctx)
                    _publish_tool_round(ctx, rounds, round_limit, "cancelled")
                    return content or "（已取消）"
            ...
            _ce = ctx.get("_cancel_event")
            if _ce is not None and _ce.is_set():
                # 审批等待期点了停止时，_handle_tool_call 会先以拒绝结案；
                # 此处必须在压缩/停滞检查/下一次模型请求前立即收工。
                _publish_tool_round(ctx, rounds, round_limit, "cancelled")
                return content or "（已取消）"
            ...
            if (_inloop_delta > _INLOOP_COMPACT_DELTA
                    and not _context_budget_requested(ctx)):
                _inloop_delta = 0
            ...
            if stall == STALL_LIMIT and not nudged:
                ...
                history.append(_bind_context_message({"role": "user", "content": rem}, ctx,
                                                     kind="user_constraint"))
            ...
            remaining = round_limit - rounds
            if 0 < remaining <= ROUNDS_REMIND_AHEAD and not rounds_reminded:
                rounds_reminded = True
                rem = (f"[系统提醒] 工具调用还剩 {remaining} 轮就到硬上限（共 {round_limit} 轮）"
...
def repl() -> None:
    ...
    if not config.API_KEY:
        print(f"[!] 没读到 KIMI_API_KEY——请确认 {config.ENV_PATH} 里填了 key。")
        return
