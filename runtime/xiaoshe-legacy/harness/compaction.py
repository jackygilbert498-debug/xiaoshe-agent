...
from .context_budget import (
    ContextBudget, ContextBudgetError, ContextItem, ContextSelection,
    canonical_public_json, select_context,
)
...
def prepare_budgeted_context(
    history: list,
    budget: ContextBudget,
    *,
    session=None,
) -> tuple[list[dict], ContextSelection]:
    """Build a non-mutating, source-traceable view for the budgeted path.

    Legacy messages are not assumed complete.  Only messages carrying the
    explicit ``_completed is True`` marker may be summarized; current user,
    tool, approval, error and evidence messages are protected categories.
    Internal bookkeeping fields and private model deliberation are never sent.
    """
    if not isinstance(history, list):
        raise TypeError("history must be a list")
    latest_user = max((index for index, msg in enumerate(history)
                       if isinstance(msg, dict) and msg.get("role") == "user"), default=-1)
    trusted_system_prefix = True
    ...
    for index, message in enumerate(history):
        ...
        if role != "system":
            trusted_system_prefix = False
        marker = message.get("_completed")
        completed = marker is True
        explicit_kind = message.get("_context_kind")
        if isinstance(explicit_kind, str) and explicit_kind:
            kind = explicit_kind
        elif role == "system":
            kind = "system"
        elif role == "tool" or (role == "assistant" and message.get("tool_calls")):
            kind = "tool_result"
        elif role == "user" and index == latest_user:
        ...
        public = {
            ...
            if not key.startswith("_") and key not in {
                "reasoning", "thinking", "chain_of_thought", "private_reasoning",
            }
        }
        protected = kind in {
            "active_task", "user_constraint", "permission_decision", "pending_approval",
            "recent_error", "tool_result", "evidence", "image_reference",
        }
        safe_public, canonical_public = canonical_public_json(public, protected=protected)
        source_id = message.get("_source_id")
        if not isinstance(source_id, str) or not source_id:
            digest = hashlib.sha256(canonical_public.encode("utf-8")).hexdigest()[:16]
            source_id = f"msg:{index}:{digest}"
        runtime_session_id = message.get("_runtime_session_id")
        task_id = message.get("_task_id")
        run_id = message.get("_run_id")
        if session is not None:
            identity = getattr(session, "identity", None)
            if identity is None:
                raise ContextBudgetError("runtime_session_required")
            if runtime_session_id is None and trusted_system_prefix and role == "system":
                runtime_session_id = identity.session_id
                task_id = identity.task_id
                run_id = identity.run_id
            elif (not isinstance(runtime_session_id, str) or not runtime_session_id
                  or runtime_session_id != identity.session_id
                  or task_id != identity.task_id or run_id != identity.run_id):
                code = "unbound_context_item" if runtime_session_id is None else "context_domain_mismatch"
                raise ContextBudgetError(code, (source_id,))
        items.append(ContextItem(
            source_id, kind, safe_public, completed=completed,
            task_id=task_id if isinstance(task_id, str) else None,
            run_id=run_id if isinstance(run_id, str) else None,
            session_id=runtime_session_id if isinstance(runtime_session_id, str) else None,
            sequence=index,
        ))
    selection = select_context(items, budget, session=session)
    selected_by_id = {item.source_id: item for item in selection.items}
    output: list[dict] = []
    ...
    if selection.summaries:
        summary_message = {
            "role": "system",
            "content": "\n".join(summary.text for summary in selection.summaries),
        }
    for item in items:
        selected = selected_by_id.get(item.source_id)
        if selected is not None:
            if summary_message is not None and not inserted_summary and selected.kind != "system":
                output.append(summary_message)
                inserted_summary = True
            if isinstance(selected.content, dict):
                output.append(dict(selected.content))
