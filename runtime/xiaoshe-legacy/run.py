...
from harness.headless import run_doctor, run_headless
from harness.runtime_adapters import route_cli_runtime
from harness.runtime_closure import RuntimeClosureError
from harness.runtime_session import RuntimeIdentity


def _closure_denied(error: RuntimeClosureError) -> int:
    report = None
    if error.report is not None:
        try:
            report = error.report.public_dict()
        except ValueError:
            report = None
    print(json.dumps(
        {"code": error.code, "closure": report},
        ensure_ascii=False,
        sort_keys=True,
    ), file=sys.stderr)
    return 1
...
def main() -> int:
    ...
    if any(value is not None for value in bound):
        ...
        from harness.runtime_session import RuntimeActivationSnapshot
        from harness.schedule import activate_schedule_trigger_closure
        ...
        try:
            ...
            if activation.closure_mode != "off":
                closure_session = activate_schedule_trigger_closure(
                    ...
                    activation,
                    legacy_route="headless.task_trigger",
                )
            tasking_mode = (
                config.tasking_mode()
                if activation.closure_mode == "off"
                else activation.tasking_mode
            )
            if tasking_mode != "on":
    ...
    try:
        route_cli_runtime(
            RuntimeIdentity(f"runtime-{uuid.uuid4().hex}", "cli"),
            "",
            lambda _value: repl(),
        )
    except RuntimeClosureError as error:
