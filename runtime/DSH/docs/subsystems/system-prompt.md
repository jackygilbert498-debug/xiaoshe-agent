...
<a id="system-promptfinalized--waterfall"></a>

#### `system-prompt/finalized` — waterfall

Observe or reject the exact assembly returned to the agent after complete/suppression enforcement.

```ts cordis-catalog
/**
 * Observe or reject the exact assembly returned to the agent after
 * complete/suppression enforcement.
 * @param assembly - the final enforced assembly about to reach the agent.
 * @param context - the caller's per-assembly context.
 * @mode waterfall
 */
'system-prompt/finalized'(this: Scoped<SystemPrompt>, assembly: PromptAssembly, context: AssembleContext, next: () => Promise<PromptAssembly>): Promise<PromptAssembly>
```
