...
export class DeepSeekSearchProvider implements WebSearchProvider {
   ...
   * @param transport - optional in-process HTTP dependency for transport-level
   * observation/testing; not a Profile setting. Omission retains native fetch.
   */
  constructor(
    private readonly resolveOptions: () => DeepSeekSearchProviderOptions,
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    try {
      response = await (this.transport ?? fetch)(endpoint, {
        method: 'POST',
        redirect: 'error',
        headers: {
