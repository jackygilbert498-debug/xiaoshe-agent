...
import { resolveWithTrustedDoh } from './trusted-doh.ts'
...
export async function resolvePublicAddresses(
  ...
  if (normalized === 'localhost' || normalized.endsWith('.localhost')) {
    throw new WebError('localhost is not a public web destination', 'WEB_BLOCKED_URL')
  }
  ...
  // Only a complete OS synthetic set permits trusted re-resolution. A literal,
  // mixed answer, private result, or malformed family never gains an exception.
  if (literalFamily === 0 && resolved.every(entry => entry.family === 4 && isSyntheticResolverAddress(entry.address))) {
    resolved = [...await raceWithSignal(resolveWithTrustedDoh(unbracketed, signal), signal)]
    if (resolved.length === 0) throw new WebError('trusted DNS resolved to no addresses', 'WEB_PROVIDER_ERROR')
  }
...
/** RFC 2544 fake-IP resolver range, never itself a fetch destination. */
export function isSyntheticResolverAddress(input: string): boolean {
...
/**
 * Use the operator's proxy without letting proxy-side DNS bypass public-address
 * admission. Each attempt owns its dispatcher, and cancellation destroys it so
 * a failed CONNECT/TLS handshake cannot keep a task alive after its deadline.
 */
export async function requestViaPinned(
  proxy: string,
  url: URL,
  addresses: readonly PublicAddress[],
  headers: Record<string, string>,
  ...
  for (const address of addresses) {
    ...
    if (!isPublicIpAddress(address.address)) throw new WebError('proxy destination is not public', 'WEB_BLOCKED_URL')
    const pinned = new URL(url)
    pinned.hostname = address.family === 6 ? `[${address.address}]` : address.address
    // proxy-exempt: per-request, IP-pinned transport through the selected policy proxy.
    // Fetch removes caller Host headers. Restore the logical authority at the
    // dispatcher boundary while keeping the connection/CONNECT origin pinned.
    const dispatcher = new class extends ProxyAgent {
      override dispatch(options: Dispatcher.DispatchOptions, handler: Dispatcher.DispatchHandler): boolean {
        const retainedHeaders = Array.isArray(options.headers)
          ? [...options.headers, 'host', url.host]
          : { ...options.headers, host: url.host }
        return super.dispatch({ ...options, headers: retainedHeaders }, handler)
      }
    }({
      uri: proxy,
      proxyTunnel: url.protocol === 'https:',
      // Pass cancellation into TLS itself: an in-progress handshake has not
      // entered the agent pool yet and dispatcher.destroy() alone cannot stop it.
      requestTls: { servername: stripIpv6Brackets(url.hostname), rejectUnauthorized: true, signal },
    ...
    try {
      const response = await fetch(pinned, {
        ...
        headers: { ...headers, host: url.host }, signal, dispatcher,
      })
  ...
  throw lastError ?? new WebError('no public proxy destination', 'WEB_PROVIDER_ERROR')
}
