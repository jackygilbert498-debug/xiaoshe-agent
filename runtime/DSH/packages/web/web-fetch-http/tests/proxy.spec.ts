...
describe('fetching through a proxy', () => {
  it('pins the approved public address even through a configured proxy', async () => {
    ...
    expect(proxied).toEqual(['http://8.8.8.8/page'])
  ...
  it('does not hand a private DNS answer to the proxy', async () => {
    disposeProxy = await installProxy()
    const { WebError } = await import('@deepseek-ai/dsh-web')
    vi.spyOn(publicHttpNetwork, 'resolve').mockRejectedValue(new WebError('private DNS answer', 'WEB_BLOCKED_URL'))
    await expect(new HttpFetchProvider(limits).fetch({ url: proxyTarget }))
      .rejects.toThrow(expect.objectContaining({ code: 'WEB_BLOCKED_URL' }))
    expect(proxied).toEqual([])
  })
