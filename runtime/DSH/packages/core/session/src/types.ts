...
export interface SessionEventMap {
  ...
  /**
   * Independent post-mutation verification linked to one exact effect-capable
   * call and one exact verifier call. Producers append this only after the
   * verifier's own durable result; mutation-owned metadata is never proof.
   */
  'verification/result': {
    turn: number
    mutationCallId: ToolCallId
    verifierCallId: ToolCallId
    gate:
      | 'typecheck'
      | 'test'
      | 'build'
    ...
    status: 'passed' | 'failed' | 'skipped' | 'not-run' | 'blocked' | 'not-applicable'
    evidence?: string
  }
  /**
   * Xiaoshe's durable direct-goal boundary. Consumers use the generation to
   * keep a late tool result from an older task out of the active task receipt.
   */
  'xiaoshe/task-generation': {
    version: 1
    generation: number
   ...
   * The marker never duplicates fetched content: the referenced `tool/result`
   * remains authoritative, while this event records the generation/turn/call
   * relationship needed to rebuild the evidence ledger after a cold reload.
   */
  'xiaoshe/research-evidence': {
    version: 1
    generation: number
    turn: number
    kind: 'body'
  ...
  /**
   * Latest append-only state for one ordered-read obligation in a task
   * generation. `primary` + `fallback` is the obligation identity.
   */
  'xiaoshe/obligation-state': {
    version: 1
    generation: number
    turn: number
    kind: 'ordered-read'
    status: 'pending' | 'blocked' | 'satisfied'
    primary: string
    fallback: string
    reason?: string
  } | {
    ...
    turn: number
    kind: 'research'
    status: 'pending' | 'blocked' | 'bounded-partial' | 'satisfied'
    reason?: 'no-source' | 'body-missing' | 'citation-missing' | 'stale-only'
    sourceResultSeqs: number[]
    bodyResultSeqs: number[]
    citedBodyResultSeqs: number[]
  } | {
    /** Reserved projection contract for product-owned cross-route recovery. */
    version: 1
    generation: number
    turn: number
    kind: 'route-recovery'
    status: 'needs-alternative' | 'needs-proof' | 'satisfied' | 'blocked'
    failedFamily: string
    alternativeFamily?: string
...
/** Optional envelope metadata for log-only plugin events; never valid on surface messages. */
