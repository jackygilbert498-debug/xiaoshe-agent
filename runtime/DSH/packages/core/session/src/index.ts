...
export class Session {
  ...
  append<T extends SessionEventType>(
    ...
    const appendOpts = opts[0] as (SurfaceIntent & LogOnlyAppendOptions) | undefined
    // A marker is an explicit compatibility promise for optional log records,
    // never permission to silently drop model-facing history on older readers.
    if (appendOpts?.ignorable !== undefined
      && (appendOpts.ignorable !== true || isSurfaceEligibleType(type))) {
      throw new Error('ignorable: true is valid only for log-only session events')
    }
    const surfaceOpts: SurfaceIntent | undefined = appendOpts
