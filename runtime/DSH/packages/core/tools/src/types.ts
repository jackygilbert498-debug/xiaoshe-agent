...
export interface PtcDispatchEventData extends PtcDispatchStartEventData {
  ...
  /**
   * Tool-authored replay projection. This is the same bounded, lossless JSON
   * produced by `output.presentationMeta`; it never copies the raw result
   * value. In particular shell tools persist process status here without
   * duplicating stdout/stderr or command environment data.
   */
  meta?: JsonValue
}
...
declare module '@deepseek-ai/dsh-session/types' {
  interface SessionEventMap {
     ...
     * (`content` + `isError` + optional replay-safe `meta`), so UIs render a sub-call through the exact
