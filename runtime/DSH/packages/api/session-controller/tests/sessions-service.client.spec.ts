...
describe('fork', () => {
  it('forwards the workspace identity without inventing the target child cwd', async () => {
    const b = bench()
    await feedList(b, [{ id: 'source', cwd: '/source' }])
    b.api.onFork = () => Promise.resolve(ok({ sessionId: sid('target-child') }))
    await b.svc.fork({ sessionId: sid('source'), workspaceId: 'target' as never })
    expect(b.api.callsOf('session.fork')).toEqual([{ sessionId: 'source', workspaceId: 'target' }])
    expect(b.svc.list.getSnapshot().byId[sid('target-child')]?.cwd).toBeUndefined()
    expect(b.svc.list.getSnapshot().byId[sid('source')]?.cwd).toBe('/source')
    await b.ctx.fiber.dispose()
  })
  it.each([
    ['Roadmap', 'Roadmap (1)'],
