...
describe('Session fork failures', () => {
  it('forks into a registered workspace without mutating the source or accepting arbitrary cwd', async () => {
    const ctx = await baseContext()
    const source = completedSession(ctx, 'explicit-workspace-source', '/source')
    const original = source.snapshotEvents()
    const attached: SessionId[] = []
    const workspace = { id: 'target' as WorkspaceId, path: '/trusted-target', sessionIds: attached, attachSession: async (id: SessionId) => { attached.push(id) } }
    ctx.provide('workspaceRegistry', { get: (id: string) => id === 'target' ? workspace : undefined, list: () => [] } as never)
    const create = vi.spyOn(ctx.agents, 'create').mockImplementation(options => Promise.resolve(resolvedHandle(ctx, options.sessionId)))
    const controller = new SessionCommandController(ctx, controllerAgents(), '/default')
    const child = await controller.fork({ sessionId: source.id, workspaceId: workspace.id })
    expect(create.mock.calls[0]?.[0]).toMatchObject({ meta: { cwd: '/trusted-target', parentSession: source.id }, seed: original, inheritedEventCount: original.length })
    expect(attached).toEqual([child.sessionId])
    expect(source.header.cwd).toBe('/source')
    expect(source.snapshotEvents()).toEqual(original)
    await expect(controller.fork({ sessionId: source.id, workspaceId: 'unknown' as WorkspaceId })).rejects.toMatchObject({ code: 'workspace/not-found' })
    await expect(controller.fork({ sessionId: source.id, targetCwd: '/untrusted' } as never)).rejects.toMatchObject({ code: 'gateway/bad-request' })
