...
import SessionStore, { SessionId as makeSessionId } from '@deepseek-ai/dsh-session'
...
describe('Web session model selection', () => {
  it('sends the changed effort to the real next AgentLoop request while preserving the running stream', async () => {
    const { ctx } = await harness()
    const entered = Promise.withResolvers<void>()
    const release = Promise.withResolvers<void>()
    // This recorder always stores the key, including an omitted provider effort.
    const captured: { provider: string; model: string; reasoningEffort: GenerateOptions['reasoningEffort'] }[] = []
    ctx.llm.registerAdapter(['capture'], new class extends CatalogAdapter {
      override async *stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
        captured.push({ provider: options.provider, model: options.model, reasoningEffort: options.reasoningEffort })
        if (captured.length === 1) { entered.resolve(); await release.promise }
        yield* textResponse('done')
      }
    }('Capture', [], REASONING))
    const remote = createSessionTestRemote(ctx, { defaultModelSelection: () => ({ provider: 'capture', model: 'm' }), cwd: '/tmp' })
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(AgentLoop, { agents: [] })
    const agent = await ctx.agentLoop.create(makeSessionId('running-effort-capture'), { provider: 'capture', model: 'm' })
    try {
      expectValue(await remote.selectModel({ sessionId: agent.id, provider: 'capture', model: 'm', reasoningEffort: 'high' }))
      expectValue(await remote.prompt(promptRequest({ sessionId: agent.id, mode: 'queue', content: [{ type: 'text', text: 'first' }] })))
      await entered.promise
      expect(agent.status).toBe('running')
      expect(await remote.selectModel({ sessionId: agent.id, provider: 'capture', model: 'm', reasoningEffort: 'max' })).toMatchObject({ ok: true, value: { effective: 'next-request' } })
      expectValue(await remote.prompt(promptRequest({ sessionId: agent.id, mode: 'queue', content: [{ type: 'text', text: 'second' }] })))
      release.resolve()
      await agent.whenIdle()
      expect(captured).toEqual([
        { provider: 'capture', model: 'm', reasoningEffort: 'high' },
        { provider: 'capture', model: 'm', reasoningEffort: 'max' },
      ])
    } finally { release.resolve(); await ctx.fiber.dispose() }
  })

  it('accepts running effort changes only on the current route at the next assembly boundary', async () => {
    ...
    try {
      expectValue(await remote.selectModel({ sessionId, provider: 'deepseek-official', model: 'deepseek-chat', reasoningEffort: 'high' }))
      await ctx.systemPrompt.assemble()
      Object.assign(agent, { status: 'running' })
      const signal = new AbortController().signal
      const capture = () => agentEvents(ctx, agent).waterfall('agent/request', { turn: 1, step: 1, signal }, () => Promise.resolve({ provider: 'seed', model: 'seed' }))
      expect(await capture()).toEqual({ provider: 'deepseek-official', model: 'deepseek-chat', reasoningEffort: 'high' })
      const accepted = await remote.selectModel({ sessionId, provider: 'deepseek-official', model: 'deepseek-chat', reasoningEffort: 'max' })
      expect(accepted).toMatchObject({ ok: true, value: { effective: 'next-request' } })
      // Already assembled/in-flight inference remains immutable.
      expect(await capture()).toEqual({ provider: 'deepseek-official', model: 'deepseek-chat', reasoningEffort: 'high' })
      const before = agent.session.snapshotEvents().length
      for (const selection of [
        { provider: 'deepseek-official', model: 'deepseek-chat', reasoningEffort: 'unsupported' },
        { provider: 'deepseek-official', model: 'deepseek-reasoner', reasoningEffort: 'max' },
        { provider: 'other', model: 'deepseek-chat', reasoningEffort: 'max' },
      ]) expect((await remote.selectModel({ sessionId, ...selection })).ok).toBe(false)
      expect(agent.session.snapshotEvents()).toHaveLength(before)
      await ctx.systemPrompt.assemble()
      expect(await capture()).toEqual({ provider: 'deepseek-official', model: 'deepseek-chat', reasoningEffort: 'max' })
    } finally { await ctx.fiber.dispose() }
  })

  it('holds the real Agent idle phase while resolving a selection and queues a concurrent prompt', async () => {
    const { ctx } = await harness()
    const remote = createSessionTestRemote(ctx, { defaultModelSelection: () => ({ provider: 'deepseek-official', model: 'deepseek-chat' }), cwd: '/tmp' })
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(AgentLoop, { agents: [] })
    const agent = await ctx.agentLoop.create(makeSessionId('maintenance-selection'), { provider: 'deepseek-official', model: 'deepseek-chat' })
    const resolving = Promise.withResolvers<void>()
    const release = Promise.withResolvers<void>()
    const original = ctx.llm.resolveCallConfig.bind(ctx.llm)
    vi.spyOn(ctx.llm, 'resolveCallConfig').mockImplementationOnce(async (request, signal) => {
      resolving.resolve(); await release.promise
      return original(request, signal)
    })
    const selected = remote.selectModel({ sessionId: agent.id, provider: 'deepseek-official', model: 'deepseek-reasoner' })
    await resolving.promise
    expect(() => agent.runMaintenance(async () => {})).toThrow('already has active work')
    expectValue(await remote.prompt(promptRequest({ sessionId: agent.id, mode: 'queue', content: [{ type: 'text', text: 'queued' }] })))
    expect(agent.session.snapshotEvents().some(event => event.type === 'turn/start')).toBe(false)
    // Remove the queued fixture before releasing; no provider stream is needed to prove admission ordering.
    agent.inbox.clear()
    release.resolve()
    expectValue(await selected)
    expect(agent.session.snapshotEvents().some(event => event.type === 'model/selection')).toBe(true)
    await agent.whenIdle()
    await ctx.fiber.dispose()
  })

  it('returns session-only when the persistence service explicitly reports no durable write', async () => {
    const { ctx, sessionId } = await harness()
    const remote = createSessionTestRemote(ctx, { defaultModelSelection: () => ({ provider: 'deepseek-official', model: 'deepseek-chat' }), cwd: '/tmp' })
    vi.spyOn(ctx.agentDefaultModel, 'saveSelection').mockResolvedValue(false)
    expect(expectValue(await remote.selectModel({ sessionId, provider: 'deepseek-official', model: 'deepseek-reasoner' })).persistence.status).toBe('session-only')
    await ctx.fiber.dispose()
  })
  it('rejects an open turn before model resolution or selection mutation', async () => {
    const { ctx, agent, sessionId } = await harness()
    Object.assign(agent, { status: 'running' })
    const remote = createSessionTestRemote(ctx, { defaultModelSelection: () => ({ provider: 'deepseek-official', model: 'deepseek-chat' }), cwd: '/tmp' })
    const resolve = vi.spyOn(ctx.llm, 'resolveCallConfig')
    expect(await remote.selectModel({ sessionId, provider: 'deepseek-official', model: 'deepseek-reasoner' })).toMatchObject({ ok: false, error: { code: 'session/agent-busy', details: { reason: 'open-turn' } } })
  ...
  it('rejects a text-only selection for active or pending images but permits compacted-away images', async () => {
    const { ctx, agent, sessionId } = await harness()
    ...
    expect(expectValue(await remote.selectModel({ sessionId, provider: 'text-only', model: 'plain' })).selected).toEqual({ provider: 'text-only', model: 'plain' })
    save.mockClear()
    ...
    ;(agent.inbox.nextStep as UserMessage[]).push((agent.inbox.nextTurn as UserMessage[]).pop()!)
    expect(await remote.selectModel({ sessionId, provider: 'text-only', model: 'plain' })).toMatchObject({ ok: false, error: { code: 'session/model-unavailable' } })
    expect(save).not.toHaveBeenCalled()
    await ctx.fiber.dispose()
  })
  ...
  it('saves an accepted selection as the default and survives a storage failure', async () => {
    ...
    const persisted = expectValue(await remote.selectModel(request({
      sessionId, provider: 'deepseek-official', model: 'deepseek-reasoner', reasoningEffort: 'max',
    })))
    expect(persisted.persistence).toEqual({ status: 'saved' })
    ...
    expect(stillAccepted.persistence).toEqual({ status: 'session-only', warning: '模型已应用于当前会话，但未能保存为新会话默认值。' })
    expect(JSON.stringify(stillAccepted)).not.toContain('read-only document')
    expect(currentSelection(ctx, sessionId))
