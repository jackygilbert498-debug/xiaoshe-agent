...
import {
  ReasoningEffortId, assistantStreamChunks, contentHasImage, createUserMessage, freezeMessage,
...
export class SessionCommandController {
  ...
  async selectModel(request: SessionSelectModelRequest): Promise<SessionSelectModelValue> {
    ...
    const assertRunningRoute = (): void => {
      const current = this.agents.selectionFor(agent).current
      if (agent.status === 'running' && (current.provider !== request.provider || current.model !== request.model)) throw selectionBusy()
    }
    assertRunningRoute()
    const applySelection = async (signal: AbortSignal): Promise<SessionSelectModelValue> => {
      try {
        ...
        // Validate the active surface, not every historical event: compaction
        // can remove an image from the actual input. Queued images still count.
        const images = [...agent.inbox.nextTurn, ...agent.inbox.nextStep, ...agent.session.deriveMessages()]
          .some(message => contentHasImage(message.content))
        if (images) {
          const info = await this.ctx.llm.resolveModelInfo(resolved.provider, resolved.model)
          if (info.inputModalities !== undefined && !info.inputModalities.includes('image')) {
            throw new RemoteError('session/model-unavailable', 'The selected model does not support images already present in this session.', { provider: request.provider, model: request.model })
          }
        }
        ...
        let persistence: SessionSelectModelValue['persistence'] = {
          status: 'session-only', warning: '模型已应用于当前会话，但未能保存为新会话默认值。',
        }
        try {
          const defaults = this.ctx.get('agentDefaultModel')
          if (defaults !== undefined && await defaults.saveSelection(selected) !== false) persistence = { status: 'saved' }
        } catch {
          this.ctx.logger.warn(
            'session-controller: model selection changed for the Session but the default was not saved',
          )
        }
        return { selected: { ...selected }, persistence, effective: 'next-request' }
      } catch (error) {
    ...
    return this.agents.serializeImageAdmission(agent, () => {
      // Recheck after admission waits. A running request owns its immutable
      // assembly snapshot; changing the pending effort affects later assembly.
      assertRunningRoute()
      return agent.status === 'running'
        ? applySelection(new AbortController().signal)
        : claimIdleSelection(agent, applySelection)
    })
  }
  ...
  async fork(request: SessionForkRequest): Promise<SessionForkValue> {
    if ('targetCwd' in request || 'cwd' in request) {
      throw new RemoteError('gateway/bad-request', 'session.fork accepts a registered workspaceId, not a destination path', {})
    }
    ...
    try {
      ...
      if (request.workspaceId !== undefined && workspace === undefined) {
        throw new RemoteError('workspace/not-found', 'fork workspace not found', { workspaceId: request.workspaceId })
      }
    } catch (error) {
      if (remoteErrorOf(error) !== undefined) throw error
      throw new RemoteError(
    ...
    try {
      ...
      await this.ctx.agents.create({
        ...
        meta: {
          ...((workspace?.path ?? source.header.cwd) === undefined ? {} : { cwd: workspace?.path ?? source.header.cwd }),
...
/** Preserve the old idle-phase claim; the image-admission chain alone is not an Agent lock. */
function assertIdleSelection(agent: Agent): void {
  if (agent.status === 'running') throw selectionBusy()
}

function selectionBusy(): RemoteError {
  return new RemoteError('session/agent-busy', 'model selection is unavailable while the session is running', { reason: 'open-turn' })
}

function claimIdleSelection<T>(agent: Agent, operation: (signal: AbortSignal) => Promise<T>): Promise<T> {
  assertIdleSelection(agent)
  try { return agent.runMaintenance(operation) } catch { throw selectionBusy() }
}
