...
function namespaceView(descriptor: SettingsDescriptor): SettingsNamespaceView {
  return {
    ...
    status: descriptor.status ?? 'degraded',
    error: descriptor.status === 'ready' ? null
      : 'Stored settings are invalid or their health is unavailable; a safe fallback remains read-only.',
  }
}
