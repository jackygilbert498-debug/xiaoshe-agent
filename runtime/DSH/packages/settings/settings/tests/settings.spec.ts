...
describe('registration', () => {
  ...
  it('refuses a write its owner could not act on, and keeps the last good value for a stored one', async () => {
    ...
    // The retained Xiaoshe contract forbids merging a partial edit into a
    // degraded document. Only a provider-observed repair (or an explicitly
    // opted-in validated wholesale replacement) may make it writable again.
    await expect(ctx.settings.update(ns, { fontSize: 18 })).rejects.toThrow(/degraded/)
    expect(scope.get()).toEqual(before)
    ;(ctx.settings as unknown as { publish(doc: Record<string, unknown>): void })
      .publish({ 'ui-theme': { fontSize: 18 } })
    expect(scope.get()).toMatchObject({ fontSize: 18 })
    await ctx.settings.update(ns, { fontSize: 20 })
    expect(scope.get()).toMatchObject({ fontSize: 20 })
  })
...
describe('revision and conflict detection', () => {
  ...
  it('moves the revision past a stored section that was not an object', async () => {
    ...
    // Invalid external bytes also invalidate an old editor; both transitions
    // advance the revision, not just the later repair.
