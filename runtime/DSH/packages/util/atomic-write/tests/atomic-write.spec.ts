...
vi.mock('node:fs/promises', async (importOriginal) => {
  ...
  return {
    ...
    rename: (async (...args: Parameters<typeof actual.rename>) => {
      // Xiaoshe installs a non-empty ownership directory atomically. Inject
      // contention at that real boundary, not the retired lock-file write.
      if (state.failLockCreateWithEPERM && String(args[1]).endsWith('.lock')) {
        state.failLockCreateWithEPERM = false
        throw Object.assign(new Error('EPERM: injected lock-install failure'), { code: 'EPERM' })
      }
...
describe('writeFileAtomic', () => {
  ...
  it('replaces a symlinked target itself without writing through to the referent', async (context) => {
    const dir = await scratch()
    const victim = join(dir, 'victim')
    await writeFile(victim, 'victim-content')
    const target = join(dir, 'doc.yaml')
    try { await symlink(victim, target) } catch (error) {
      if (process.platform === 'win32' && (error as NodeJS.ErrnoException).code === 'EPERM') {
        context.skip('Windows user lacks file-symlink privilege; rerun on a symlink-capable host')
        return
      }
      throw error
    }
...
describe('withFileLock', () => {
  ...
  it('preserves EPERM when no lock path exists', async () => {
    ...
    expect(state.failLockCreateWithEPERM).toBe(false)
  })
