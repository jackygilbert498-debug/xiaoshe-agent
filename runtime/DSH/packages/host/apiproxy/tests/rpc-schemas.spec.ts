...
describe('sessions domain schemas', () => {
  ...
  it('validates the per-method request/value pairs', () => {
    ...
    // The scope is exactly one of Workspace, explicit cwd, or Host-owned loose storage.
    expect(sessionCreateRequestSchema.parse({ workspaceId: 'w1', sessionId: 's1' }).sessionId).toBe('s1')
    expect(sessionCreateRequestSchema.parse({ loose: true }).loose).toBe(true)
    expect(() => sessionCreateRequestSchema.parse({ workspaceId: 'w1', cwd: '/w' })).toThrow(/only one/)
    expect(() => sessionCreateRequestSchema.parse({ workspaceId: 'w1', loose: true })).toThrow(/only one/)
