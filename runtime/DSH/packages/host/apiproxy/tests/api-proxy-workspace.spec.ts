...
async function harness(
  ...
  const api = createApiProxy(ctx, {
    ...
    cwd: root,
    looseSessionsRoot: join(root, 'loose-conversations'),
...
describe('session creation and Workspace membership', () => {
  it('creates every projectless session in its own Host-owned directory', async () => {
    const { api, root } = await harness()
    const sessionId = SessionId('session-projectless')

    expectOk(await api.sessions.create(request({ loose: true, sessionId })))

    expect(existsSync(join(root, 'loose-conversations', sessionId))).toBe(true)
