...
export interface ApiProxyDefaults {
  ...
  cwd: string
  /** Xiaoshe-owned root for conversations that are not assigned to a project. */
  looseSessionsRoot?: string
  ...
  return {
    sessions: {
      ...
      async create(request) {
        ...
        const cwd = request.payload.loose === true
          ? join(defaults.looseSessionsRoot ?? join(homedir(), '.xiaoshe', 'conversations'), sessionId)
          : workspace?.path ?? request.payload.cwd ?? defaults.cwd
        const requestedPreset = request.payload.agentPreset
        try {
          if (request.payload.loose === true) await mkdir(cwd, { recursive: true })
      ...
      async fork(request) {
        ...
        try {
          workspace = workspaceId === undefined
            ...
            : ctx.workspaceRegistry.get(brandWorkspaceId(workspaceId))
        ...
        try {
          await ctx.agents.create({
            ...
            meta: {
              ...((workspace?.path ?? source.header.cwd) === undefined
                ? {}
                : { cwd: workspace?.path ?? source.header.cwd }),
