...
export const RELEASED_V0_EVENT_DISPOSITIONS: Readonly<Record<string, ReleasedV0PayloadDisposition>> = Object.freeze({
  // Product events already existed in V0. Keep a closed vocabulary and validate
  // each obligation variant separately; unknown historical events still refuse.
  'verification/result': disposition(['turn', 'mutationCallId', 'verifierCallId', 'gate', 'status'], ['evidence']),
  'xiaoshe/task-generation': disposition(['version', 'generation', 'relation', 'triggerMessageId']),
  'xiaoshe/research-evidence': disposition(['version', 'generation', 'turn', 'kind', 'callId'], ['url']),
  'xiaoshe/obligation-state': disposition(['version', 'generation', 'turn', 'kind', 'status'],
    ['primary', 'fallback', 'reason', 'sourceResultSeqs', 'bodyResultSeqs', 'citedBodyResultSeqs',
      'failedFamily', 'alternativeFamily', 'alternativeTool', 'toolContractDigest', 'presetId', 'proofResultSeq']),
