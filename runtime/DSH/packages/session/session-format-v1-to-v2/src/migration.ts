...
function remapPayloadReferences(
  ...
  const data = record(event.data)
  switch (event.type) {
    case 'xiaoshe/obligation-state': {
      // Chunk compaction changes coordinates, not opaque evidence strings.
      if (data['kind'] === 'research') return {
        ...data,
        ...Object.fromEntries(['sourceResultSeqs', 'bodyResultSeqs', 'citedBodyResultSeqs'].map(key =>
          [key, mapList(numberArray(data[key] as SessionFormatJsonValue), mapping, `${event.type} ${event.seq} ${key}`)])),
      }
      if (data['kind'] === 'route-recovery' && data['proofResultSeq'] !== undefined) return {
        ...data,
        proofResultSeq: mapOne(coordinate(data['proofResultSeq']), mapping, `${event.type} ${event.seq} proofResultSeq`),
      }
      return data
    }
    case 'command/done':
