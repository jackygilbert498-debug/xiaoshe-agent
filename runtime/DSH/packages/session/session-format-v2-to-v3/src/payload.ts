...
import { XIAOSHE_EVENT_TYPES, assertXiaosheEvent, assertDispatchMeta } from './xiaoshe-events.ts'
...
export function assertEvent(event: SessionFormatEvent, version: 2 | 3): void {
  ...
  const xiaoshe = XIAOSHE_EVENT_TYPES.has(event.type)
  if (disposition === undefined && !feedback && !xiaoshe) {
  ...
  if (xiaoshe) { assertXiaosheEvent(event); return }
  if (feedback) {
  ...
  keys(data, admitted.required, event.type === 'tool/code-dispatch' ? [...admitted.optional, 'meta'] : admitted.optional, event.type + ' data')
  if (event.type === 'tool/code-dispatch') assertDispatchMeta(data)
...
export function assertV3Event(event: SessionFormatEvent, knownEventTypes?: ReadonlySet<string>): void {
  ...
  const known = !obsolete && (SURFACE_TYPES.has(event.type)
    ...
    || XIAOSHE_EVENT_TYPES.has(event.type) || knownEventTypes?.has(event.type) === true)
  const opaque = !known
