  ...
  switch (event.type) {
    case 'xiaoshe/obligation-state':
      if (data['kind'] === 'research') {
        data = { ...data, sourceResultSeqs: list(data['sourceResultSeqs']), bodyResultSeqs: list(data['bodyResultSeqs']), citedBodyResultSeqs: list(data['citedBodyResultSeqs']) }
      } else if (data['kind'] === 'route-recovery' && data['proofResultSeq'] !== undefined) {
        data = { ...data, proofResultSeq: one(data['proofResultSeq']) }
      }
      break
    case 'command/done':
