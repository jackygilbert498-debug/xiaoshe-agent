...
export class SettingsScopeController<T> implements SettingsScope<T> {
  ...
  private derive(): void {
    ...
    const status = view.status ?? 'degraded'
    this.store.update((draft) => {
      draft.revision = view.revision
      draft.base = view.base
      draft.user = view.user
      draft.writable = writable && status === 'ready'
      draft.error = status === 'degraded'
        ? view.error ?? 'Stored settings health is unavailable; the last safe value remains read-only.'
        : null
      if (status === 'degraded') draft.status = 'degraded'
