...
describe('root outlet', () => {
  ...
  it('renders an actionable recovery face when the selected root entry crashes', () => {
    const h = makeHost()
    h.add('root', { component: () => { throw new Error('root boom') } })
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {})
    ...
    expect(alert?.getAttribute('data-slot-error')).toBe('root')
    expect(alert?.textContent).toContain('界面暂时无法显示')
    expect(view.container.querySelector('button')?.textContent).toBe('重新加载')
  })
})
...
describe('child outlets and the renderSlot binding', () => {
  ...
  it('isolates a crashing entry without collapsing siblings', () => {
    ...
    expect(view.container.querySelector('[role="alert"]')?.textContent).toContain('这个区域暂时无法显示')
    expect(view.container.querySelector('button')?.textContent).toBe('重新加载')
  })
})
