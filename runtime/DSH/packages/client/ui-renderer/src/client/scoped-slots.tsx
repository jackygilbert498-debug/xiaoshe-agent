 ...
 */
import { Component, useMemo, useState, useSyncExternalStore, type CSSProperties, type FC, type ReactNode } from 'react'
import {
...
const CRASH_FACE_STYLE: CSSProperties = {
  display: 'grid',
  alignContent: 'center',
  justifyItems: 'start',
  gap: '8px',
  ...
  minHeight: '96px',
  padding: '20px',
  border: '1px solid color-mix(in srgb, var(--dsw-text, #243129) 14%, transparent)',
  borderRadius: '14px',
  color: 'var(--dsw-text, #243129)',
  background: 'color-mix(in srgb, var(--dsw-surface, #fff) 92%, var(--dsw-accent, #789381) 8%)',
  fontFamily: 'var(--dsw-font-sans, system-ui, sans-serif)',
}

function SlotCrashFace({ slotKey, onRetry }: { slotKey: string; onRetry?: () => void }) {
  const isRoot = slotKey === 'root'
  const style: CSSProperties = isRoot
    ? { ...CRASH_FACE_STYLE, minHeight: '100dvh', border: 0, borderRadius: 0, padding: 'clamp(24px, 8vw, 112px)' }
    : CRASH_FACE_STYLE
  return (
    <section data-slot-error={slotKey} role="alert" aria-live="assertive" style={style}>
      <strong style={{ fontSize: isRoot ? 'clamp(20px, 3vw, 32px)' : '15px', fontWeight: 650 }}>
        {isRoot ? '界面暂时无法显示' : '这个区域暂时无法显示'}
      </strong>
      <span style={{ maxWidth: '46ch', color: 'color-mix(in srgb, currentColor 68%, transparent)', lineHeight: 1.6 }}>
        运行组件遇到错误。已保留诊断记录，你可以重新加载后继续。
      </span>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '4px' }}>
        {onRetry === undefined ? null : (
          <button type="button" onClick={onRetry}>重试界面</button>
        )}
