...
export interface WorkspaceListState {
  ...
  /**
   * Non-zero while an unscoped user New Session gesture needs the resident
   * conversation surface to open its Workspace picker. The UI acknowledges
   * the exact token after adopting the request, so a later gesture remains
   * distinguishable even when no Session selection changed.
   */
...
export class WorkspaceRuntime implements IWorkspaces {
  ...
  /** Currently unacknowledged picker request retained for compatibility. */
  private workspacePickerRequestId = 0
  ...
  constructor(ctx: Context, private readonly api: IApiClient, private readonly sessions: SessionsPort) {
    ...
    this.list = createSnapshotStore<WorkspaceListState>({
      ...
      baselinesReady: false, recentWorkspaceId: undefined, workspacePickerRequestId: 0,
    })
   ...
   * button, workspace browser): an explicit Workspace creates there; an
   * unscoped gesture creates an independent conversation in the Host-owned
   * loose-conversation root. Startup restoration remains separate and may
   * still use the recent Workspace.
  ...
  startSession(workspaceId?: WorkspaceId): void {
    if (workspaceId === undefined) {
      void this.sessions.create({ loose: true }).then(
  ...
  /**
   * Consume the exact Workspace-picker request adopted by the UI. A stale
   * acknowledgement cannot erase a newer New Session gesture.
   * @param requestId - token observed from the Workspace list snapshot.
   */
  acknowledgeWorkspacePickerRequest(requestId: number): void {
    if (this.workspacePickerRequestId !== requestId) return
    this.workspacePickerRequestId = 0
    this.project()
  }
  ...
  /** Move a temporary conversation into a Workspace while retaining completed history. */
  async moveSessionToWorkspace(sessionId: SessionId, workspaceId: WorkspaceId): Promise<SessionId> {
    ...
    const targetId = source.blank
      ? await this.sessions.create({ workspaceId })
      : await this.sessions.fork({ sessionId, workspaceId, targetCwd: workspace.path, increaseTitle: false })
    await this.archiveSession(sessionId)
    this.sessions.open(targetId)
    return targetId
  }
  ...
  private project(): void {
    ...
    this.list.set({
      ...
      workspacePickerRequestId: this.workspacePickerRequestId,
    })
  }
}
