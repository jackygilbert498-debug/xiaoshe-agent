...
export interface IWorkspaces {
  ...
  /**
   * The New Session flow: connect the explicit or current-Session Workspace
   * and open the resulting session. Without either, request the Workspace
   * picker instead of silently using recency.
   * @param workspaceId - explicit target; omitted inherits the current
   * Session's Workspace.
   */
  startSession(workspaceId?: WorkspaceId): void
  /** Acknowledge the exact Workspace-picker request adopted by the UI. */
  ...
  /** Move a temporary conversation into a Workspace while retaining history. */
