...
export class SessionManager {
   ...
   */
  async create(
    opts: { workspaceId?: WorkspaceId; cwd?: string; sessionId?: SessionId; loose?: boolean } = {},
  ): Promise<RpcResult<{ sessionId: SessionId }>> {
    try {
      const shared = opts.sessionId === undefined ? {} : { sessionId: opts.sessionId }
      const payload = opts.workspaceId !== undefined
        ? { workspaceId: opts.workspaceId, ...shared }
        : { ...(opts.cwd === undefined ? {} : { cwd: opts.cwd }), ...(opts.loose === true ? { loose: true } : {}), ...shared }
   ...
   */
  async fork(
    opts: { sessionId: SessionId; atSeq?: number; workspaceId?: WorkspaceId; targetCwd?: string },
  ): Promise<RpcResult<{ sessionId: SessionId }>> {
    try {
      ...
      if (childId !== undefined) {
        const childCwd = opts.targetCwd ?? source?.cwd
        this.recordMutation({ kind: 'upsert', summary: {
