...
describe('WorkspaceRuntime', () => {
  ...
  it('targets an explicit Workspace or creates an independent conversation when unscoped', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    ...
    const create = vi.spyOn(sessions, 'create').mockResolvedValue(sid('loose-new'))
    const open = vi.spyOn(sessions, 'open').mockImplementation(() => {})
    workspaces.startSession()
    await Promise.resolve()
    expect(create).toHaveBeenCalledWith({ loose: true })
    expect(open).toHaveBeenCalledWith(sid('loose-new'))
    expect(connect).toHaveBeenCalledTimes(1)
  })

  it('moves a completed independent conversation into a Workspace by forking then archiving', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    ...
    await Promise.all([workspaces.refresh(), sessions.refresh()])
    const fork = vi.spyOn(sessions, 'fork').mockResolvedValue(sid('moved'))
    const archive = vi.spyOn(workspaces, 'archiveSession').mockResolvedValue()
    const open = vi.spyOn(sessions, 'open').mockImplementation(() => {})

    await expect(workspaces.moveSessionToWorkspace(sid('source'), wid('target'))).resolves.toBe(sid('moved'))
    expect(fork).toHaveBeenCalledWith({
      sessionId: sid('source'), workspaceId: wid('target'), targetCwd: '/w/target', increaseTitle: false,
    })
    expect(archive).toHaveBeenCalledWith(sid('source'))
    expect(open).toHaveBeenCalledWith(sid('moved'))
  })
