...
New Session immediately creates a projectless Session in the Host-owned loose-conversation root, so conversation no longer depends on choosing a Workspace first. The smaller New Project action beside it opens the composed directory flow, registers the selected directory as a Workspace, and starts a Session there. Workspace-specific controls and the shared picker belong to ui-workspace.
