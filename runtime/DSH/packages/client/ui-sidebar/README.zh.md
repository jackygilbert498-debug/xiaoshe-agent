...
新建会话会立即在 Host 持有的临时会话根目录中创建一条不归属项目的 Session，因此用户无须先选 Workspace。它旁边较小的「新建项目」操作会打开组合提供的目录流程，把选中的目录注册为 Workspace，并在其中启动 Session。Workspace 专属控件与共享选择器由 ui-workspace 持有。
