...
describe('SidebarRoot.module.css', () => {
  ...
  it('gives the slotted brand enough vertical ink room to avoid clipping', () => {
    expect(declarations('.brandIdentity')?.get('min-height')).toBe('46px')
    expect(declarations('.brandName')?.get('min-height')).toBe('46px')
    expect(declarations('.brandName')?.get('line-height')).toBe('1.45')
    expect(declarations('.brandName')?.get('overflow')).toBe('visible')
