...
it('destroys an unread upload and aborts the request signal when its handler throws', async () => {
  const request = new Readable({ read() {} }) as unknown as IncomingMessage
  Object.assign(request, { url: '/api/session/uploadFileBinary', method: 'POST', headers: {} })
  const response = Object.assign(new EventEmitter(), { writableEnded: false }) as unknown as ServerResponse
  let signal: AbortSignal | undefined
  const failure = new Error('synthetic handler failure')
  await expect(bridge(request, response, {
    requestBodyMode: () => 'streaming',
    fetch: input => { signal = input.signal; throw failure },
