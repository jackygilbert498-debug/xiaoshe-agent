...
export function AgentPresetSeat({ load, select, introduced, useAgentPresetSeat, t }: AgentPresetSeatProps) {
  ...
  const optionItem = (option: AgentPresetSeatState['options'][number], icon?: MenuItem['icon']): MenuItem => {
  ...
  const automatic = state.options.find(option => option.id === 'standard')
  const creator = state.options.find(option => option.id === 'cordis')
  const manual = state.options.filter(option => option.id !== 'standard' && option.id !== 'cordis')
  const menuItems: MenuEntry[] = [
    ...(automatic === undefined
      ? []
      : [optionItem(automatic, <IconSparkle16 />)]),
    ...(manual.length === 0
      ? []
      : [{
          id: 'manual-settings',
          ...
          label: (
            <span className={css.item}>
              <span className={css.itemName}>{t('manualSettingsName')}</span>
              <span className={css.itemDesc}>{t('manualSettingsDescription')}</span>
            </span>
          ),
          submenu: manual.map(option => optionItem(option)),
        }]),
  ]
  const menuFooter: MenuEntry[] = creator === undefined
    ? []
    : [optionItem(creator, <IconPlusOutline16 />)]
  ...
  return (
    <Menu
      ...
      anchor={(
        ...
        >
          <span
            data-slot="agent-preset.auto-icon"
            className={introducing ? `${css.seatIconWrap} ${css.introIcon}` : css.seatIconWrap}
          >
            <IconSparkle16 className={css.seatIcon} />
          </span>
          {shownLabel}
