...
describe('the new-session chip', () => {
  ...
  it('offers each preset with what it is for', () => {
    ...
    fireEvent.focus(screen.getByRole('menuitem', { name: /Manual settings/ }))
  ...
  it('keeps automatic choice prominent and groups specialist choices behind manual settings', () => {
    renderSeat({
      options: [
        { id: 'standard', trust: 'system', name: 'Standard mode' },
        { id: 'code', trust: 'system', name: 'PTC mode' },
        { id: 'minimal', trust: 'system', name: 'Minimal mode' },
        { id: 'cordis', trust: 'system', name: 'Creator mode' },
        { id: 'mine', trust: 'user', name: 'My preset' },
      ],
    })

    const trigger = screen.getByRole('button')
    expect(trigger.querySelector('[data-slot="agent-preset.auto-icon"]')).not.toBeNull()
    fireEvent.click(trigger)

    expect(screen.getByText(en.presetStandardDescription)).toBeTruthy()
    const manual = screen.getByRole('menuitem', { name: /Manual settings/ })
    expect(screen.getByRole('menuitem', { name: /Create an agent/ })).toBeTruthy()
    expect(screen.queryByText(en.presetCodeDescription)).toBeNull()

    fireEvent.focus(manual)

    expect(screen.getByText(en.presetCodeDescription)).toBeTruthy()
    expect(screen.getByText(en.presetMinimalDescription)).toBeTruthy()
