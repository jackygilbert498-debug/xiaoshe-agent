...
/** Keep the canonical view usable by replacement shells without dead settings. */
export interface Config { registerSettings?: boolean }
export const Config: z<Config> = z.object({ registerSettings: z.boolean().default(true) })
...
export function apply(ctx: Context, config: Config = Config({})): void {
  ...
  if (config.registerSettings !== false) ctx.slots.inject('settings.general.item', () => ctx.slots.register({
