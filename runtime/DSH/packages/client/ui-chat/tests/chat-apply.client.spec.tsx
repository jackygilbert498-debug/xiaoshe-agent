...
async function bench(options: { registerSettings?: boolean } = {}) {
  ...
  const chat = await runtime.mount({ inject: [...injectChat], apply: ctx => applyChat(ctx, options) })
...
describe('Chat apply wiring', () => {
  it('replacement shells retain canonical chat without unused transcript settings', async () => {
    const b = await bench({ registerSettings: false })
    expect(b.sourceDescriptor).toBeDefined()
