...
> & {
  ...
  /** Open the project-assignment dialog for a conversation. */
              ...
              // browser-local account; real Workspaces may also write Host order.
                ...
                return (
                  <SessionNodeItem
                    ...
                    onMove={group.workspaceId === undefined ? onSessionMove : undefined}
                    drag={dragProps}
                    t={t}
                  />
                )
...
function FlatList({
  useSessions, open, forkSession, onSessionRename, onSessionArchive, onSessionMove, archivedSessionIds,
...
export function WorkspaceBrowser({
  ...
  const confirmMove = (workspaceId: WorkspaceId) => {
    if (moveTarget === null || moving || moveSessionToWorkspace === undefined) return
    setMoving(true)
    setMoveError(null)
    moveSessionToWorkspace(moveTarget, workspaceId).then(() => {
      setMoving(false)
      setMoveTarget(null)
    }).catch((reason: unknown) => {
      setMoving(false)
  ...
  return (
    <div className={clsx(css.root, !wide && css.rail)}>
      ...
      >
        <div className={css.workspaceChoices}>
          {workspaces.length === 0
            ? <div className={css.empty}>{t('move.session.empty')}</div>
            : workspaces.map(workspace => (
              <button
                key={workspace.workspaceId}
                type="button"
                className={css.workspaceChoice}
                disabled={moving}
                onClick={() => { confirmMove(workspace.workspaceId) }}
              >
        ...
        {moving && <div className={css.deleteStatus} role="status">{t('move.session.pending')}</div>}
