...
describe('WorkspaceBrowser', () => {
  ...
  it('moves an ungrouped conversation into a selected project', async () => {
    const moveSessionToWorkspace = vi.fn(async (): Promise<void> => {})
    mount({
      useSessions: hook(sessionState([summary('loose', 1)], { current: sid('loose') })),
      useWorkspaces: hook(workspaceState([workspace('alpha', [])], [])),
    ...
    await waitFor(() => { expect(moveSessionToWorkspace).toHaveBeenCalledWith(sid('loose'), wid('alpha')) })
