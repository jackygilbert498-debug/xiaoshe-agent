...
describe('command launcher chrome and control seats', () => {
  ...
  it('the Access chip renders the projection value and submits a non-autonomous pick directly', async () => {
    ...
    expect(busy.textContent).toBe('项目内执行')
  ...
  it('requires explicit risk acknowledgement before submitting Autonomous execution', async () => {
    const command = vi.fn(() => Promise.resolve(true))
    const permissions = {
      options: [
        { value: 'workspace-write', name: 'workspace-write' },
    ...
    fireEvent.click(view.getByRole('menuitem', { name: '自主执行' }))

    expect(command).not.toHaveBeenCalled()
    expect(view.getByRole('dialog', { name: '确认启用自主执行？' })).toBeTruthy()
    const enable = view.getByRole('button', { name: '启用自主执行' }) as HTMLButtonElement
    ...
    expect((view.getByLabelText(/^访问模式/) as HTMLButtonElement).textContent).toBe('自主执行')
    await act(async () => {})
  })

  it('cancels an Autonomous selection without changing permission and resets acknowledgement', () => {
    const command = vi.fn(() => Promise.resolve(true))
    const permissions = {
      options: [
        { value: 'workspace-write', name: 'workspace-write' },
    ...
    expect(command).not.toHaveBeenCalled()
    expect((view.getByLabelText(/^访问模式/) as HTMLButtonElement).textContent).toBe('项目内执行')

    openConfirmation()
    expect((view.getByRole('checkbox') as HTMLInputElement).checked).toBe(false)
    expect((view.getByRole('button', { name: '启用自主执行' }) as HTMLButtonElement).disabled).toBe(true)
  })

  it('revokes an open Autonomous confirmation when the task locks', () => {
    const command = vi.fn(() => Promise.resolve(true))
    const permissions = {
      options: [
        { value: 'workspace-write', name: 'workspace-write' },
  ...
  it('resets an open Autonomous confirmation when switching tasks', () => {
    const command = vi.fn(() => Promise.resolve(true))
    const permissions = {
      options: [
        { value: 'workspace-write', name: 'workspace-write' },
