...
const workspaceState = (items: readonly WorkspaceView[]): WorkspaceListState => ({
  ...
  baselinesReady: true, recentWorkspaceId: undefined, workspacePickerRequestId: 0,
})
...
function mount(
  ...
  const stop = vi.fn()
  const open = vi.fn()
  const acknowledgeWorkspacePickerRequest = vi.fn()
  ...
  const props: ConversationRootProps = {
    ...
    acknowledgeWorkspacePickerRequest,
    t,
  }
...
describe('ConversationRoot resident composer', () => {
  it.each([
    ['idle', {}],
    ['waiting-model', { running: true }],
    ['model-running', { running: true, partial: { blocks: [] } as never }],
    ['tool-running', { running: true, runningCalls: [{} as never] }],
    ['awaiting-approval', { running: true, pending: [{ kind: 'approval' } as never] }],
    ['stopped', { nodes: [{ kind: 'assistant', interrupted: true } as never] }],
  ] as const)('publishes the authoritative %s runtime state', (state, overrides) => {
    const b = mount(conversationSnapshot(overrides as Partial<ConversationSnapshot>))
    expect(b.view.container.querySelector('[data-phase]')?.getAttribute('data-session-runtime-state')).toBe(state)
  })

  it('opens the Workspace picker for each unscoped New Session request and acknowledges it', () => {
    const b = mount(conversationSnapshot({ composerPhase: 'blank' }))
    expect((b.pickerOwner() as { open: boolean }).open).toBe(false)

    act(() => {
      b.workspaces.set({ ...b.workspaces.getSnapshot(), workspacePickerRequestId: 7 })
    })
    expect((b.pickerOwner() as { open: boolean }).open).toBe(true)
    expect(b.acknowledgeWorkspacePickerRequest).toHaveBeenCalledWith(7)

    act(() => { (b.pickerOwner() as { onClose: () => void }).onClose() })
    expect((b.pickerOwner() as { open: boolean }).open).toBe(false)
    act(() => {
      b.workspaces.set({ ...b.workspaces.getSnapshot(), workspacePickerRequestId: 8 })
    })
    expect((b.pickerOwner() as { open: boolean }).open).toBe(true)
    expect(b.acknowledgeWorkspacePickerRequest).toHaveBeenLastCalledWith(8)
  })
