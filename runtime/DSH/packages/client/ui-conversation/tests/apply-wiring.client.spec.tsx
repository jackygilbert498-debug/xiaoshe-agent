...
async function bench(options: { declareConversation?: boolean; registerSettings?: boolean } = {}) {
  ...
  const feature = await runtime.mount({ inject: [...inject], apply: ctx => apply(ctx, { maxConcurrentFileUploads: 2, ...options }) })
...
describe('target-neutral Conversation apply wiring', () => {
  it('replacement shells retain assembly without irrelevant composer settings', async () => {
    const b = await bench({ registerSettings: false })
    expect(b.runtime.ctx.get('uiConversation')).toBeDefined()
    expect(b.runtime.slots.entries('settings.general.item')).toHaveLength(0)
