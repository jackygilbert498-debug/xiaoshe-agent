...
import type { ConversationSnapshot, WorkspaceId } from '@deepseek-ai/dsh-client-runtime/client'
...
/** Publish one semantic activity state for product shells without making them inspect localized DOM copy. */
export function sessionRuntimeState(session: ConversationSnapshot | undefined): SessionRuntimeState {
  if (session?.pending.some(interaction => interaction.kind === 'approval') === true) return 'awaiting-approval'
  if (session?.running === true) {
    if (session.runningCalls.length > 0) return 'tool-running'
    return session.partial === null ? 'waiting-model' : 'model-running'
  }
  const latestAssistant = session?.nodes.findLast(node => node.kind === 'assistant')
  return latestAssistant?.kind === 'assistant' && latestAssistant.interrupted === true ? 'stopped' : 'idle'
}

export function ConversationRoot({
  sessionId, useSession, useSessions, useWorkspaces, useInput, useComposerBlock,
  renderSlot, renderSlotChain, selectWorkspace, acknowledgeWorkspacePickerRequest, t,
  ...
  const workspacePickerRequestId = workspaces.workspacePickerRequestId ?? 0
  useEffect(() => {
    if (workspacePickerRequestId === 0) return
    setPickerOpen(true)
    acknowledgeWorkspacePickerRequest(workspacePickerRequestId)
  }, [workspacePickerRequestId, acknowledgeWorkspacePickerRequest])
  ...
  return (
    <div className={css.root} data-phase={phase} data-session-runtime-state={runtimeState}>
