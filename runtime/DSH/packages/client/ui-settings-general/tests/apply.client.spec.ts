...
describe('ui-settings-general apply', () => {
  it('deduplicates overridden General rows and restores standalone controls when Appearance leaves', async () => {
    const { ctx, slots } = await bench()
    const releaseRoot = declare(slots)
    await ctx.plugin({ inject: [...inject], apply }).await()
    const item = (id: string, priority = 0) => slots.register({ name: 'settings.general.item', id, priority }, () => null)
    const releaseItems = [item('language'), item('language', 10), item('font-size'), item('appearance'), item('composer-enter'), item('transcript-view'), item('new-feature')]
    const source = (generalEntry(slots)!.inject as unknown as () => { hooks: { itemIds: { getSnapshot(): readonly string[] | undefined } } })().hooks.itemIds
    expect(source.getSnapshot()).toBeUndefined()
    const releaseAppearance = slots.register({ name: 'settings.section', id: 'appearance' }, () => null)
    expect(source.getSnapshot()).toEqual(['language', 'new-feature'])
    expect(source.getSnapshot()).toBe(source.getSnapshot())
    releaseAppearance()
    expect(source.getSnapshot()).toBeUndefined()
    releaseItems.forEach(release => release())
    releaseRoot()
  })
