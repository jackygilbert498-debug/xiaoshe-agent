...
describe('GeneralSection', () => {
  ...
  it('renders each remaining feature row when Appearance owns the theme controls', () => {
    const renderSlot = vi.fn(() => null)
    render(<GeneralSection {...kit} close={() => {}} renderSlot={renderSlot}
      useItemIds={select => select(['language', 'xiaoshe-composer-enter', 'new-plugin-option'])} />)
    expect(renderSlot.mock.calls).toEqual([
      ['settings.general.item', {}, { only: 'language' }],
      ['settings.general.item', {}, { only: 'xiaoshe-composer-enter' }],
      ['settings.general.item', {}, { only: 'new-plugin-option' }],
    ])
  })
})
