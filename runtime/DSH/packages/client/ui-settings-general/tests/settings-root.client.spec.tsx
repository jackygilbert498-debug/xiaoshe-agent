...
it('keeps keyboard focus inside settings and respects an already handled Escape', () => {
  mount()
...
it('groups settings by purpose without losing an unknown plugin section', () => {
  mount({ dictionary: zh, rows: [
    { id: 'appearance', order: 5, label: '外观' },
    { id: 'models', order: 10, label: '模型' },
    { id: 'custom-plugin', order: 50, label: '我的插件' },
  ] })
  fireEvent.click(screen.getByRole('button', { name: '设置' }))
  expect(screen.getByText('使用偏好')).toBeTruthy()
  expect(screen.getByText('模型与能力')).toBeTruthy()
  fireEvent.click(screen.getByRole('button', { name: '我的插件' }))
  expect(screen.getByTestId('section-custom-plugin')).toBeTruthy()
})

it('exposes group headings as labels rather than navigation controls', () => {
  mount({ dictionary: zh, rows: [
    { id: 'general', order: 0, label: '通用设置' },
    { id: 'appearance', order: 5, label: '外观' },
    { id: 'security', order: 10, label: '权限与安全' },
  ] })
  fireEvent.click(screen.getByRole('button', { name: '设置' }))
  const heading = screen.getByRole('heading', { name: '使用偏好' })
  expect(screen.queryByRole('button', { name: '使用偏好' })).toBeNull()
  ...
  expect(appearance.querySelector('[data-xs-settings-nav-check]')).toBeTruthy()
  expect(screen.getByRole('button', { name: '通用设置' }).querySelector('[data-xs-settings-nav-check]')).toBeNull()
  expect(appearance.querySelector('[data-xs-settings-nav-glyph]')?.getAttribute('aria-hidden')).toBe('true')
})
...
describe('Xiaoshe settings shell boundary', () => {
  it('exposes stable hooks for native-shell focus isolation and responsive styles', () => {
    mount()
    const trigger = openPanel()
    const panel = screen.getByRole('dialog')
    expect(panel.hasAttribute('data-xs-settings-panel')).toBe(true)
    expect(panel.closest('[data-xs-settings-overlay]')).not.toBeNull()
    for (const name of ['nav', 'nav-title', 'nav-list', 'content', 'header', 'actions', 'close', 'options']) {
      expect(panel.querySelector('[data-xs-settings-' + name + ']'), name).not.toBeNull()
    }
    expect(panel.querySelector('[data-xs-settings-nav-item="models"]')).not.toBeNull()
    fireEvent.click(panel.querySelector('[data-xs-settings-close]')!)
