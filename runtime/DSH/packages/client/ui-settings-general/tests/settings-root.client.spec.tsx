...
describe('SettingsPanel chrome seats', () => {
  it('names the dialog via aria-labelledby pointing at the header seat node', () => {
    ...
    expect(dialog.getAttribute('data-xs-settings-panel')).toBe('')
    expect(dialog.parentElement?.getAttribute('data-xs-settings-overlay')).toBe('')
    expect(dialog.querySelector('[data-xs-settings-nav]')).toBeTruthy()
    expect(dialog.querySelector('[data-xs-settings-content]')).toBeTruthy()
    expect(dialog.querySelector('[data-xs-settings-header]')).toBeTruthy()
    expect(dialog.querySelector('[data-xs-settings-options]')).toBeTruthy()
...
describe('SettingsPanel close paths', () => {
  it('restores focus to the settings trigger after closing', async () => {
    mount()
    const trigger = screen.getByRole('button', { name: 'Settings' })
    fireEvent.click(trigger)
...
describe('SettingsPanel navigation', () => {
  it('projects rows, marks the first active, and renders only that section', () => {
    ...
    expect(general.getAttribute('data-xs-settings-nav-item')).toBe('general')
