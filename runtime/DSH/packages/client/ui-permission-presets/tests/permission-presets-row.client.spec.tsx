...
describe('PermissionRow', () => {
  it('loads the descriptor, opens the menu, and selects a new default', async () => {
    ...
    fireEvent.click(button)
    fireEvent.click(screen.getByRole('menuitem', { name: 'Project access' }))
    await screen.findByRole('button', { name: 'Project access' })
  ...
  it('requires explicit acknowledgement before saving Full access', async () => {
    ...
    fireEvent.click(screen.getByRole('menuitem', { name: 'Autonomous' }))
    expect(mutate).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
    expect(screen.queryByRole('dialog', { name: 'Enable Full access?' })).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: 'Read only' }))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Autonomous' }))
    const dialog = screen.getByRole('dialog', { name: 'Enable Autonomous execution?' })
    const enable = screen.getByRole('button', { name: 'Enable Autonomous execution' })
