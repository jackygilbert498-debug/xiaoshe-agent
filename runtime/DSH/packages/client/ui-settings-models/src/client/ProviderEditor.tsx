...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  ...
  const materializesEmptyNativeProfile = layout === 'pi-ai'
    && fallback === undefined
    && committedOriginal === undefined
    && Object.keys(draft).length === 0
  const settingsDirty = props.credentialOnly !== true
    && (materializesEmptyNativeProfile || pathOps(settingsPath, committedOriginal, draft).length > 0)
  const formDirty = settingsDirty || keyValue.length > 0
