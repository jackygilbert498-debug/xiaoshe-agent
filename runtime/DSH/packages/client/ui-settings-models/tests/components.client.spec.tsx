...
describe('ModelsSection', () => {
  ...
  it('drops the typed text when reset replaces the rows it annotated', async () => {
    ...
    // Reset put the draft back where it started, so Save stays unavailable
    // rather than pretending an unchanged form can commit.
  ...
  it('keeps an existing provider save disabled until the user changes something', async () => {
    await mountSection()
    fireEvent.click(screen.getByRole('button', { name: openaiCopy(en.editProvider) }))
    const save = await screen.findByText<HTMLButtonElement>(en.apply)
    expect(save.disabled).toBe(true)
    fireEvent.change(screen.getByLabelText<HTMLInputElement>(en.keyInput), { target: { value: 'sk-new' } })
    expect(save.disabled).toBe(false)
  })
  ...
  it('keeps pi-ai provider-native authentication when no key is entered', async () => {
    const { mutate, set } = await mountSection()
    fireEvent.click(screen.getByText(en.add))
    fireEvent.change(await screen.findByLabelText(en.provider), { target: { value: 'anthropic' } })
    fireEvent.click(screen.getByText(en.apply))
  ...
  it('switches the add card target and degrades unknown or broken targets loudly', async () => {
    ...
    fireEvent.change(screen.getByLabelText<HTMLSelectElement>(en.provider), { target: { value: 'plain' } })
    await waitFor(() => {
