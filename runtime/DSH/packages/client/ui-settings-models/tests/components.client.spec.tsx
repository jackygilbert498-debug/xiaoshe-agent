...
describe('ModelsSection', () => {
  it('starts adding with an explicit empty choice and grouped provider purposes', async () => {
    const scripted = scriptedFace()
    scripted.face.llm.listConfigurableProviders.mockResolvedValue(remoteOk([
      { provider: 'openai', displayName: 'openai', settingsNs: 'llm-pi-ai', settingsPath: ['providers', 'openai'], active: true },
      ...['anthropic', 'azure-openai', 'openrouter', 'ollama'].map(provider => ({
        provider, displayName: provider, settingsNs: 'llm-pi-ai', settingsPath: ['providers', provider], active: false,
      })),
    ]))
    ...
    expect(screen.getByRole<HTMLOptionElement>('option', { name: en.chooseProvider }).disabled).toBe(true)
    expect([...pick.querySelectorAll('optgroup')].map(group => group.label)).toEqual([
      en.directProviderGroup, en.cloudProviderGroup, en.gatewayProviderGroup, en.localProviderGroup,
    ])
    expect(screen.queryByLabelText(en.keyInput)).toBeNull()
    fireEvent.change(pick, { target: { value: 'azure-openai' } })
    expect(screen.getByText(en.cloudProviderPurpose)).toBeTruthy()
  ...
  it('drops the typed text when reset replaces the rows it annotated', async () => {
    ...
    // Xiaoshe preserves its existing no-op guard: a reset to the original
    // draft disables Apply instead of pretending a commit occurred.
