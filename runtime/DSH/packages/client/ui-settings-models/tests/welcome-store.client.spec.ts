...
describe('WelcomeNoticeStore', () => {
  it.each([undefined, 'degraded'] as const)('does not accept an acknowledged last-safe value with namespace status %s', async (status) => {
    const view = { ...acknowledgedNamespace(WELCOME_NOTICE_VERSION), status }
    const describeCall = vi.fn(() => Promise.resolve(ok({
      writable: true, hasDocument: false, namespaces: [view],
    })))
    const mutate = vi.fn(() => Promise.resolve(ok(view)))
    const { mirror, controller } = buildWelcome({ describe: describeCall, mutate })
    await mirror.load()
    await controller.load()
    expect(controller.store.getSnapshot()).toMatchObject({ status: 'error', acknowledged: false })
    expect(controller.store.getSnapshot().error).toBeTruthy()
