...
import {
  CENTER_MIN, DETAILS_DEFAULT, SIDEBAR_COLLAPSED, SIDEBAR_DEFAULT,
...
describe('AppFrame', () => {
  ...
  it('ignores unselected states and closes only when the Session id changes', () => {
    ...
    expect(tracks(frame)).toEqual([SIDEBAR_DEFAULT, DETAILS_DEFAULT])
  ...
  it('sidebar drag widens through rAF-batched pointer moves', () => {
    ...
    drag(handles[0]!, SIDEBAR_DEFAULT, SIDEBAR_DEFAULT + 70)
    expect(tracks(frame)[0]).toBe(SIDEBAR_DEFAULT + 70)
  })
  ...
  it('drag base is the rendered (concession-clamped) width, not the preference', () => {
    frameWidth = 1220 // step-2 squeeze: details renders 332 while preference is 360
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openDetails() })
...
describe('AppFrame — guard branches', () => {
  ...
  it('two moves inside one frame coalesce through the pending rAF', () => {
    ...
    act(() => {
      ...
      handle.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 60, bubbles: true }))
      vi.advanceTimersByTime(20)
    })
    act(() => { handle.dispatchEvent(new PointerEvent('pointerup', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 60, bubbles: true })) })
    expect(instance.getSnapshot().sidebar).toBe(SIDEBAR_DEFAULT + 60)
  })
  ...
  it('pointerup with a pending rAF cancels it and commits the final position', () => {
    ...
    act(() => { handle.dispatchEvent(new PointerEvent('pointerdown', { pointerId: 1, clientX: SIDEBAR_DEFAULT, bubbles: true })) })
    act(() => {
      handle.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 80, bubbles: true }))
      // No timer advance: the rAF is still pending when pointerup arrives.
      handle.dispatchEvent(new PointerEvent('pointerup', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 80, bubbles: true }))
    })
    expect(instance.getSnapshot().sidebar).toBe(SIDEBAR_DEFAULT + 80)
  })
