...
describe('computeColumns', () => {
  it('uses the Xiaoshe-balanced open sidebar width', () => {
    expect(SIDEBAR_MIN).toBe(248)
  ...
  it('step 2: details shrinks first, center pinned at min', () => {
    // 248 + 360 + 640 = 1248 > 1220; details concedes to 1220-248-640 = 332.
    const cols = computeColumns(1220, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: CENTER_MIN, details: 332 })
  })
  ...
  it('step 3: details auto-closes when its min still starves center — sidebar holds its preference', () => {
    // 248 + 300 + 640 = 1188 > 1178 → details 0; sidebar untouched: center = 930.
    const cols = computeColumns(1178, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: 930, details: 0 })
  })

  it('the sidebar never concedes: center absorbs the deficit below CENTER_MIN', () => {
    // 700 < 248+640: sidebar keeps 248, center takes 452 < CENTER_MIN.
    const cols = computeColumns(700, open(SIDEBAR_DEFAULT), closed(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: 452, details: 0 })
  })
