...
export class TestWorkspaces implements IWorkspaces {
  ...
  /**
   * Acknowledge a Workspace-picker request (recorded; default no-op).
   * @param requestId - exact request token adopted by the conversation UI.
   */
  acknowledgeWorkspacePickerRequest(requestId: number): void {
    this.calls.push({ method: 'acknowledgeWorkspacePickerRequest', args: [requestId] })
    this.stubs.get('acknowledgeWorkspacePickerRequest')?.(requestId)
  }
  ...
  /** Move a session into a workspace (recorded). */
  async moveSessionToWorkspace(sessionId: SessionId, workspaceId: WorkspaceId): Promise<SessionId> {
    this.calls.push({ method: 'moveSessionToWorkspace', args: [sessionId, workspaceId] })
    const stub = this.stubs.get('moveSessionToWorkspace')
    if (stub !== undefined) return await (stub(sessionId, workspaceId) as Promise<SessionId>)
    return sessionId
  }
